  ...
  <AmitySocialHomePage />
    ...
    class MyApp extends StatelessWidget {
      ...
      Widget build(BuildContext context) {
        // Do NOT wrap this in your own MaterialApp — AmityUIKitProvider builds
        // its own MaterialApp internally (navigatorKey, theme, localizations).
        return AmityUIKitProvider(
          child: Builder(
            // `context` here is BELOW the provider, so the view models that
            // registerDevice() needs are available.
    ...
    /// Logs the user in, then shows the Social home page only once the
    ...
    class _AuthGateState extends State<AuthGate> {
      ...
      void initState() {
        ...
        // Show the home page as soon as the session becomes Established.
        AmityUIKit().observeSessionState().listen((state) {
          if (!mounted) return;
          if (state == SessionState.Established) {
            setState(() => _established = true);
          }
        });

        // Register the user (see login() from the previous step).
        AmityUIKit().registerDevice(
          context: context,
          userId: 'USER_ID',
          displayName: 'DISPLAY_NAME', // optional
          // authToken: 'AUTH_TOKEN',  // only when secure mode is enabled
          callback: (isSuccess, error) {
            if (!isSuccess && mounted) setState(() => _error = error);
          },
        );
      }
      ...
      Widget build(BuildContext context) {
        ...
        // Logged in → show the Social home page.
        return const SocialHomePageConfigProviderWidget();
      }
    }
    ```
    </CodeGroup>

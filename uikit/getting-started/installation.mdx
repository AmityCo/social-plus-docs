...
<Tabs>
  ...
  <Tab title="React Native">
    ...
    **Simple Integration:**
    ...
    From v4.3.1, each chat page can also be rendered on its own inside `AmityPageRenderer` — see [Chat Components](/uikit/components/chat/overview#quick-start-example).
    ...
    Instead of mounting the whole UIKit, you can render a single page or component inside `AmityPageRenderer`, which sets up the navigation the UIKit screens need.
    ...
    export default function App() {
      return (
        ...
        >
          <AmityPageRenderer>
            <CommunityProfilePage defaultCommunityId="community123" />
          </AmityPageRenderer>
        </AmityUiKitProvider>
      );
    }
    ...
    For the pages and components you can render this way, see the React Native examples in [Social Components](/uikit/components/social/overview) and, from v4.3.1, [Chat Components](/uikit/components/chat/overview).

    <Info>
      **Note**: `AmityPageRenderer` is only required when using individual screen components. It's not needed for `AmityUiKitSocial` or `AmityUiKitChat`.
    </Info>

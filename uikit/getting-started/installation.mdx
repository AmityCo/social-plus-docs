...
<Tabs>
  ...
  <Tab title="React Native">
    ...
    <Info>
      React Native UIKit v4 is **generally available** and published on npm as [`@amityco/react-native-social-uikit`](https://www.npmjs.com/package/@amityco/react-native-social-uikit).
    </Info>
    ...
    | Android | `minSdkVersion` 24, `compileSdkVersion` 36, `targetSdkVersion` 36, `buildToolsVersion` 36.0.0 |

    React Native CLI is required (not Expo).

    ### Install the UIKit

    ```bash
    # Install the UIKit
    yarn add @amityco/react-native-social-uikit

    # Install required peer dependencies
    yarn add @amityco/ts-sdk-react-native@^7.28.0 react-native-fs@2.20.0 react-native-reanimated@~4.3.4 react-native-worklets@~0.8.3 @livekit/react-native @livekit/react-native-webrtc livekit-client @react-native-firebase/app @react-native-firebase/messaging @react-native-async-storage/async-storage @react-native-camera-roll/camera-roll @react-native-clipboard/clipboard @react-native-community/datetimepicker "@react-native-community/netinfo@>=11.5.0" @react-navigation/native @react-navigation/native-stack @react-navigation/stack react-native-compressor react-native-gesture-handler react-native-get-random-values react-native-haptic-feedback react-native-image-picker react-native-linear-gradient react-native-safe-area-context react-native-screens react-native-svg react-native-video react-native-video-controls react-native-vision-camera @babel/plugin-transform-export-namespace-from

    # Build-time only
    yarn add -D @react-native/babel-preset @react-native/metro-config
    ```

    <Note>
      `react-native-fs` is pinned to `2.20.0`, and `react-native-reanimated` (`~4.3.4`) and `react-native-worklets` (`~0.8.3`) are pinned to a patch range. Installing versions outside these is unsupported. `@amityco/ts-sdk-react-native` must be `7.28.0` or later.
    </Note>

    <Info>
      **Upgrading from 4.2.x:** v4.3.0 adds `react-native-reanimated`, `react-native-worklets`, and `@react-native-camera-roll/camera-roll` as peer dependencies, and raises `@react-native-community/netinfo` to `>=11.5.0`. The `react-native-netinfo` package is no longer required — remove it if your app only installed it for the UIKit. Then complete the Babel and [save-to-gallery permission](#required-permissions) steps below.
    </Info>

    ### Configure Babel

    Register both Babel plugins in your app's `babel.config.js`, or the build fails. `react-native-worklets/plugin` must be the **last** entry in `plugins`:
    ...
    <Warning>
      `metro-react-native-babel-preset` is obsolete. If your app still uses it, replace it with `module:@react-native/babel-preset`.
    </Warning>

    <Tip>
      To customize the UIKit source, see the React Native tab under [GitHub Fork Installation](#github-fork-installation).
    </Tip>
    ...
    **iOS Configuration (ios/Podfile):**
    ...
    ```ruby
    platform :ios, '15.1'  # Minimum iOS 15.1
    ...
    **iOS Deployment Target:**
    ...
    - In Xcode, set **Minimum Deployments** to at least **iOS 15.1**
    ...
    **iOS Permissions (ios/YourApp/Info.plist):**
    ...
    <string>App needs access to save photos and videos to your gallery</string>
    ```
    ...
    **Android Permissions (android/app/src/main/AndroidManifest.xml):**
    ...
    <Warning>
      **Required from v4.3.0.** Chat lets users save photos and videos to the device gallery.
      - **iOS:** without `NSPhotoLibraryAddUsageDescription`, iOS terminates the app the first time a user saves media.
      - **Android:** Android 9 (API 28) and below need `WRITE_EXTERNAL_STORAGE` to save to the gallery. Without it, saving always fails with a permission error. Keep `android:maxSdkVersion="28"`: Android 10 and above don't need the permission.
    </Warning>
    ...
    **Simple Integration:**
    ...
    `AmityUiKitSocial` mounts the full social experience. To mount chat instead, render `AmityUiKitChat` — see [Add Chat](#add-chat) below.
    ...
    From v4.3.0, the same package includes Chat UIKit. Render `AmityUiKitChat` inside `AmityUiKitProvider` to mount the chat experience, starting at the chat list. It needs no extra package and uses the same provider props as `AmityUiKitSocial`.
    ...
    ### Render a Single Chat Page

    From v4.3.1, every chat page is also exported on its own, under the same names as the Web UIKit. Render a page inside `AmityPageRenderer` when you want to place it yourself, for example to open a conversation straight from a push notification. `AmityUiKitChat` remains the simplest way to show the whole chat experience.
    ...
    export default function ConversationScreen({ channelId }) {
      return (
        <AmityUiKitProvider
          apiKey="YOUR_API_KEY"
          apiRegion="us"
    ...
    Each page that takes props exports its type as `<PageName>Props`, for example `AmityChatPageProps`. From the rendered page, users can still move on to the other chat pages, such as a group's settings or its member list.

    <Note>
      The page you render is the first screen in its stack, so its back button has nothing to go back to. Close the page from your app's own navigation instead. Social screens rendered in `AmityPageRenderer` behave the same way.
    </Note>
...
<Warning>
  **Note**: The Flutter UIKit is only available through GitHub forking, not
  package installation. React Native is available both ways: install from npm
  under [Package Installation](#package-installation), or fork it here to
...
<Tabs>
  ...
  <Tab title="React Native">
    Fork the React Native UIKit when you need to change the UIKit itself, such as its components, screens, or navigation flows. If you only need the published UIKit, use the React Native tab under [Package Installation](#package-installation) instead.
    ...
    Build a package from your fork and install it into your app in place of the npm package:

    ```bash
    cd Amity-Social-UIKit-React-Native-CLI-OpenSource
    yarn install
    npm pack        # builds the UIKit and produces amityco-react-native-social-uikit-x.x.x.tgz

    # from your app
    yarn add ../path/to/amityco-react-native-social-uikit-x.x.x.tgz
    ```

    Imports stay the same (`@amityco/react-native-social-uikit`). Rebuild and reinstall the package each time you change the UIKit source.

    ### Complete the App Setup

    A local build needs the same app setup as the npm package. Follow these steps in the React Native tab under [Package Installation](#package-installation):

    - Install the peer dependencies (skip `yarn add @amityco/react-native-social-uikit`)
    - Configure Babel
    - Platform configuration
    - Required permissions
...
<CardGroup cols={3}>
  <Card
    ...
    icon="key"
    href="/uikit/getting-started/authentication"
  >
  ...
  <Card
    ...
    icon="puzzle-piece"
    href="/uikit/getting-started/first-steps"
  >

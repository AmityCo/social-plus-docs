...
The Clips tab is not available on React Native.
...
When a visitor attempts a write action (react, comment, reply, follow), UIKit shows a sign-in prompt by default. Override `handleVisitorUserAction` in `AmityGlobalBehavior` to navigate to your own sign-in screen instead. On iOS the equivalent is named `handleGuestUserAction`.
...
When a visitor exceeds their **daily usage limit** (its size is an ops-level setting), the backend returns error `400323`. UIKit automatically subscribes to the SDK's `visitorUsageLimitReached` event and shows a full-screen `VisitorUsageLimitPage` that blocks further navigation until the user signs in.
...
<CardGroup cols={2}>
  <Card title="Default Page" icon="ban">
    ...
    - **React Native**: Navigator replaced at root, hardware back press swallowed
  </Card>
</CardGroup>
...
| React Native | 4.1.0\+ | `AmityGlobalBehaviour` overrides available |
    ...
    <AmityUiKitProvider
      behaviour={{
        AmityGlobalBehaviour: {
          // Replace the full-page error with your own UI
          handleVisitorUsageLimitReached: () => {
            navigation.navigate('VisitorLimit');
          },

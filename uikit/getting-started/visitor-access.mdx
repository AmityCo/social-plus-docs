...
When a visitor exceeds their **daily read quota** (its size is a network-level setting), the backend returns error `400323`. UIKit automatically subscribes to the SDK's `visitorUsageLimitReached` event and shows a full-screen `VisitorUsageLimitPage` that blocks further navigation until the user signs in.
...
| React Native | 4.1.0\+ | `VisitorUsageLimit` page exported from the public API |

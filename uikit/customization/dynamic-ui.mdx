...
<Note>
<strong>Flutter:</strong> to switch the theme (light / dark / system) from your own code at runtime — e.g. an in-app appearance toggle — use <a href="/uikit/customization/set-preferred-theme">Set Preferred Theme</a>.
</Note>
...
<Tabs>
  <Tab title="Sync Configuration">
    ...
    <Note>
    **React Native**: The React Native UIKit doesn't support Dynamic UI sync — it has no `syncNetworkConfig`. Theme it with the local config file passed to `AmityUiKitProvider` through `configs`.
    </Note>
  </Tab>
...
<CardGroup cols={3}>
  ...
  <Card title="Theming Basics" href="/uikit/customization/component-styling" icon="book">Layering model & tokens</Card>

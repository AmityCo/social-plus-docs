...
<Tabs>
  ...
  <Tab title="React Native">
    <Info>
      Requires `@amityco/react-native-social-uikit` **v4.3.1** or later. Only Chat UIKit strings go through this API; Social UIKit screens don't. In v4.3.0 the package has no localization API — strings can only be changed by [forking the UIKit](/uikit/getting-started/installation#github-fork-installation).
    </Info>
    ...
    <Steps>
      ...
      <Step title="Register via localeMap">
        Pass the bundle via `localeMap` on `AmityUiKitProvider` so the UIKit picks it from the device language:

        ```tsx
        import { AmityUiKitProvider, defaultLocaleMap } from '@amityco/react-native-social-uikit';
        import jaLocale from './locales/ja.json';
        ...
        The UIKit checks the device language — exact match first (e.g. `"ja"`), then language-prefix match (e.g. `"ja-JP"` → `"ja"`). Falls back to English if no match is found. Spread `defaultLocaleMap` to also include the built-in Thai bundle; leave it out to offer only your own languages.
      </Step>
      <Step title="Verify coverage">
    ...
    <Note>
      **English by default:** without a `localeMap`, the UIKit doesn't detect the device language and renders in English for every user. The source includes a Thai bundle for chat strings. To use it, pass `localeMap: defaultLocaleMap` (Thai for devices set to Thai) or `localeBundle: thLocaleBundle` (Thai for everyone).
    </Note>

    ### Other Scenarios

    <AccordionGroup>
      <Accordion title="Use one language for every user">
        Set `localeBundle` to skip device-language detection and use the same bundle for everyone:

        ```tsx
        import { AmityUiKitProvider, thLocaleBundle } from '@amityco/react-native-social-uikit';
      ...
      <Accordion title="Switch locale at runtime">
        `useLocale` controls the active locale. `useString` consumes strings and automatically re-renders all consumers when the locale changes. Both must be used inside `AmityUiKitProvider`.

        ```tsx
        import { Button, Text } from 'react-native';
        import { useLocale, useString } from '@amityco/react-native-social-uikit';
        import jaLocale from './locales/ja.json';
        ...
        function ArchiveLabel() {
          const label = useString('amity_chat_archive');

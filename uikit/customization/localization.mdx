...
| Platform | Localization mechanism | Covered below |
| --- | --- | --- |
| iOS | `AmityStringProvider` namespaces (`social`, `chat`, `common`) | ✅ |
| Android | String resource files | ✅ |
| Web (React) | Locale bundle passed to the provider | ✅ |
| React Native | `localization={{ localeBundle }}` on `AmityUIKitProvider` | See note below |
| Flutter | Standard Flutter localization (generated `AppLocalizations` / `context.l10n`) | See note below |

<Note>
  **React Native** localizes through the provider rather than a string-provider API: pass `localization={{ localeBundle }}` to `AmityUIKitProvider`. Locales resolve by exact match first, then by language prefix — so a `ja-JP` device falls back to a `ja` bundle. Missing keys fall back to English, the same as the platforms below.

  **Flutter** does not expose a UIKit string-provider API. It uses standard Flutter localization — the generated `AppLocalizations` accessed through `context.l10n` — so you localize it the way you localize the rest of your Flutter app.
</Note>

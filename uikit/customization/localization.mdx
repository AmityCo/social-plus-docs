...
<Tabs>
  <Tab title="iOS (Swift)">
    ...
    | Method | Behavior |
    | --- | --- |
    | `setLocale(_:bundle:)` | Set the active language by providing a full translation bundle (replace semantics). Activates immediately. |
    ...
    | `setOverrides(_:)` | Override individual strings on top of any active locale — useful for customizing specific labels without changing the whole language. Multiple calls merge. |
  ...
  <Tab title="Android (Kotlin)">
    ...
    | Method | Behavior |
    | --- | --- |
    | `setLocale(id, bundle)` | Set the active language by providing a full translation bundle (replace semantics). Activates immediately. |
    | `clearLocale()` | Deactivate the current locale. Keys fall through to library defaults. |
    | `setOverrides(map)` | Override individual strings on top of any active locale — useful for customizing specific labels without changing the whole language. |
    | `clearOverrides()` | Remove all programmatic overrides. |

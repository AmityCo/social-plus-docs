...
<Tabs>
  ...
  <Tab title="React Native">
    <Warning>
      **No public localization API yet.** In v4.3.0, `@amityco/react-native-social-uikit` doesn't export localization hooks or accept localization props on `AmityUiKitProvider`, and it doesn't detect the device language. Chat UI renders in English unless you build the UIKit from source.
    </Warning>

    Chat UIKit strings (v4.3.0+) use the same flat key format as Web (for example, `amity_chat_archive`). The source includes two bundles:

    | Bundle | Path in the repository |
    | --- | --- |
    | English (default) | `src/core/localization/defaults/en.json` |
    | Thai | `src/core/localization/defaults/th.json` |

    Social UIKit screens don't use these bundles.

    ### Use Thai or Another Language

    <Steps>
      <Step title="Build the UIKit from source">
        Fork the [React Native UIKit repository](https://github.com/AmityCo/Amity-Social-UIKit-React-Native-CLI-OpenSource) and install your build into the app — see [Building from source](/uikit/getting-started/installation).
      </Step>
      <Step title="Set the initial bundle">
        In `src/core/providers/AmityUIKitProvider.tsx`, pass a bundle to `LocaleProvider` through `initialLocaleBundle`. For your own language, copy `en.json`, translate the values, and import that file instead.
        ...
        Keys missing from the bundle fall back to English.
      </Step>
    </Steps>
  </Tab>
</Tabs>

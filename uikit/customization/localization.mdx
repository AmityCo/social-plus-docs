  ...
  <Tab title="React Native">
    ...
    <Steps>
      ...
      <Step title="Register via localeMap">
        ...
        ```tsx
        import { AmityUiKitProvider } from '@amityco/react-native-social-uikit';
        import jaLocale from './locales/ja.json';
        ...
        The UIKit checks the device language — exact match first (e.g. `"ja"`), then language-prefix match (e.g. `"ja-JP"` → `"ja"`). Falls back to English if no match is found.
      </Step>
      <Step title="Verify coverage">
    ...
    <Note>
      **English by default:** English is the only built-in language. Without a `localeMap`, the UIKit doesn't detect the device language and renders in English for every user.
    </Note>
    ...
    <AccordionGroup>
      <Accordion title="Use one language for every user">
        ...
        <AmityUiKitProvider localization={{ localeBundle: jaLocale }}>
          <App />
        </AmityUiKitProvider>
        ```
      </Accordion>

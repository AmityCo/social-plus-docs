  ...
  <Tab title="React Native">
    ...
    <Steps>
      ...
      <Step title="Register via localeMap">
        ...
        The UIKit checks the device language — exact match first (e.g. `"ja"`), then language-prefix match (e.g. `"ja-JP"` → `"ja"`). Falls back to English if no match is found. A `localeMap` you pass replaces the built-in one, so spread `defaultLocaleMap` to keep the built-in Thai bundle.
      </Step>
      <Step title="Verify coverage">
    ...
    <Note>
      **Built-in Thai:** when you don't pass `localization`, the UIKit uses its built-in locale map, which includes Thai. Users whose device language is Thai see chat in Thai automatically. To always use English, pass `localization={{ localeMap: {} }}`.
    </Note>

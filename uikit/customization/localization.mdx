...
<Tabs>
  ...
  <Tab title="React Native">
    <Warning>
      **Requires a fork.** The npm package `@amityco/react-native-social-uikit` doesn't expose a localization API yet: `AmityUiKitProvider` takes no localization props, and the package exports no localization hooks. To change UIKit strings, [fork the UIKit](/uikit/getting-started/installation#github-fork-installation) and make the changes below in your fork. Only Chat UIKit strings (v4.3.0+) go through this localization layer. Social UIKit screens don't.
    </Warning>
    ...
    <Steps>
      <Step title="Create a translation file">
        Create a flat JSON file in your fork with the keys you want to translate, for example `src/core/localization/defaults/ja.json`. You only need to include keys you're translating — missing keys automatically fall back to English.

        ```json
        {
          "amity_chat_archive": "アーカイブ",
          "amity_chat_unarchive": "アーカイブ解除",
          "amity_chat_action_block_user": "ユーザーをブロック"
        }
        ```
      </Step>
      <Step title="Register the locale bundle">
        In `src/core/providers/AmityUIKitProvider.tsx`, pass the bundle to `LocaleProvider` through `initialLocaleBundle`:

        ```tsx
        import jaLocale from '../localization/defaults/ja.json';

        <LocaleProvider initialLocaleBundle={jaLocale}>
          {children}
          {/* ... */}
        </LocaleProvider>
        ```

        The React Native UIKit doesn't detect the device language, so it uses the bundle you set here for every user. To follow the device language, choose the bundle in your fork before rendering `LocaleProvider`.
      </Step>
      <Step title="Verify coverage">
        Compare your translation file against [`en.json`](https://github.com/AmityCo/Amity-Social-UIKit-React-Native-CLI-OpenSource/blob/develop/src/core/localization/defaults/en.json) to identify missing keys. Any untranslated key silently falls back to English — no errors are thrown.
    ...
    <AccordionGroup>
      <Accordion title="Use the built-in Thai bundle">
        The UIKit source includes a Thai bundle for chat strings at `src/core/localization/defaults/th.json`. Register it the same way:

        ```tsx
        import thLocaleBundle from '../localization/defaults/th';

        <LocaleProvider initialLocaleBundle={thLocaleBundle}>
          {children}
          {/* ... */}
        </LocaleProvider>
        ```
      ...
      <Accordion title="Override specific strings">
        Use `initialOverrides` on `LocaleProvider` to replace individual keys without providing a full locale bundle. Overrides take precedence over the active locale bundle.

        ```tsx
        <LocaleProvider
          initialOverrides={{
            amity_chat_archive: 'Hide chat',
            amity_chat_block_confirm_label: 'Block now',
          }}
        >
          {children}
          {/* ... */}
      ...
      <Accordion title="Switch locale at runtime">
        `useLocale` controls the active locale, and every string in the chat UIKit re-renders when it changes. It isn't exported from the package root, so export it from `src/index.tsx` in your fork first:

        ```tsx
        // src/index.tsx in your fork
        export { useLocale } from './core/localization';
        ```

        Then use it anywhere inside `AmityUiKitProvider`:

        ```tsx
        import { Button } from 'react-native';
        import { useLocale } from '@amityco/react-native-social-uikit';
        import jaLocale from './locales/ja.json';
        ...
        function LanguageSwitcher() {
          ...
          return (
            <>
              <Button title="日本語" onPress={() => setLocaleBundle(jaLocale)} />
              <Button title="English" onPress={() => clearLocaleBundle()} />
            </>
          );
        }
        ```

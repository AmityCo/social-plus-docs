...
<Checklist>
  <Check>All hex colors valid (#RRGGBB / #RRGGBBAA)</Check>
  <Check>No trailing commas in JSON objects</Check>
  <Check>Wildcard positions correct (`page/*/*`, `*/component/*`, `*/*/element`)</Check>
  <Check>Excluded IDs exist before removing</Check>
  <Check>Dark theme provides sufficient contrast</Check>
</Checklist>

## Troubleshooting

<AccordionGroup>
  <Accordion title="Override Not Applying">
    Confirm path accuracy; test with a more generic wildcard to isolate which segment mismatches.
  </Accordion>
  <Accordion title="Unexpected Color in Dark Mode">
    Missing dark fallback—ensure both themes defined or rely on global token.
  </Accordion>
  <Accordion title="Icon Not Showing">
    Verify asset filename, case sensitivity, and bundling path.
  </Accordion>
  <Accordion title="Broken Layout After Exclusion">
    Re-add element; some components expect internal spacing anchors.
  </Accordion>
</AccordionGroup>

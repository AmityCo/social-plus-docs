...
<Update label="4.3.1  (25/09/2026)" tags={["New Releases","Improvements"]}>
  ## 🚀 New Features

  #### Standalone Chat Pages

  All 16 chat pages are now exported from the package root under the same names as the Web UIKit, so you can render any one of them inside `AmityPageRenderer`. For example, render `<AmityChatPage channelId="..." />` to open a conversation straight from a push notification. Each page that takes input exports a `<PageName>Props` type, such as `AmityChatPageProps`. From a rendered page, users can still move on to the other chat pages. See [Chat Components](/uikit/components/chat/overview#quick-start-example).

  #### Localization API

  `AmityUiKitProvider` takes a `localization` prop (`localeBundle`, `overrides`, `localeMap`), the same shape as the Web UIKit. `useLocale` and `useString` are exported from the package root. When you pass a `localeMap`, the UIKit picks a bundle from the device language. Without one, chat stays in English as before. See [Localization](/uikit/customization/localization).

  ## ✨ Improvements

  - Removed the built-in Thai string bundle. English is the only built-in language, as on the other platforms. Add Thai or any other language with your own bundle through `localization`.
...
<Update label="4.3.0  (25/09/2026)" tags={["New Releases","Improvements"]}>
  ...
  - Chat strings ship in English. The package has no API to change them until 4.3.1 — see [Localization](/uikit/customization/localization).

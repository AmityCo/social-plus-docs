...
<Update label="4.3.1  (25/09/2026)" tags={["New Releases"]}>
  ...
  `AmityUiKitProvider` takes a `localization` prop (`localeBundle`, `overrides`, `localeMap`), the same shape as the Web UIKit. `useLocale`, `useString`, `defaultLocaleMap`, and `thLocaleBundle` are exported from the package root. Device-language detection now works on React Native. See [Localization](/uikit/customization/localization).

  ## 🚨 Compatibility Changes

  - Chat now follows the device language by default. Users whose device language is Thai see chat in Thai after upgrading. To keep English for everyone, pass `localization={{ localeMap: {} }}`.
...
<Update label="4.3.0  (25/09/2026)" tags={["New Releases","Improvements"]}>
  ...
  - Chat ships with English and Thai string bundles. The package has no API to switch language until 4.3.1 — see [Localization](/uikit/customization/localization).

...
<Update label="4.26.0  (11/09/2026)" tags={["Improvements"]}>
  ## Breaking Behaviour

  - User search results now contain display-name matches only. Users whose `userId` matches the keyword but whose display name does not are no longer returned.

  This applies to every UIKit surface backed by global user search: user search, add member to community, invite member to community, add member to a group chat, select members for a new group chat, and create conversation. UIKit sets `searchBy` to display name internally; it is not configurable from UIKit.

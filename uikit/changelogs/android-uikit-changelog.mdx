...
<Update label="4.18.0  (05/06/2026)" tags={["New Releases", "Improvements"]}>
  ## 🚀 New Features

  #### Event Info on Livestream Ring
  The global live ring now displays event metadata for event-associated livestreams, and chat access is gated to the event community.

  #### Visitor Usage Limit Page
  Added `AmityVisitorUsageLimitPage` — a full-screen error shown when a visitor exceeds their daily read quota (error `400323`). The page shows a "Sign in" CTA and blocks back navigation so visitors cannot dismiss it without signing in. Two new `AmityGlobalBehavior` overrides let you customise the experience:
  ...
  - Fixed home tab incorrectly shown for visitor users.

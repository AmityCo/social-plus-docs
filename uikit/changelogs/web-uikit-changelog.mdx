...
<Update label="4.24.0  (25/09/2026)" tags={["New Releases","Improvements"]}>
  ...
  A new embeddable widget, exported as `AmityDiscoveryWidgetComponent`, surfaces a topic's posts as a card carousel. Configure it with `topicId`, `showHeader`, `minVisibilityThreshold`, and `pageId`, and handle a card tap with either the `onCardClick` prop or the `AmityDiscoveryWidgetComponentBehavior.goToDestination` page behavior.

  #### Pinned Messages in Livestream Chat

  Hosts, co-hosts, moderators, and anyone holding the `PIN_MESSAGE` permission can now pin and unpin a message from the livestream chat message menu. The pinned message stays visible in a banner above the chat feed until it is unpinned or replaced.
  ...
  - Fixed chat avatar and mention taps not opening the tapped user's profile, including from the channel header.
  ...
  - Fixed the community profile menu to check the `EDIT_COMMUNITY` permission instead of moderator membership, so a custom role with that permission gets the same options as a built-in moderator.
  - Fixed the public-community icon rendering filled instead of outlined.
  - Fixed the pin option still showing in livestream chat after a user's pin permission was revoked at the network level.

...
<Update label="4.17.0  (05/06/2026)" tags={["New Releases", "Improvements"]}>
  ...
  Added `VisitorUsageLimitPage` — a full-screen error shown when a visitor exceeds their daily read quota (error `400323`). The page shows a "Sign in" CTA and blocks back navigation so visitors cannot dismiss it without signing in. Two new `AmityGlobalBehavior` overrides let you customise the experience:
  ...
  - Fixed incorrect moderator check causing unintended post action restrictions.
  - Fixed visitor users' header not being hidden as expected.
  - Fixed back navigation behavior.
  - Fixed cohost invite handling when the referenced post is hidden.
  - Fixed type popup display.

  ## ✨ Improvements

  - Upgraded `@amityco/ts-sdk` to `7.22.0`.

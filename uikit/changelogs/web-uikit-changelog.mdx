...
<Update label="4.24.0  (25/09/2026)" tags={["New Releases","Improvements"]}>
  ## 🚀 New Features
  ...
  - The full-screen video and clip viewer now streams through the same adaptive HLS player used elsewhere in the UIKit, replacing its separate custom video implementation.
  - Upgraded `@amityco/ts-sdk` to 7.28.0.

  ## 🐞 Bug Fixes

  - Fixed a pending comment in a community that requires comment approval appearing as published before it was approved; the composer now shows an "awaiting review" notice instead.
  - Fixed tapping an @mention or an avatar in chat not opening the user's profile, including from the channel header.
  - Fixed the reply band not being clickable, the reply-quote width and header gap, the new-message pill, the reaction-list skeleton rows, the failed-message sheet icons, and long reaction-list names not truncating.
  - Fixed the chat group name wrapping instead of truncating with an ellipsis.
  - Fixed the community profile menu checking moderator membership instead of the `EDIT_COMMUNITY` permission. A custom role with that permission now gets the same options as a built-in moderator.
  - Fixed the public-community icon rendering filled instead of outlined.
  - Fixed a globally banned user not being removed from an active livestream session immediately.
  - Fixed missing customization exclude keys for the livestream/create-post button, the media feed tabs, and the user-profile create button.
  - Fixed chat media downloads saving the video file with an incorrect URL and extension.
  ...
  <AccordionGroup>
    ...
    <Accordion title="Dependencies" icon="book">
      | Library | Version |
      | :-- | :-- |
      | @amityco/ts-sdk | 7.28.0 |
    </Accordion>
  </AccordionGroup>
</Update>

<Update label="4.23.0  (11/09/2026)" tags={["New Releases","Improvements"]}>
  ## 🚀 New Features
  ...
  - Adopted the SDK's new `searchBy` capability for more accurate user search — results now match display names, so user-ID-only matches no longer appear.

  ## 🐞 Bug Fixes

  - Gate moderation actions by permission.
  - Auto-retry a failed media upload.
  - Show the terminated and ended-livestream screens instead of a broken player.
  - Invite member: hide the default user list, and add a no-users-available state and a search placeholder.
  - Fixed the blocked-user header padding and size, the single-line group name, the comment placeholder, and the camera video-capture option.
  - Hide the product tag when product tags are disabled.
  ...
  <AccordionGroup>
    ...
    <Accordion title="Dependencies" icon="book">
      | Library | Version |
      | :-- | :-- |
      | @amityco/ts-sdk | 7.27.0 |
    </Accordion>
  </AccordionGroup>
</Update>

<Update label="4.22.0  (28/08/2026)" tags={["Improvements"]}>
  ## 🐞 Bug Fixes

  - Fixed the post title not rendering on the post-review page.
  - Restored the invite-co-host button style, and fixed the livestream moderator badge updating on role change.
  - Fixed the brand badge on pending posts and the invitation banner.
  - Show the review modal when editing an event post in an admin-review community; hide "Post event to feed" for non-community-members.
  - Fixed the host being unable to see their own event on the community event tab.
  - Fixed page headers and empty states for blocked users, pending invitations, and follow requests.
  - Fixed clip composer/see-more styling and the deleted-clip view on reaction.
  - Fixed the notification highlight color and reply-comment profile navigation.
  - Fixed post/poll placeholder copy and the story "shared" toast.
  ...
  <AccordionGroup>
    ...
    <Accordion title="Dependencies" icon="book">
      | Library | Version |
      | :-- | :-- |
      | @amityco/ts-sdk | 7.26.0 |
    </Accordion>
  </AccordionGroup>
</Update>

<Update label="4.21.0  (14/08/2026)" tags={["Improvements"]}>
  ## 🐞 Bug Fixes

  - Fixed member-management permissions.
  - Fixed a failed image poll.
  - Fixed the custom-role promote/demote option.
  ...
  <AccordionGroup>
    ...
    <Accordion title="Dependencies" icon="book">
      | Library | Version |
      | :-- | :-- |
      | @amityco/ts-sdk | 7.25.0 |
    </Accordion>
  </AccordionGroup>
</Update>

<Update label="4.20.0  (31/07/2026)" tags={["New Releases","Improvements"]}>
  ## 🚀 New Features
  ...
  The Explore and "Happening Now" event surfaces now exclude the current user's own events.
  ...
  - Fixed the "see more" page title, toast copy, and RSVP reset.
...
<Update label="4.19.1  (17/07/2026)" tags={["New Releases","Improvements"]}>
  ## 🚀 New Features

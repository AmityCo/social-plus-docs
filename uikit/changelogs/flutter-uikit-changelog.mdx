...
<Update label="4.22.0  (25/09/2026)" tags={["New Releases", "Improvements"]}>

  ## 🚀 New Features

  #### Semantic Color Tokens and Dark Theme for Chat

  The Flutter UIKit now resolves Chat colors through the same semantic color tokens as the Android UIKit. Set them per mode under `theme.light` and `theme.dark` in `config.json`, and scope them with the `page_id/component_id/element_id` cascade. See [Component Styling](/uikit/customization/component-styling#supported-theme-tokens).

  - Chat list, chat room, and chat search follow the token set in both light and dark themes.
  - Social features are unchanged and keep resolving through the existing core colors.

  ## 🐞 Bug Fixes

  - Chat list: colors, the create-chat menu, the new group page, member permissions, profile pictures, the no-results icon, the group profile, the deleted-user row, and toasts now match the design in both themes.
  - Chat room: reaction colors now mark your own reaction; the iOS keyboard, action sheet, and error alert follow the UIKit theme; the mention list and `@All` row, failed-message badge, compose box, and report sheets match the design; the Report → Others sheet no longer shows an error screen.
  - Chat room loading: the reaction sheet no longer loads forever, and the link preview and message skeletons match the design.
  - Chat search: only the matched text is highlighted, and the search bar opens a dark keyboard in dark mode.
  - The media viewer close button and the unreport icon render without extra backgrounds.
...
<Update label="4.21.0  (28/08/2026)" tags={["Improvements"]}>
...
<Update label="4.20.20  (24/08/2026)" tags={["New Releases"]}>

  ## 🚀 New Features

  #### Runtime Theme Switching

  Added `setPreferredTheme`, a public API to switch the UIKit theme (light / dark / system default) at runtime. See [Set Preferred Theme](/uikit/customization/set-preferred-theme).
...
<Update label="4.20.19  (03/07/2026)" tags={["Improvements"]}>

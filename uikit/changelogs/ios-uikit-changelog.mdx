...
<Update label="4.29.0  (25/09/2026)" tags={["New Releases","Improvements"]}>
  ## 🚀 New Features
  ...
  - **Host container:** give the widget wrap-content or a minimum height, and do not place it inside a horizontally scrolling parent.
  ...
  - **Customization:** `*/livestream_chat_feed/pinned_message_banner`. The pin and unpin menu items are not customizable on iOS.

  ## ✨ Improvements

  - Moderation controls are now gated on the permission each action requires instead of on the user's role name, so a custom role with the right permission gets the same controls as a built-in moderator. This covers message delete, mute, group settings, member actions, banned members, mention-all, community settings and profile actions, pending-post review, post-menu actions, story management, and event edit, and the gates follow permission changes in real time.

  ## 🐞 Bug Fixes

  - Fixed video download and playback in chat using swapped URLs — downloading a video previously saved the streamed playback asset, and playback used the download asset.
  - Fixed the post, poll, and livestream options in an event's create-post menu ignoring the `excludes` configuration.
  - Fixed toast messages appearing in the wrong position on livestream, post detail, and story surfaces; toasts now sit above the bottom bar.
  - Fixed Picture-in-Picture starting automatically while a livestream chat sheet (message actions, moderation, share, or menu) was open.
  - Fixed the post-creation button state after removing a failed media attachment.
  - Fixed the media carousel not remembering its current index after a manual swipe.
  - Fixed the product tag icon missing in the post composer's media viewer.
  - Fixed empty-state rendering in the community add-member and invite-member pages.
  - Fixed the group name field not wrapping to a new line, and its placeholder rendering in the value text color instead of the placeholder color.
...
<Update label="4.28.0  (11/09/2026)" tags={["New Releases","Improvements"]}>
  ## 🚀 New Features
  ...
  - Adopted the SDK's new `searchBy` capability for more accurate user search — results now match display names, so user-ID-only matches no longer appear. The same applies to @mention search in posts and comments.

  ## 🐞 Bug Fixes

  - Fixed the media carousel index not resetting after a feed refresh.
  - Fixed channel avatar parsing on chat pages.
  - Prevented Picture-in-Picture from activating on the report-reason screen and while a bottom sheet is open on a livestream.
  - Chat links now follow the existing link-opening behavior.
...
<Update label="4.27.0  (28/08/2026)" tags={["New Releases","Improvements"]}>
  ## 🚀 New Features
  ...
  - Fixed the notification unread red-dot issue.
  - Fixed the Explore Community button.
  - Excluded the product tag on event posts.
  - Fixed the avatar placeholder color and assorted color issues.
  - Fixed staging regressions.
  ...
  <AccordionGroup>
    ...
    <Accordion title="Dependencies" icon="book">
      | Library | Version |
      | :-- | :-- |
      | Amity-Social-Cloud-SDK | 8.6.0 |
    </Accordion>
  </AccordionGroup>
</Update>

<Update label="4.26.4  (26/08/2026)" tags={["Improvements"]}>
  ## 🐞 Bug Fixes

  - Fixed the Explore Community button to open the Communities / Explore tab.
  ...
  <AccordionGroup>
    ...
    <Accordion title="Dependencies" icon="book">
      | Library | Version |
      | :-- | :-- |
      | Amity-Social-Cloud-SDK | 8.5.4 |
    </Accordion>
  </AccordionGroup>
</Update>

<Update label="4.26.3  (25/08/2026)" tags={["Improvements"]}>
  ## 🐞 Bug Fixes

  - Fixed news-feed pull-to-refresh: no more empty-page/skeleton flash, the featured post no longer disappears mid-refresh, and live updates resume after a refresh error.
  - Fixed tapping a user avatar opening a blank screen.
  - Sorted the community list by display name.
  - Fixed permissions on the About tab.
...
<Update label="4.26.2  (18/08/2026)" tags={["Improvements"]}>
  ## 🐞 Bug Fixes

  - Fixed avatars and images hosted on external (non-Amity) CDNs failing to load — the image loader now retries once without the `Authorization` header when the host rejects the bearer token.
  - Fixed the selected tab color ignoring your configured primary color; tab bars now use the theme's primary color instead of the highlight color.
  - Fixed the notification tray item color and the disabled-button color in the post and comment composers.
  - Aligned assorted surfaces with the web UIKit for visual consistency.
  ...
  <AccordionGroup>
    ...
    <Accordion title="Dependencies" icon="book">
      | Library | Version |
      | :-- | :-- |
      | Amity-Social-Cloud-SDK | 8.5.3 |
    </Accordion>
  </AccordionGroup>
</Update>

<Update label="4.26.0  (14/08/2026)" tags={["Improvements"]}>
  ## 🐞 Bug Fixes

  - Fixed the notification tray showing the wrong user's avatar.
  - Resolved custom user avatars (`avatarCustomUrl`) on the remaining chat pages.
  - Removed the redundant overflow menu button on the group chat page.
  ...
  <AccordionGroup>
    ...
    <Accordion title="Dependencies" icon="book">
      | Library | Version |
      | :-- | :-- |
      | Amity-Social-Cloud-SDK | 8.5.2 |
    </Accordion>
  </AccordionGroup>
</Update>

<Update label="4.25.1  (07/08/2026)" tags={["Improvements"]}>
  ## ✨ Improvements
  ...
  <AccordionGroup>
    ...
    <Accordion title="Dependencies" icon="book">
      | Library | Version |
      | :-- | :-- |
      | Amity-Social-Cloud-SDK | 8.5.1 |
    </Accordion>
  </AccordionGroup>
</Update>

<Update label="4.25.0  (31/07/2026)" tags={["New Releases","Improvements"]}>
  ## 🚀 New Features

  #### Chat Design Refresh

  Migrated the Chat UIKit to the new design system.

  #### Event Created Notification

  Added a "created event" item to the notification tray.

  ## 🐞 Bug Fixes

  - Fixed large image preview decoding.
  - Fixed avatar rendering on profiles.
  - Fixed tapping a comment on a clip not navigating to the post detail.
  - Fixed several chat regressions.
  - Increased the touch area of the chat back button and added a navigation handler.
  ...
  <AccordionGroup>
    ...
    <Accordion title="Dependencies" icon="book">
      | Library | Version |
      | :-- | :-- |
      | Amity-Social-Cloud-SDK | 8.5.0 |
    </Accordion>
  </AccordionGroup>
</Update>

<Update label="4.24.0  (16/07/2026)" tags={["New Releases","Improvements"]}>
  ## 🚀 New Features
  ...
  - Fixed the chat back button not working when there was no navigation stack, and added an optional back button to the chat list.
  - Fixed a discussion livestream being published as an event's recorded stream.
  - Fixed the livestream not keeping its current state when switching between background and foreground.

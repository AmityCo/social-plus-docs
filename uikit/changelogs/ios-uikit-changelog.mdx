...
<Update label="4.29.0  (25/09/2026)" tags={["New Releases","Improvements"]}>
  ...
  Added the Discovery Widget — a read-only, topic-scoped carousel of ranked community posts that you embed on a page you own, configured from a console-generated snippet. Set it up with `AmityDiscoveryWidgetComponent`, passing `topicId`, `showHeader`, `minVisibilityThreshold`, and an optional `onCardClick` and `pageId`; each card renders via `AmityDiscoveryWidgetPostCardComponent`. Card taps are unhandled by default — wire `onCardClick` or override `AmityDiscoveryWidgetComponentBehavior` to route them. The widget renders nothing and takes up no layout space when the pool is below threshold or the content call fails.

  #### Pinned Messages in Livestream Chat

  Hosts, co-hosts, and moderators (or holders of the `PIN_MESSAGE` permission) can now pin a message in livestream chat. The pinned message shows as a persistent, expandable banner at the top of the chat for every viewer, and can be pinned or unpinned from the message's ⋮ menu.

  ## ✨ Improvements

  - Migrated chat, community, event, and story moderation controls from role-name checks to backend permission checks, so gates (message delete, mute, group settings, member actions, banned members, mention-all, community settings and profile actions, pending-post review, post-menu actions, story management, event edit) reflect a viewer's actual permissions in real time rather than a role label.
  ...
  - Fixed the Post, Poll, and Live stream options in an event's create-post sheet not responding to configured excludes.

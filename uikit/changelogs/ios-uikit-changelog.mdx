...
<Update label="4.21.0 (15/05/2026)" tags={["New Releases", "Improvements"]}>
  ...
  Add localization support across Social features.
...
<Update label="4.20.0 (08/05/2026)" tags={["New Releases"]}>
  ...
  UIKit now integrates with **AmitySDK v8.1.0**. The Realm and RealmSwift dependency has been removed — `Realm.xcframework` and `RealmSwift.xcframework` are no longer installed through the `SharedFrameworks` Package.swift file.

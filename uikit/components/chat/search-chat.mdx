...
<Info>
**Platform Support**: Chat Search is available on **Flutter** (standalone `AmitySearchChannelPage`), **React Web**, and **React Native** v4.3.0+ (reached from the chat home page, or rendered standalone as `AmitySearchChannelPage` from v4.3.1 — see the notes below). Refer to the platform UIKit for iOS and Android availability.
</Info>
...
<Tabs>
  <Tab title="Chat Search">
    ...
    **React Web**: Search is not a standalone exported page on React Web. It is reached from `AmityChatHomePage` via the header search entry, which opens the search view with the Chat / Message tabs. The Chat tab is backed by the SDK's `ChannelRepository.searchChannels` (member channels of type `conversation` and `community`, sorted by last activity), and archived channels are indicated inline. No extra wiring is required — enable the chat home page and search is available.
    ...
    <Info>
    **React Native** (v4.3.0+): Search opens from the search button on the chat home page, with **Chats** and **Messages** tabs. From v4.3.1 you can also render `AmitySearchChannelPage` on its own inside `AmityPageRenderer`. Results appear once the query is at least 3 characters. Archived channels show an archived badge; swipe a result row to archive or unarchive it. Tapping a result in the **Messages** tab opens the chat scrolled to that message. One-to-one chats with users the viewer has blocked are hidden from both tabs.
    </Info>
  </Tab>

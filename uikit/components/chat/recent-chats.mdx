...
<Tabs>
  <Tab title="Chat Home Interface">
    ...
    **Automatic Management**: The chat home page automatically handles real-time updates, sorting, and network connectivity through the underlying SDK.
    ...
    <Info>
    **React Native** (v4.3.0+): The chat home page is where `AmityUiKitChat` opens. It shows unread and mention badges on each chat and lets users swipe a chat to archive it. When push notifications are disabled for the user's account or for the chat module (for example, by an admin), the list shows a "Push notifications have been disabled by admin" banner.
    </Info>
  </Tab>
  ...
  <Tab title="Archived Chat Management">
    **Organized archive functionality**
    ...
    ```jsx React Native - Archived Chat Page
    // React Native exports the full archived chats page, not the list component.
    <AmityPageRenderer>
  ...
  <Tab title="Conversation Creation">
    **New conversation interface**
    ...
    | Feature                  | Description                                                             |
    | ------------------------ | ----------------------------------------------------------------------- |
    | Search Users             | Allows users to search for other users by display name by typing at least 3 characters |

...
<Tabs>
  <Tab title="Main Chat Interface">
    ...
    **Important Note**: The AmityChatPage automatically handles message loading, real-time updates, link previews, and user interactions.
    ...
    // See "Customize Navigation (React Native)" below.
    <AmityUiKitProvider
      /* ...credentials */
      behaviour={{
        AmityChatPageBehavior: {
          onAvatarTap: ({ userId, avatarUrl }) => {
            // Navigate to your own profile screen
          },
        },
      }}
    >
...
## Customize Navigation (React Native)

By default, tapping an avatar in chat opens the picture full screen, and a resolved @mention is styled but does nothing when tapped. To send users to your own profile screen instead, pass handlers through the `behaviour` prop of `AmityUiKitProvider` (v4.3.0+). These apply to both one-to-one and group chats.

| Behavior | Handler | Fires when | Context |
| --- | --- | --- | --- |
| `AmityChatPageBehavior` | `onAvatarTap` | The user taps the other person's avatar in the one-to-one chat header | `{ userId: string; avatarUrl?: string }` |
| `AmityMessageBubbleBehavior` | `onAvatarTap` | The user taps the sender's avatar beside an incoming message | `{ userId: string; avatarUrl?: string }` |
| `AmityMessageBubbleBehavior` | `onMentionUserTap` | The user taps a resolved @mention in a message | `{ userId: string }` |

When you set a handler, the UIKit calls it instead of running its default action. Handlers you leave unset keep the default.

```jsx React Native
import { AmityUiKitProvider, AmityUiKitChat } from '@amityco/react-native-social-uikit';
...
export default function ChatScreen({ navigation }) {
  ...
  return (
    <AmityUiKitProvider
      ...
      behaviour={{
        AmityChatPageBehavior: { onAvatarTap: openProfile },
        AmityMessageBubbleBehavior: {
          onAvatarTap: openProfile,
          onMentionUserTap: openProfile,
        },
      }}
    >
      <AmityUiKitChat />

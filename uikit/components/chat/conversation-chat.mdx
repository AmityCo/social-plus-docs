...
<Info>
**Platform Support**: Conversation Chat components are available on **iOS, Android, React, React Native, and Flutter**. On **React Native**, these screens ship from v4.3.0 as part of `AmityUiKitChat`, and from v4.3.1 each page can also be rendered on its own inside `AmityPageRenderer` — see [Quick Start](/uikit/components/chat/overview#quick-start-example). Only pages are exported on React Native; the building-block components shown here (list, bubble, composer, action sheets) aren't.
</Info>
...
<Tabs>
  <Tab title="Main Chat Interface">
    **Complete chat conversation interface**
    ...
    <CodeGroup>
    ```swift iOS
    let page = AmityChatPage(channelId: "channelId")
    ...
    ```tsx React
    import { AmityChatPage } from '@amityco/ui-kit';
    ...
    ```jsx React Native
    // channelId is required; userDisplayName and jumpToMessageId are optional.
    <AmityPageRenderer>
      <AmityChatPage channelId="channelId" />
    </AmityPageRenderer>
    ```
    ...
    **Important Note**: The AmityChatPage automatically handles message loading, real-time updates, link previews, and user interactions.
    ...
    <CodeGroup>
    ```swift iOS
    class CustomChatPageBehavior: AmityChatPageBehavior {
        override func onAvatarTap(context: AmityChatPageBehavior.Context) {
            // Route the header-avatar tap to your own screen.
        }
    }

    // Register once, e.g. at app start.
    AmityUIKit4Manager.behaviour.chatPageBehavior = CustomChatPageBehavior()
    ```
    ...
    // Register once, e.g. at app start.
    AmityChatBehaviorHelper.chatPageBehavior = CustomChatPageBehavior()
    ```
    ...
    // Register once, e.g. at app start.
    AmityUIKit4Manager.behavior.chatPageBehavior = CustomChatPageBehavior();
    ```

    ```jsx React Native
    // Pass handlers through the behaviour prop of AmityUiKitProvider.
    <AmityUiKitProvider
      /* ...credentials */
      behaviour={{
        AmityChatPageBehavior: {
          onAvatarTap: ({ userId, avatarUrl }) => {
            // Route the header-avatar tap to your own screen.
          },
        },
      }}
  ...
  <Tab title="Message Components">
    **Individual message rendering and composition**
    ...
    let viewModel = AmityChatRoomViewModel(channelId: "channelId")

    AmityChatMessageListComponent(viewModel: viewModel)
    AmityChatMessageComposeBar(viewModel: viewModel)
    ```
    ...
    fun conversationMessages() {
        ...
        val viewModel = remember { AmityChatPageViewModel(channelId = "channelId") }

        AmityChatMessageList(viewModel = viewModel)
        AmityMessageComposer(viewModel = viewModel)
    }
    ```
    ...
    Override `AmityMessageBubbleBehavior` to route taps on a sender's **avatar** and on a **mentioned user** to your own screens. The mention-user tap override is available on iOS (UIKit 4.28.0), Android (UIKit 4.26.0), and React Native (UIKit 4.3.0).
    ...
    ```jsx React Native
    // Pass handlers through the behaviour prop of AmityUiKitProvider.
    // Handlers receive { userId, avatarUrl? } and { userId } respectively.
    <AmityUiKitProvider
      /* ...credentials */
      behaviour={{
    ...
    <Note>
    **The header avatar is a separate hook.** `AmityMessageBubbleBehavior.onAvatarTap` covers only the sender avatar on a message bubble. The peer avatar in the conversation header is served by `AmityChatPageBehavior.onAvatarTap`, which takes the same arguments and also opens the avatar viewer by default. Override that one to redirect the header tap.
    </Note>
  </Tab>
  ...
  <Tab title="User Actions">
    ...
    **Global Configuration**: These user action configurations are set at the global level in AmityUIKitConfig.json and apply to all conversation chat components automatically.
    ...
    <Warning>
    **React Native**: v4.3.0 doesn't read `conversation_chat_user_actions`. All three actions — notification toggle, report, and block — are always shown.
    </Warning>

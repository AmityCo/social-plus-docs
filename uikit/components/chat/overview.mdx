...
<Info>
**React Native** ships Chat UIKit from v4.3.0 of `@amityco/react-native-social-uikit`, covering the chat list, conversation and group chat, and chat search. Live chat is not included.
</Info>
...
// Requires @amityco/react-native-social-uikit v4.3.0 or later.
function MyChatApp() {
  return (
    <AmityUiKitProvider
      apiKey="API_KEY"
...
<Note>
**React Native**: `AmityUiKitChat` opens at the chat list, and every other chat screen is reached from there. From v4.3.1, every chat page is also exported on its own under the same names as the Web UIKit. To open a specific conversation directly — for example from a push notification — render `<AmityChatPage channelId="..." />` inside `AmityPageRenderer`; each component page shows its React Native example. Pages that take props export a `<PageName>Props` type, such as `AmityChatPageProps`. The page you render is the first screen in its stack, so its back button has nothing to go back to — close it from your app's own navigation. See [Installation](/uikit/getting-started/installation#add-chat) for the peer dependencies and save-to-gallery permissions chat requires.
</Note>

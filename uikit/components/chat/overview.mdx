...
<Note>
**Light & dark themes.** Chat components support both light and dark themes. Set `preferred_theme` to `default` to follow the device's OS setting, or force `light` / `dark`. Provide dark values under `theme.dark` (and per-surface under `customizations`). Dark mode is currently verified for **chat** components; other UIKit features adopt the expanded color set in a later release — see [Component styling](/uikit/customization/component-styling).
</Note>

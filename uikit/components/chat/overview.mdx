...
<Note>
**React Native**: `AmityUiKitChat` opens at the chat list, and every other chat screen is reached from there. To open a specific conversation directly — for example from a push notification — render `<AmityChatPage channelId="..." />` inside `AmityPageRenderer` (v4.3.1+). See [Installation](/uikit/getting-started/installation#add-chat) for the peer dependencies and save-to-gallery permissions chat requires.
</Note>

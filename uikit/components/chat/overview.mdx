...
<Note>
**React Native**: `AmityUiKitChat` opens at the chat list, and every other chat screen is reached from there. The package doesn't export individual chat pages, so you can't open a specific channel directly by `channelId`. See [Installation](/uikit/getting-started/installation) for the peer dependencies and save-to-gallery permissions chat requires.
</Note>

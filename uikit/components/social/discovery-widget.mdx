...
The component is named `AmityContentWidgetComponent` on all three platforms. Pass a topic ID and handle card selection with `onCardClick`.
...
| Platform | Behavior |
| --- | --- |
| Web | `AmityContentWidgetComponentBehavior.goToDestination({ topicId, post })` |
| Android | `AmityContentWidgetBehavior.goToDestination(context, topicId, post)` |
| iOS | `AmityContentWidgetBehavior.goToDestination(context)` |

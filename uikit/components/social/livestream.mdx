...
The UIKit shows the pin controls when the current user is the host or a co-host, or holds `PIN_MESSAGE` in the live chat channel, and the room is live. The server enforces the permission on every request. If a user's role is revoked mid-stream, their next pin or unpin returns `403`, the UIKit refreshes their permissions, and the controls disappear without an error message.

...
<CardGroup cols={2}>
  ...
  <Card title="Stream Viewing" icon="play">
    **Live and recorded stream playback**
    ...
    - Pinned message in live chat (community only)
...
<Tabs>
  <Tab title="Stream Creation">
    ...
    **Optional Thumbnail Upload**: Adding a thumbnail during stream creation is optional. The backend automatically generates a live thumbnail once the stream starts (`liveThumbnailUrl`), and a recorded thumbnail after the stream ends. If the creator does upload a cover image, it will take priority over the auto-generated thumbnails.
    ...
    | Hardware Start Triggers | Start the stream with a paired Bluetooth remote shutter or the device's hardware volume key, the same way those controls fire the shutter in the native camera app (iOS and Android) |
  ...
  <Tab title="Stream Viewing">
    **Live stream viewing and playback interfaces**
    ...
    | Viewer Count | Real-time display of total number of users currently watching the stream, gated by the network-level [viewer count visibility rule](#viewer-count-visibility) |
    | Who's Watching List | View the list of users currently watching the live stream |
    ...
    **Automatic Thumbnail**: The backend automatically generates a live thumbnail for the room session once the stream goes live (available via `liveThumbnailUrl` on the room model). After the stream ends and recording is processed, a recorded thumbnail is also auto-generated. UIKit uses these thumbnails automatically when displaying livestream posts in feeds -- no additional configuration is needed. If the creator uploaded a custom cover image, it takes priority over the auto-generated thumbnails.
    ...
    | `livestream_player_page/*/live_viewer_count_element` | Element | Customize the viewer-count pill |
...
| Mode | Viewer behavior |
| --- | --- |
| **Always show** (default) | The count is visible whenever there is at least one viewer. At zero viewers the pill shows the `LIVE` indicator without a number. Matches the previous behavior for tenants that never configured this setting. |
| **Hide entirely** | Viewers never see the count. The `LIVE` indicator still renders. |
| **Show above minimum** | Viewers see the count only when it reaches a network-configured threshold (positive integer, `1`–`1000`, default `50`). Below the threshold, the `LIVE` indicator still renders without a number. |

### Propagation

UIKit reads the config **once when the viewer joins the stream**. Config changes made by an admin while a stream is live do not push to already-connected viewers — the new rule takes effect the next time each viewer opens the player page. A viewer who leaves and rejoins picks up the latest saved config.

Two things do update in-place within a single mount:

- **Count ticks.** Under _Show above minimum_, the pill appears or disappears as the count naturally crosses the threshold.
- **Role changes.** A viewer promoted to co-host mid-stream immediately sees the actual count — no rejoin needed.

### Fail-Open

If the SDK cannot read the config (network error, malformed payload, threshold out of range), the element treats the effective config as _Always show_ — today's behavior is never silently regressed to hiding the count.
...
<Tip>
  Building a custom viewer surface instead of using UIKit? Read the config yourself via `client.getLiveViewerCountConfig()` — see the [Live Viewer Count Config SDK guide](/social-plus-sdk/video-new/broadcasting/viewer-count-config).
</Tip>
...
To route that tap somewhere else, override `onLivestreamProductTagClick` on `AmityGlobalBehavior`. The handler receives the tapped product on `context.product`.

<CodeGroup>
```swift iOS
class MyGlobalBehavior: AmityGlobalBehavior {
    override func onLivestreamProductTagClick(context: AmityGlobalBehavior.Context) {
        guard let product = context.product else { return }
        MyNavigator.showProduct(id: product.productId)
    }
}

AmityUIKit4Manager.behaviour.globalBehavior = MyGlobalBehavior()
```
...
AmityUIKit4Manager.behavior.globalBehavior = object : AmityGlobalBehavior() {
    override fun onLivestreamProductTagClick(context: AmityGlobalBehavior.Context): Boolean {
        val product = context.product ?: return false
        MyCommerceFlow.openProduct(product.getProductId())

        // Return true to claim the tap, so the built-in product web view is skipped.
        return true
    }
}
```
...
<AmityUIKitProvider
  pageBehavior={{
    AmityGlobalBehavior: {
      onLivestreamProductTagClick: ({ product }) => {
...
<Note>
**Returning `false` on Android is not the same as doing nothing.** It gives you UIKit's built-in product web view, which leaves the stream playing in a floating window over it. iOS and Web return no value, so an override always replaces the default. On iOS, call `super.onLivestreamProductTagClick(context:)` where you still want the in-app browser.
</Note>
...
A host, co-host, or moderator can pin one chat message during a livestream so it stays visible above the live chat for every viewer, no matter how fast the feed scrolls. Use it for rules, a promo code, a link, or an announcement.

<Frame>
  <img
    src="/images/livestream-pinned-message-collapsed.png"
    alt="Livestream chat with a collapsed pinned message banner above the feed"
    style={{ maxWidth: "36%", display: "inline-block", margin: "0 2%" }}
  />
  <img
    src="/images/livestream-pinned-message-expanded.png"
    alt="Livestream chat with the pinned message banner expanded to show the full text"
    style={{ maxWidth: "36%", display: "inline-block", margin: "0 2%" }}
  />
</Frame>
...
| Role | Pin from the ⋮ menu | Unpin (banner "X" or the ⋮ menu) | Sees the banner |
| --- | --- | --- | --- |
| Host | ✅ | ✅ | ✅ |
| Co-host | ✅ | ✅ | ✅ |
| Moderator or any role with `PIN_MESSAGE` | ✅ | ✅ | ✅ |
| Viewer, including muted viewers | — | — | ✅ |

The UIKit shows the pin controls when the current user is the host or a co-host, or holds `PIN_MESSAGE` in the live chat channel, and the room is live. The server enforces the permission on every request. When a user's permission changes mid-stream, the pin controls appear or disappear on their own — no user action, no error message: a **channel-level** role change (moderator or co-host) arrives over the `channel.roleAdded` / `channel.roleRemoved` realtime events, and a **network-level** change made from the Console arrives over the `user.updated` event (the UIKit subscribes to the current user's events while the livestream chat is open). The pinned message banner itself is unaffected (it is server-owned and stays visible to everyone). A stale pin or unpin that still slips through returns `403`, and the UIKit refreshes permissions then as a backstop.

### Pinning Behavior

- **Pin from the message menu.** "Pin message" appears at the top of a message's ⋮ menu, above "Report message" and "Delete message". It is hidden on deleted messages and on messages that failed to send. There is no confirmation dialog.
- **One pin at a time.** Pinning another message replaces the current pin in one step. The previous message loses its banner and stays in the feed.
- **The original message never moves.** Pinning only adds the banner. No pin indicator is drawn on the message inside the feed, and the feed is not reordered.
- **Unpin.** The "X" on the banner unpins, and is shown only to users who can pin. The ⋮ menu of the pinned message also offers **Unpin message** in place of **Pin message**, so there are two ways to unpin.
- **Live only.** The banner and the pin controls exist only while the room status is live. When the stream is idle, ended, or in recorded playback, the banner is not rendered even if the channel still has a pinned message.
- **Automatic unpin.** When the pinned message is deleted, or its author is banned from the channel or globally banned, the server clears the pin. The banner disappears for everyone and the message shows the "This message was deleted." placeholder in the feed. A community ban does not affect the pin.
- **Muted authors.** A muted user's message can be pinned and unpinned. The banner shows the muted indicator next to the author with the same visibility rules as message bubbles.

### Pinned Message Banner

The banner renders at the top of the chat column, above the feed and above the pinned product card. Messages scroll behind it.

| Part | Description |
| --- | --- |
| Byline | Author display name with the brand, host, co-host, or moderator badge, and the muted indicator when the author is muted |
| "Pinned" pill | Thumbtack icon with the "Pinned" label, right-aligned |
| Text | Every new pin renders collapsed to one line with an ellipsis and a trailing "more" indicator. Text that fits on one line shows no indicator. |
| "X" | Unpin control, shown only to users who can pin |

**Tap the bubble to expand or collapse.** The whole bubble is the tap target. "more" and "less" are passive labels, not buttons. Tapping anywhere on a collapsed banner expands it to the full text with a trailing "less" indicator. Tapping again collapses it. A replacing pin always starts collapsed. On mobile the chat column grows upward as the banner expands, and taps on the banner do not toggle the video controls.

### Customization Options

The pin controls and the banner use the `livestream_chat_feed` component ID. Existing `livestream_chat` keys for the compose bar and theme keep working.

All platforms build a config key as `pageId/componentId/elementId`, and `livestream_chat_feed` is the **component** ID — so it goes in the **middle** slot: `*/livestream_chat_feed/<element>`.

| Config ID | Type | Description |
| --- | --- | --- |
| `*/livestream_chat_feed/pinned_message_banner` | Element | Customize or exclude the pinned message banner. **All platforms.** |
| `*/livestream_chat_feed/pin_message_button` | Element | Customize or exclude the "Pin message" item in the message ⋮ menu. **Web and Android only** — iOS does not register this element (the pin control is not customizable there). |
| `*/livestream_chat_feed/unpin_message_button` | Element | Customize or exclude the unpin control: the "X" on the banner and, on Android and Web, the ⋮ menu's "Unpin message" item. **Web and Android only** — not registered on iOS. |

### Code Examples

The pin action and the banner are built into the livestream chat, so the pages you already use render them. The viewer sees them on the livestream player page and the streamer sees them on the broadcast view. There is nothing to mount separately. Use the config keys above to relabel or exclude the controls.
...
// Pin controls appear automatically for hosts, co-hosts, and PIN_MESSAGE holders.
const config = {
  // Hide a control entirely by excluding its element key.
  excludes: [],
  customizations: {
    '*/livestream_chat_feed/pin_message_button': { text: 'Pin message' },
    '*/livestream_chat_feed/unpin_message_button': { text: 'Unpin message' },
    '*/livestream_chat_feed/pinned_message_banner': {},
  },
};

const LivestreamViewer = ({ post }) => (
  <AmityUiKitProvider
    apiKey="API_KEY"
    apiRegion="API_REGION"
    userId="userId"
...
```json iOS config.json
{
  "excludes": [],
  "customizations": {
    // iOS registers only the banner element (`pinned_message_banner`).
    // There are no `pin_message_button` / `unpin_message_button` element IDs on iOS,
    // so the ⋮-menu pin/unpin items cannot be customized or excluded there.
    "*/livestream_chat_feed/pinned_message_banner": {}
  }
}
```
...
```swift iOS
// The pinned message banner renders inside the livestream player page
// and the broadcast view. No extra wiring is needed.
let livestreamPlayer = AmityLivestreamPlayerPage(post: post)
let viewController = AmitySwiftUIHostingController(rootView: livestreamPlayer)
navigationController?.pushViewController(viewController, animated: true)
```

```json Android config.json
{
  "excludes": [],
  "customizations": {
    "*/livestream_chat_feed/pin_message_button": { "text": "Pin message" },
    "*/livestream_chat_feed/pinned_message_banner": {},
    "*/livestream_chat_feed/unpin_message_button": {}
  }
}
```
...
<Note>
  All platforms write the component ID in the **middle** slot of a config key (`*/livestream_chat_feed/<element>`). To remove a control, add its key to `excludes`. Excluding `unpin_message_button` keeps the banner visible but removes the "X" for everyone; on Android and Web it also removes the ⋮ menu's "Unpin message" item, because both controls share that key. On **iOS**, only `pinned_message_banner` is registered — the pin and unpin menu items have no element IDs there, so they cannot be relabeled or excluded.
</Note>

### Error Handling

Pin and unpin failures are silent by design. No toast or dialog is shown. The ⋮ menu closes, the pin state is unchanged, and the banner keeps following the channel.

| Scenario | What the viewer sees |
| --- | --- |
| Network failure on pin or unpin | Nothing pin-specific. The existing "No internet connection." indicator applies when the device is offline. |
| Pin a message that was deleted concurrently | No pin is applied. The message row becomes the "This message was deleted." placeholder. |
| Pin or unpin after the user's role was revoked | Nothing. The pin controls are hidden from the next menu open onward. |
| Unpin a message that is no longer the current pin | Nothing. The banner already shows the newer pin. |

### Limits

| Limit | Value |
| --- | --- |
| Pinned messages per channel | 1 |
| Supported message type | Text |
| Channel type | `live` channels only |
...
Picture-in-Picture (PiP) keeps a livestream playing in a small floating window when the viewer leaves the player — by backgrounding the app or navigating to another page, such as opening a tagged product. Both live streams and recorded livestreams can float, and playback continues uninterrupted as the viewer moves away and back.

<Info>
  PiP uses each platform's **native system Picture-in-Picture window**, so its size, position, styling, and gestures come from the operating system. **iOS and Android only** — no configuration is required to turn it on. Broadcasters (host, co-host, and backstage) are excluded so they stay in the app to keep streaming.
</Info>
...
| Control        | Android              | iOS                                    |
| -------------- | -------------------- | -------------------------------------- |
| Play / pause   | ✅ (recorded & live) | ✅ recorded — on live, shown but inactive |
| Skip ±10s      | ✅ recorded only     | ✅ recorded — on live, shown but disabled |
| Expand / close | ✅                   | ✅                                     |
...
| ----------- | ------------------------------------------------------------------------------------------------------------------- |
| **Android** | None. UIKit declares Picture-in-Picture support in its own manifest, which merges into your app automatically.       |
| **iOS**     | Enable the **Background Modes** capability with *Audio, AirPlay, and Picture in Picture*. Without it, PiP never starts. |
...
| Feature                            | iOS | Android | Web | Flutter | React Native |
| ---------------------------------- | --- | ------- | --- | ------- | ------------ |
| Float on backgrounding             | ✅  | ✅      | —   | 🚧      | 🚧           |
| Float on in-app navigation         | ✅  | ✅      | —   | 🚧      | 🚧           |
| Live and recorded playback         | ✅  | ✅      | —   | 🚧      | 🚧           |
| Expand back to the full player     | ✅  | ✅      | —   | 🚧      | 🚧           |
| Broadcasters excluded              | ✅  | ✅      | —   | 🚧      | 🚧           |

...
| Multi-stream restriction | Product tagging is only available from the main feed; co-hosts in secondary streams cannot manage products |

### Edge Cases

| Scenario | Behaviour |
| --- | --- |
| Product tagging module disabled mid-session | An error message is shown and all product features are disabled |
| A tagged product is deleted | The product shows an "unavailable" state with a disabled CTA |
| A tagged product becomes inactive | The product remains in the list; the CTA is still accessible if a link exists |
| Network error during pin/unpin | An error message is shown; the operation can be retried |
| Concurrent host + co-host actions | Last write wins, based on server timestamp |

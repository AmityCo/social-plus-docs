...
<Tabs>
  ...
  <Tab title="Stream Viewing">
    ...
    **Automatic Thumbnail**: The backend automatically generates a live thumbnail for the room session once the stream goes live (available via `liveThumbnailUrl` on the room model). After the stream ends and recording is processed, a recorded thumbnail is also auto-generated. UIKit uses these thumbnails automatically when displaying livestream posts in feeds -- no additional configuration is needed. If the creator uploaded a custom cover image, it takes priority over the auto-generated thumbnails.
    ...
    | `livestream_player_page/*/central_button` | Element | Customize the central pause/play button (mobile) |
    | `livestream_player_page/*/skip_backward_button` | Element | Customize the −10s skip button (recorded playback) |
    | `livestream_player_page/*/skip_forward_button` | Element | Customize the +10s skip button (recorded playback) |
...
<Info>
  Playback Controls apply to the livestream viewer and the shared video player used by video posts, media gallery, community video feeds, and recorded livestream playback. Live streams do not show skip controls.
</Info>
...
<Note>
  **Desktop web player:** One-step click-to-toggle is retained. Clicking the video pauses or resumes it directly; the central play icon persists while paused, the central pause icon appears briefly when resuming. Hover-to-reveal, mute, and fullscreen behavior are unchanged.
</Note>
...
<Info>
  **Live edge resolution:** On resume, the player seeks to the HLS live-sync position when available, or to the end of the seekable range otherwise. No paused offset is tracked or restored.
</Info>
...
| +10s near end | Clamps to the final frame; natural end-of-media handling applies |
...
| Boundary during rapid taps | Boundary clamps apply mid-burst without forcing a pause |
...
<Note>
  On desktop web, playback controls are a **UI refresh only** — icons are updated and the skip amount changes from 15s to 10s. Click-to-toggle, hover-to-reveal, mute, and fullscreen behavior are unchanged.
</Note>

...
Picture-in-Picture (PiP) keeps a livestream playing in a small floating window when the viewer leaves the player — either by backgrounding the app or by moving to another page inside it. Playback is never interrupted: the same player continues, so position, play state and audio carry across the transition and back again.

<Info>
  PiP uses each platform's **native system Picture-in-Picture window**, not a custom player. The window's size, position, styling and gestures come from the OS, and the viewer's device-level PiP setting governs it. **Android and iOS only** — no configuration is required to enable it.
</Info>
...
| Closing the window                   | Ends the session — the same as closing the player                                                                       |
...
flowchart TD
    ...
    C -->|PiP off in device settings| E[Playback stops — no error; navigation continues]

...
| Theme token | Default | Description |
| --- | --- | --- |
| `live_stream_chat_bubble_pinned_color` | `rgb(41 43 50 / 30%)` | Background of the pinned message banner. **Web only.** |
| `live_stream_chat_pinned_badge_color` | `rgb(99 104 120 / 50%)` | Background of the "Pinned" pill. **Web only.** |

<Note>
  The two theme tokens above exist only on **Web**. iOS hardcodes the banner background colour and ships the "Pinned" pill as a flat image; Android derives the colour from its theme background. Neither exposes these tokens, so the banner cannot be recolored the same way across platforms.
</Note>

| Default | Localization key (iOS) | Localization key (Web) |
| --- | --- | --- |
| Pin message | `live_chat_pin_message` | `amity_livechat_pinned_message_pin_button` |
| Unpin message | `live_chat_unpin_message` | `amity_livechat_pinned_message_unpin_button` |
| Pinned | `live_chat_pinned_message_badge` | `amity_livechat_pinned_message_badge` |
| more | `live_chat_pinned_message_more` | `amity_livechat_pinned_message_more` |
| less | `live_chat_pinned_message_less` | `amity_livechat_pinned_message_less` |

### Code Examples

The pin action and the banner are built into the livestream chat, so the pages you already use render them. The viewer sees them on the livestream player page and the streamer sees them on the broadcast view. There is nothing to mount separately. Use the config keys above to relabel or exclude the controls and to recolor the banner.

...
| Role | Pin from the ⋮ menu | Unpin with the banner "X" | Sees the banner |
| --- | --- | --- | --- |
| Host | ✅ | ✅ | ✅ |
| Co-host | ✅ | ✅ | ✅ |
...
The UIKit shows the pin controls when the current user is the host or a co-host, or holds `PIN_MESSAGE` in the live chat channel, and the room is live. The server enforces the permission on every request. When a user's role changes mid-stream, the `channel.roleAdded` / `channel.roleRemoved` realtime events update their channel permissions, and the pin controls appear or disappear on their own — no user action, no error message. The pinned message banner itself is unaffected (it is server-owned and stays visible to everyone). A stale pin or unpin that still slips through returns `403`, and the UIKit refreshes permissions then as a backstop.
...
- **Unpin from the banner.** The "X" on the banner is the only unpin control and is shown only to users who can pin. There is no "Unpin" item in the ⋮ menu.
...
| Config ID | Type | Description |
| --- | --- | --- |
| `livestream_chat_feed/*/pinned_message_banner` | Element | Customize or exclude the pinned message banner |
| `livestream_chat_feed/*/pin_message_button` | Element | Customize or exclude the "Pin message" item in the message ⋮ menu |
| `livestream_chat_feed/*/unpin_message_button` | Element | Customize or exclude the "X" unpin control on the banner |

| Theme token | Default | Description |
| --- | --- | --- |
| `live_stream_chat_bubble_pinned_color` | `rgb(41 43 50 / 30%)` | Background of the pinned message banner |
| `live_stream_chat_pinned_badge_color` | `rgb(99 104 120 / 50%)` | Background of the "Pinned" pill |
...
<Note>
  Android writes the component ID in the middle slot of a config key (`*/livestream_chat_feed/pin_message_button`). iOS and Web write it first (`livestream_chat_feed/*/pin_message_button`). To remove a control, add its key to `excludes`. Excluding `unpin_message_button` keeps the banner visible but removes the "X" for everyone.
</Note>

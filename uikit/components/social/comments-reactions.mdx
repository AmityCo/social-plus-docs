...
<AccordionGroup>
  ...
  <Accordion title="Optimistic Rendering" icon="bolt">
    **Replies appear before the server confirms them**

    - A new comment or reply is prepended to the thread immediately, so the thread does not wait for the network round trip
    - The reply settles in place once the server accepts it; if creation fails, it is not left behind as a successful reply
    - Applies to L0 comments and to L1/L2 replies on iOS, Android, and Web

    For the underlying sync states your own surfaces can read, see [optimistic comment creation](/social-plus-sdk/social/content-management/comments/creation/text-comment).
  </Accordion>
</AccordionGroup>

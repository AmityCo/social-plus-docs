...
<Tabs>
  <Tab title="Story Target Tab Component">
    ...
    **Live Now Rings** are visual indicators that appear alongside story rings to highlight currently active livestreams. They enable users to discover and join ongoing broadcasts directly from the story component.

    #### Overview

    Live Now Rings enhance the story discovery experience by displaying active livestreams as ring elements within the story tab component. These rings appear:
    - **In Global Feed**: Before story targets (AmityLiveRoomTarget)
    - **In Community Feed**: After story targets (AmityCommunityLiveRoomTarget)

    #### Features

    | Feature | Description |
    |---------|-------------|
    | Live Stream Discovery | Visual rings showing currently active livestreams |
    | Real-Time Updates | Automatically added/removed as streams start/end |
    | Direct Access | Tap to join livestream immediately |
    | Visual Differentiation | Distinct styling from regular story rings |

    #### Global Feed Integration

    In the global feed story tab component, Live Now Rings appear **before** regular story targets:

    **Display Order:**
    1. AmityLiveRoomTarget (Live Now Rings)
    2. AmityStoryTargetElement (Regular stories - resized for global case)

    <CodeGroup>
    ```swift iOS
    // Global feed with Live Now Rings
    let globalStoryTab = AmityStoryTabComponent(type: .globalFeed)
    // Live Now Rings automatically appear before story rings
    ```

    ```kotlin Android
    @Composable
    fun composeGlobalStoryWithLiveRings() {
        val feedType = AmityStoryTabComponentType.GlobalFeed()
        AmityStoryTabComponent(
            type = feedType
        )
        // Live Now Rings automatically appear before story rings
    ...
    In community profile story tab component, Live Now Rings appear **after** regular story targets:

    **Display Order:**
    1. AmityStoryTargetElement (Community stories - existing size)
    2. AmityCommunityLiveRoomTarget (Live Now Rings)

    <CodeGroup>
    ```swift iOS
    // Community feed with Live Now Rings
    let communityStoryTab = AmityStoryTabComponent(
        type: .communityFeed("community-123")
    )
    // Live Now Rings automatically appear after story rings
    ```

    ```kotlin Android
    @Composable
    fun composeCommunityStoryWithLiveRings(communityId: String) {
        val feedType = AmityStoryTabComponentType.CommunityFeed(communityId)
        AmityStoryTabComponent(
            type = feedType
        )
        // Live Now Rings automatically appear after story rings
    ...
    When a user taps a Live Now Ring, they navigate directly to the livestream viewer:

    <CodeGroup>
    ```swift iOS
    // Handled automatically by UIKit
    // Tapping Live Now Ring opens AmityLivestreamPlayerPage
    // with the associated post or postId
    ```
    ...
    **Global Feed:**
    - If no live rooms AND no stories exist: Hide entire AmityStoryGlobalTabComponent

    **Community Profile:**
    - If no live room AND no story AND no create story permission: Hide AmityCommunityLiveRoomTarget

    ```swift
    // Empty state is handled automatically by the component
    // Component visibility determined by:
    // - Live room posts availability (from getLiveRoomPosts API)
    // - Story targets availability
    // - User permissions (create story)
    ```

    #### Refresh Behavior

    Page-level refresh operations automatically affect live room posts queries:

    ```swift
    // Pull-to-refresh on parent page automatically:
    // - Refreshes live room posts query
    // - Updates Live Now Ring visibility
    // - Syncs ring status with backend
    ```

    #### Real-Time Updates

    Live Now Rings update automatically when:
    - A livestream starts (ring appears)
    - A livestream ends (ring disappears)
    - Room status changes
    - User joins/leaves stream

    #### Customization Options

    | Config ID | Type | Description |
    |-----------|------|-------------|
    | `*/story_tab_component/live_room_ring` | Element | Customize live room ring with `progress_color` and `background_color` |
    | `*/story_tab_component/live_room_ring_badge` | Element | Customize "LIVE" badge styling |

    #### Visual Differentiation

    Live Now Rings are visually distinct from story rings:
    - Different progress color
    - "LIVE" badge indicator
    - Unique background styling
    - Priority positioning (global: before stories, community: after stories)

    #### Data Source

    Live Now Rings are powered by:
    - **Global Feed**: `getLiveRoomPosts()` API
    - **Community Feed**: `getCommunityLiveRoomPosts([communityId])` API

    These APIs return posts where `room.status === 'live'`, automatically filtered and updated by the SDK.

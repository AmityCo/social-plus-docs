...
<Tabs>
  <Tab title="Global Search Page">
    **Comprehensive search across communities and users**
    ...
    | Feature | Description |
    | --- | --- |
    | Top Search Bar | Type search keyword and it will search across list of all communities and users. User results match display name only |
    ...
    <Note>
    UIKit user search matches the keyword against display names only. User IDs and profile handles are not matched. If you need ID matching, call the SDK's user search directly with the `searchBy` option, as described in [Search and Query Users](/social-plus-sdk/core-concepts/user-management/user-operations/search-and-query-users).
    </Note>
  ...
  <Tab title="Search Results">
    **Specialized search result displays**
    ...
    | Feature | Description |
    | --- | --- |
    | Users list | A list of users whose display name matches the search keyword |

...
<Tabs>
  ...
  <Tab title="Notification Tray">
    **Centralized notification management**
    ...
    Because these combine, a poll can land while the server cache still holds the previous value — in that case the change is only picked up on a **later** poll, so it can take up to roughly **two intervals** before a new notification lights the dot or appears in the tray. Opening the tray marks it seen and clears the dot immediately. Design for eventual consistency here, not instant updates.

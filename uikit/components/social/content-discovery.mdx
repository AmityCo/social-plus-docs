...
<CardGroup cols={2}>
  <Card title="Advanced Search" icon="filter">
    **Targeted Results**
    ...
    - Community, post, and user search
...
<Tabs>
  <Tab title="Global Search Page">
    **Comprehensive search across posts, communities, and users**
    ...
    The Global Search Page offers robust and intuitive search functionality, allowing users to search for and discover posts, communities, and users that match their interests and needs.

    ### Features

    | Feature | Description |
    | --- | --- |
    | Top Search Bar | Type a search term; results are scoped to the selected Posts, Communities, or Users tab |
    | Post search result | Displays matching posts and opens the post detail page |
    ...
    <Note>
      In the current Flutter UIKit, the Posts tab uses semantic (natural-language) post search, the Communities tab uses keyword search, and the Users tab uses display-name search. The Flutter SDK also exposes semantic community search separately, but this UIKit page does not wire that API into its Communities tab.
    </Note>
  ...
  <Tab title="Notification Tray">
    **Centralized notification management**
    ...
    #### Refresh Behavior & Data Freshness

    The notification tray — including the social home unread indicator (red dot) — is **not a real-time surface**. Two layers of latency stack up:

    1. **Server-side cache (SDK).** Tray items and the seen/unseen status are served from a short-lived server cache, so a newly triggered notification isn't immediately available to query.
    2. **UIKit polling.** The social home unread indicator re-checks the seen status on a fixed interval (approximately every 60 seconds) rather than streaming updates.

    Because these combine, a poll can land while the server cache still holds the previous value — in that case the change is only picked up on a **later** poll, so it can take up to roughly **two intervals** before a new notification lights or the dot appears in the tray. Opening the tray marks it seen and clears the dot immediately. Design for eventual consistency here, not instant updates.
...
<CardGroup cols={3}>
  <Card title="Social Feeds" icon="newspaper" href="/uikit/components/social/feeds">
    **Content Streams** Social feeds and content discovery components
  </Card>

  <Card title="Communities" icon="users" href="/uikit/components/social/communities">
    **Community Features** Community creation and management tools
  </Card>

  <Card title="Chat Components" icon="messages" href="/uikit/components/chat">
    **Messaging Features** Real-time chat and communication tools
  </Card>
</CardGroup>

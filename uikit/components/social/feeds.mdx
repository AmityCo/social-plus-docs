...
<Note>
Ranking, and the mix between these sources, is owned by the backend. There are no client-side knobs in the UIKit or SDK to weight or filter them. For network-level tuning, contact [social.plus Support](https://ekoapp.atlassian.net/servicedesk/customer/portal/3). See [For You Feed](/social-plus-sdk/social/discovery-engagement/feed/for-you-feed#what-the-for-you-feed-includes) for the SDK-level behavior.
</Note>
...
`AmitySocialHomePage` is the container that hosts the feed tabs, the create-post menu, and community discovery.
...
### Customize Social Home Page navigation
...
The components that render feed content: stories plus posts, the chronological global feed, and the first-run empty state.
...
```typescript React
import { AmityGlobalFeedComponent } from '@amityco/ui-kit';
...
### Customize Global Feed navigation
...
The entry point users tap to create a post, story, poll, or live stream.
...
### Customize Create Post Menu navigation
...
### Community Discovery config IDs

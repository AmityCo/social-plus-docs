...
<CardGroup cols={2}>
  ...
  <Card title="In-App Browser" icon="arrow-up-right-from-square">
    Tapping a product opens its link in an in-app browser, keeping users within the app. Override this to route the tap yourself — see below.
  </Card>
</CardGroup>

### Customize the Product Tap

The in-app browser is the default. Override `onPostProductTagClick` on `AmityGlobalBehavior` to send the tap to your own product screen or commerce flow instead.
...
<Note>
On Android the handler returns a `Boolean`: return `true` when your app handled the tap so the default in-app browser is skipped, or `false` to fall through to it. The iOS handler returns no value.
</Note>

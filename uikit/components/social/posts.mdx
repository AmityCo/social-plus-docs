...
The carousel picks **one** frame shape from the **first** attachment and locks it for every frame, so the post height never shifts as members move through the carousel. The shape is chosen from the source's width-to-height ratio, evaluated as **ordered bands** — not a simple "wider than tall" test, which would push near-square media to the wrong shape:

| Source ratio (`width / height`) | Frame shape |
| --- | --- |
| `ratio ≥ 1.25` | **16:9** — landscape |
| `0.80 < ratio < 1.25` | **1:1** — square |
| `ratio ≤ 0.80` | **4:5** — portrait |

Each frame is cropped to fill the locked shape; the full-screen viewer shows each item at its original aspect ratio. For **videos**, the file's rotation is applied to the stored dimensions before classifying, so a portrait clip is never locked into a landscape frame. In the composer, if the first attachment is removed, the shape re-derives from the new first attachment.

...
| Media Carousel (multiple images/videos) | ✅ | ✅ | ✅ | ✅ | ❌ |
...
<Frame>
  <img
    src="/images/post-media-carousel.png"
    alt="Media carousel — a post with multiple images shown as full-width swipeable frames with a pagination indicator and an n/total counter"
...
<CardGroup cols={2}>
  ...
  <Card title="Consistent Height" icon="ruler">
  ...
  <Card title="Position Indicator" icon="ellipsis">
  ...
  <Card title="Full-Screen Viewer" icon="up-right-and-down-left-from-center">
...
| Limit | Value |
| --- | --- |
| Attachments per post | 2–10 render as a carousel (a single attachment renders as one frame) |
| Media types per post | Images **or** videos — a single post cannot mix images and videos |
| Duplicate selection | Duplicate files are silently excluded before upload |
...
<Frame>
  <img
    ...
    alt="Post product tagging — tagged products displayed as a carousel below post content"
    style={{ maxWidth: "40%", margin: "0 auto", display: "block" }}
  />
</Frame>
...
Opening the product link is the default. Override `onPostProductTagClick` on `AmityGlobalBehavior` to send the tap to your own product screen or commerce flow instead.

The handler receives the tapped product on the context. Read it to decide where to go.

| Context field | Type | Available on |
| --- | --- | --- |
| `product` | `AmityProduct` | iOS, Android, Web |
| `pageContext` | `android.content.Context` | Android |

`AmityProduct` carries `productId`, `productName`, `productUrl`, `price`, `currency`, and `thumbnailUrl`. Use `productId` to look the item up in your own catalogue, or `productUrl` to open it yourself.

<CodeGroup>
```swift iOS
class MyGlobalBehavior: AmityGlobalBehavior {
    override func onPostProductTagClick(context: AmityGlobalBehavior.Context) {
        guard let product = context.product else { return }

        if let item = MyCatalogue.find(id: product.productId) {
            MyNavigator.showProduct(item)
        } else {
            // Fall back to the UIKit in-app browser.
            super.onPostProductTagClick(context: context)
        }
    }
}
...
AmityUIKit4Manager.behavior.globalBehavior = object : AmityGlobalBehavior() {
    override fun onPostProductTagClick(context: AmityGlobalBehavior.Context): Boolean {
        val product = context.product ?: return false

        val intent = Intent(context.pageContext, ProductActivity::class.java)
        intent.putExtra("productId", product.getProductId())
        context.pageContext.startActivity(intent)

        // Return true to claim the tap, so the in-app browser is skipped.
        return true
    }
}
```
...
When a post is pinned or featured, the UIKit renders a **read-only badge** on it in the feed. This applies to any post, not just events.

Pinning and featuring are **managed in the Console** — there is no in-app control in the UIKit to change that state; the component only reflects it. See the [pin/feature note](/social-plus-sdk/social/content-management/posts/moderation/pin-post) for how the state is set.

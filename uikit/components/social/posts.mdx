...
<Info>
  Product tagging requires the Product Catalog to be configured and enabled for your network in the console. The feature is available on iOS, Android, and Web only. In the post UIKit, the documented composer and renderer cover text captions and image/video attachments; livestream tagging uses the room-post surface. SDK support for other post types does not imply a UIKit composer or renderer for those types.
</Info>

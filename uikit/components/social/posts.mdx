...
| Scenario | Behaviour |
| --- | --- |
| Product tagging disabled mid-session | A dialog is shown, all tags are removed, and the user can continue publishing without tags |
| Tagged product is deleted | The product is hidden from the carousel on post view |
| Product catalog unavailable | An empty state is shown in the product picker |

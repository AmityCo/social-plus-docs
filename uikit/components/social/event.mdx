...
<Tabs>
  ...
  <Tab title="Event as Post">
    ...
    **Public API:**
    ...
    | Config ID | Type | Description |
    | --- | --- | --- |
    | `post_composer_page/*/event_post_card` | Element | Style the composer's embedded event card (background, border radius) |
    ...
    ### AmityEventPostCardElement

    Use **AmityEventPostCardElement** to render an event inside a post. In feeds, `AmityPostContentComponent` mounts this element automatically when `post.dataType === "event"`.
    ...
    **States:**
    ...
    | Deleted | Shows "This event is no longer available." The card is still tappable; the destination renders an error state. Comments and reactions on the post survive. |

    **Public API:**

    ```swift
    AmityEventPostCardElement(event: AmityEvent?, isLoading: Bool = false)
    ```
    ...
    | Config ID | Type | Description |
    | --- | --- | --- |
    | `*/*/event_post_card_element` | Element | Background, border radius, title / description / time / deleted text colors |

    ### Event Post in Feeds

    `AmityPostContentComponent` renders an event card whenever a post's `dataType` is `event`. The tap-through behavior differs from other post types:
...
```json
{
  ...
  "post_composer_page/*/event_post_card": {
    "background_color": "",
    "border_radius": ""
  },

  "*/*/event_post_card_element": {
    "background_color": "",
    "title_text_color": "",

...
<Tabs>
  ...
  <Tab title="Event Details">
    ...
    **Public API:**
    ...
    <Note>
    For a **user-hosted** event there is no origin community, so the member rule has no referent. Who may create a post for a user-hosted event is not yet specified — do not assume the community matrix applies.
    </Note>
  ...
  <Tab title="Event as Post">
    ...
    2. The user picks a target on the [target-selection page](#target-selection-page) — unless the event was created in a private community, in which case the target is locked to that community.
    ...
    There is a second entry point into the same flow. Immediately after a host creates an event, a modal bottom sheet prompts them to publish it — "Your event was created successfully" / "Post it to a feed so more people can find and join", with **Post to feed** and **Maybe later** actions. **Post to feed** enters the flow at step 2.

    <Warning>
    This sheet **replaces** the usual "event created" toast for the host, so it changes the existing create-event flow, not just the event-post flow. It is shown once per creation.
    </Warning>

    ### Target Selection Page

    The target-selection page picks where the event post is published. It has two variants driven by the event's origin:
    ...
    **Public API:**
    ...
    | Platform | Page |
    | --- | --- |
    | iOS, Web | `AmityEventPostTargetSelectionPage(event: AmityEvent)` — a dedicated page |
    | Android | `AmityPostTargetSelectionPage(type:, attachedEventId:, prefilledTitle:, prefilledBody:, isPrivateEvent:, lockedCommunityId:)` — the existing post target-selection page, with optional event parameters. There is no dedicated event page. |
    ...
    <Note>
    Android reuses the `AmityPostTargetSelectionPage` composable rather than introducing a dedicated page class, but it adopts the `event_post_target_selection_page` config namespace above whenever an event is attached — so these keys apply on every platform.
    </Note>
    ...
    fun composeEventPostTargetSelectionPage(event: AmityEvent) {
        // A private-community event can only be posted back into that community.
        ...
        AmityPostTargetSelectionPage(
            ...
            isPrivateEvent = isPrivateCommunityEvent,
            lockedCommunityId = event.getTargetCommunity()?.getCommunityId(),
        )
    }
    ...
    **Public API:**
    ...
    iOS and Web add a dedicated composer mode:
    ...
    Android has no separate mode. The existing create options carry optional event parameters, and the composer switches into event-post mode when `attachedEventId` is set. Note that it takes an **event ID**, not an `AmityEvent`, and has no `isTargetLocked` — on Android the lock is applied upstream on the target-selection page.

    ```kotlin
    AmityPostComposerOptions.AmityPostComposerCreateOptions(
        targetId: String?,
    ...
    If the referenced event has been deleted, the composer still opens and text edits still save; the embedded event card renders its deleted state. A **cancelled** event is treated identically to a deleted one.
    ...
    **Card states:**
    ...
    A **cancelled** event and an event missing from the payload both normalise into the deleted state — there is no separate cancelled treatment.

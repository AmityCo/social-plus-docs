...
<CardGroup cols={2}>
  ...
  <Card title="Deep Links & Navigation" icon="arrow-right">
    **Event navigation hooks via Behaviors**

    - Open event detail from notifications
    - Jump from feeds to upcoming/past pages
    - Navigate to event setup/edit
  </Card>
</CardGroup>
...
| Event Detail & Attendees Pages | ✅ | ✅ | - | - | - |
| My/Explore/Community Event Feeds | ✅ | ✅ | - | - | - |
...
<Tabs>
  ...
  <Tab title="Event Details">
    **View event information and attendees**
...
If you need to hook navigation (e.g., “View all upcoming”), use the corresponding Behavior class documented in the next tab.
  ...
  <Tab title="Behaviors">
    **Navigation hooks for integrating events into your app flow**

    Spec 6 lists the Behavior classes and required navigation methods. Override these methods to route users to the correct pages in your navigation stack.

    ### Cross-Feature Navigation

    - **AmityNotificationTrayPageBehavior**: `goToEventDetailPage(context)`
    - **AmityCommunityProfilePageBehavior**: `goToEventDetailPage(context)`
    - **AmityCreatePostMenuComponentBehavior**: `goToSelectEventTargetPage()`

    ### Event Page Navigation

    - **AmityEventTargetSelectionPageBehavior**: `goToEventSetupPage(context)`
    - **AmityEventSetupPageBehavior**: `goToEventDetailPage(context)`
    - **AmityEventDetailPageBehavior**:
      - `goToEventSetupPage(context)`
      - `goToLivestreamPostComposerPage(context)`
    ...
    ### Event Feed Navigation

    - **AmityExploreEventFeedComponentBehavior**: `goToUpcomingEventsPage(context)`
    - **AmityMyEventFeedComponentBehavior**:
      - `goToUpcomingEventsPage(context)`
      - `goToPastEventsPage(context)`

    ### Attendee Navigation

    - **AmityEventAttendeesPage**: `goToUserProfilePage(context)`

    <Info>
The exact registration APIs for behaviors depend on your UIKit integration. In most UIKit v4 integrations, behaviors are registered on the global behavior registry (for example, `AmityUIKit4Manager.behaviour`).
    </Info>
    ...
    ```swift iOS
    final class CustomEventTargetSelectionBehavior: AmityEventTargetSelectionPageBehavior {
        override func goToEventSetupPage(context: AmityEventTargetSelectionPageBehavior.Context) {
            let page = AmityEventSetupPage(mode: .create(context.targetId, context.targetName))
            let viewController = AmitySwiftUIHostingController(rootView: page)
            navigationController?.pushViewController(viewController, animated: true)
        }
    }
    ...
    ```kotlin Android
    class CustomMyEventFeedBehavior : AmityMyEventFeedComponentBehavior() {
        override fun goToUpcomingEventsPage(context: Context) {
            val intent = AmityUpcomingEventsPageActivity.newIntent(context)
            context.startActivity(intent)
        }

        override fun goToPastEventsPage(context: Context) {

...
<AccordionGroup>
  <Accordion title="User" icon="user">
    ...
    | `isGlobalMuted`   | boolean   | _(Computed)_ `true` when `globalMuteTimeout > now` (v3+).     |
    | `globalMuteTimeout` | date-time | When the global mute expires; a far-future date means no expiry. Admin-only — returned with `includeModerationDetails`. |
  ...
  <Accordion title="Room" icon="video">
    ...
    | `liveResolution`        | object    | _(Computed)_ Resolution while live: `{aspectRatio, width, height}`.                                 |
    | `recordedResolution`    | object    | _(Computed)_ Resolution of the recorded stream: `{aspectRatio, width, height}`.                     |

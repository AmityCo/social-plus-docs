...
<AccordionGroup>
  <Accordion title="Channel" icon="hashtag">
    ...
    | `pinnedMessage`                | object    | The channel's single pinned message (`live` channels only). `null` when nothing is pinned. Carries the message body plus `pinnedBy` and `pinnedAt`. |

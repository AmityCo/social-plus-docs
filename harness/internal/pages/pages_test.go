...
func TestDisplayNamesNotIncluded(t *testing.T) {
	registry, err := pages.Load("testdata/docs.json")
	require.NoError(t, err)
	// Group/tab display names must NOT be treated as valid page paths
	assert.False(t, registry.Contains("Chat"))
	assert.False(t, registry.Contains("SDK"))
	assert.False(t, registry.Contains("Channels"))
}

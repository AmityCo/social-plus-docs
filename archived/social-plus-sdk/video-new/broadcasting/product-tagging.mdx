...
<Note>
  **Deduplication**: The event key is `{productId}.view.{sourceType}.{sourceId}
  `. The same product viewed in a different post or room creates a separate event.
</Note>
...
<Note>
  **Deduplication**: The event key is `{productId}.linkClicked.{sourceType}.
  {sourceId}`. Clicking the same product in a different source creates a separate event.
</Note>

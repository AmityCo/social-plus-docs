...
class CommentActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        editingManager = CommentEditingManager(AmityCommentRepository.getComment())
...
class CommentEditingService {
  ...
  // Basic text editing
  async updateCommentText(commentId: string, newText: string): Promise<Amity.Comment> {
    try {
  ...
  canEditComment(comment: Amity.Comment, userId: string): boolean {
    // Check ownership
    if (comment.userId !== userId) return false;
...
class CommentEditingService {
  ...
  CommentEditingService(this._commentRepository);
  
  // Basic text editing
  Future<AmityComment> updateCommentText(String commentId, String newText) async {
    try {
      final updatedComment = await _commentRepository
  ...
  Future<CommentEditResult> safeUpdateComment({
    ...
    try {
      ...
      // Permission validation if requested
      if (validatePermissions) {
        final currentComment = await _commentRepository
...
class _CommentEditWidgetState extends State<CommentEditWidget> {
  ...
  void initState() {
    ...
    _editingService = CommentEditingService(AmitySocialClient.newCommentRepository());
  }
...
class CommentEditActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        imageEditingManager = ImageCommentEditingManager(
            AmityCommentRepository.getComment(),
            AmityFileRepository.getFile()
        )
...
class ImageCommentEditingService extends CommentEditingService {
  ...
  // Update comment with images
  async updateCommentWithImages(options: ImageEditOptions): Promise<Amity.Comment> {
  ...
  // Add images to existing comment
  async addImagesToComment(commentId: string, newImageFileIds: string[]): Promise<Amity.Comment> {
    const currentComment = await CommentRepository.getComment(commentId);
    const existingImageIds = currentComment.images?.map(img => img.fileId) || [];
  ...
  async removeImagesFromComment(commentId: string, imageIdsToRemove: string[]): Promise<Amity.Comment> {
    const currentComment = await CommentRepository.getComment(commentId);
    const existingImageIds = currentComment.images?.map(img => img.fileId) || [];
  ...
  async replaceCommentImages(commentId: string, newImageFileIds: string[]): Promise<Amity.Comment> {
...
extension ImageCommentEditing on CommentEditingService {
  ...
  Future<String> _uploadSingleImage(File file) async {
    ...
    final uploadResult = await AmityCoreClient
        .newFileRepository()
        .uploadImage(file);

...
Create rich, interactive comments with text, images, mentions, and threaded replies. The social.plus SDK provides optimistic updates for immediate user feedback and supports various content types for comprehensive commenting experiences.
...
class CommentInput extends React.Component<{
    postId: string;
    onCommentCreated: (comment: Amity.Comment) => void;
}> {
    private commentManager: CommentManager;
    
    constructor(props) {
        super(props);
        this.commentManager = new CommentManager(client); // client from createClient()
        this.state = {
            commentText: '',
            isSubmitting: false,
            error: null
...
class CommentManager {
  ...
  Future<AmityComment> createTextComment({
    ...
    try {
      var commentCreator = _createCommentTarget(
        referenceId: referenceId,
        referenceType: referenceType,
        parentId: parentId,
      ).create().text(text);
      ...
      if (mentionUsers.isNotEmpty) {
        commentCreator = commentCreator.mentionUsers(mentionUsers);
      }

      if (metadata != null) {
        commentCreator = commentCreator.metadata(metadata);
      }

      final comment = await commentCreator.send();
...
<Tab title="TypeScript">
```typescript
import { Client, CommentRepository, FileRepository } from '@amityco/ts-sdk';
...
class CommentManager {
  ...
  Future<AmityComment> _createCommentWithFile({
    ...
    var commentCreator = _createCommentTarget(
      ...
      referenceType: referenceType,
      parentId: parentId,
    ).create().attachments([CommentImageAttachment(fileId: fileId)]);

    if (caption != null) {
      commentCreator = commentCreator.text(caption);
    }

    return await commentCreator.send();
  }
}

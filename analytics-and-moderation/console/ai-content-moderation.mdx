...
Create safer online communities with intelligent, automated content moderation. Social+ leverages advanced AI to scan and filter inappropriate content across text, images, and video, ensuring community standards are maintained without constant manual oversight.
...
Social+ offers two complementary AI moderation approaches:
...
<Steps>
  ...
  <Step title="Configure Settings">
    Set up confidence levels and moderation categories through the Social+ Console.
  </Step>
...
<Steps>
  <Step title="Enable Image Moderation">
    Navigate to **Moderation > Image Moderation** in your Social+ Console and toggle "Enable image moderation" to **ON**.
  </Step>
...
<img src="../../.gitbook/assets/Screenshot 2567-11-13 at 11.27.13 AM.png" alt="Image Moderation Settings" />
...
### How Pre-Moderation Works

```mermaid
graph TB
    A[User Uploads Image] --> B[AI Scans Content]
    B --> C{Confidence Check}
    C -->|Below Threshold| D[Image Published]
    C -->|Above Threshold| E[Content Blocked]
    E --> F[User Notified]
...
Post-moderation uses a dual-threshold system to balance automation with human oversight:

```mermaid
graph TB
    A[Content Published] --> B[AI Analysis]
    B --> C[Confidence Score Generated]
    C --> D{Score Evaluation}
    
    D -->|Below Flag Threshold| E[Content Approved]
    D -->|Between Thresholds| F[Content Flagged]
    D -->|Above Block Threshold| G[Content Removed]
    
    F --> H[Review Queue]
    H --> I[Human Review]
    I --> J{Moderator Decision}
    
    J -->|Approve| K[Content Reinstated]
    J -->|Remove| L[Content Deleted]
    J -->|Escalate| M[Senior Review]
    
    G --> N[User Notification]
    L --> N
...
<AccordionGroup>
  <Accordion title="Configuration Strategy" icon="strategy">

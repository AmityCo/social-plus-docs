...
description: "Comprehensive guide to user roles, moderation privileges, and administrative controls in social.plus Console for effective community management."
---
  ...
  <Tab title="Admin Management">
    ...
    <Steps>
      <Step title="Access Admin Panel">
        Navigate to Console → **Admin Users** → **Manage Admin Users**
      </Step>
      <Step title="Create New Admin">
        Click **Create New Admin** and provide credentials
      </Step>
  ...
  <Tab title="Content Moderation">
    ...
    <AccordionGroup>
      <Accordion title="Blocklist Management">
        Reject content that contains words or phrases you don't want in your community.

        **How rules match:**
        - Each rule is a word or phrase, matched case-insensitively.
        - Rules match as plain text. Characters such as `*` or `.` are literal, not patterns.
        - **Partial match** (default): the rule matches anywhere in the text, including inside longer words.
        - **Exact word match** (`isMatchExactWord: true`): the rule matches only as a whole word.
        - Each rule can be enabled or disabled without deleting it.

        **Rule categories:**
        - `content` (default): checked against chat messages, posts, comments, and polls. For posts, the title is checked too.
        - `user`: checked against user display names and descriptions.

        **On violation:**
        - The request is rejected with error `400308` (HTTP 403).
        - For chat messages, the sender can also be muted in the channel when a blocklist mute duration is configured. The duration is one network-wide value, not set per rule.

        **Exemptions:** by default, users with the `global-admin` or `super-moderator` role skip the check for chat messages. Posts and comments are always checked.

        Manage rules with the blocklist API. Test text against your rules with `POST /api/v3/blocklists/verify`:

        ```json
        POST /api/v3/blocklists
        {
          "regexs": ["inappropriate word", "spam phrase"],
      ...
      <Accordion title="Allowlist Management">
        Keep links in your community limited to sources you trust. When link moderation is on, any link that doesn't match a rule on your allowlist is rejected.

        **Enable or disable:**
        - Link moderation is **on by default**. With an empty allowlist, every link is blocked.
        - Toggle it with the network moderation setting `isWhitelistEnabled`. When off, all links are allowed.

        **How rules match:**
        - Each rule is plain text, matched case-insensitively against every link found in the content.
        - A link is allowed if it **contains** the text of any enabled rule.
        - Wildcards and patterns aren't supported. `*.example.com` matches only the literal text `*.example.com`. To allow a domain and its subdomains, add `example.com`.
        - Protocols get no special treatment. `https://example.com` allows only links containing that exact text, so `http://example.com` is still blocked.
        - Because matching is by substring, a rule can also match text in another link's path or query string. For example, `example.com` allows `https://other.site/?ref=example.com`. Use a more specific rule, such as `https://example.com/`, to reduce this.
        - Each rule can be enabled or disabled without deleting it.

        **Checked content:** chat messages, posts, comments, and polls.

        **On violation:**
        - The request is rejected with error `400309` (HTTP 403).
        - For chat messages, the sender can also be muted in the channel when an allowlist mute duration is configured. The duration is one network-wide value, not set per rule.

        **Exemptions:** by default, users with the `global-admin` or `super-moderator` role skip the check for chat messages. Posts and comments are always checked.

        Manage rules with the allowlist API. Test links against your rules with `POST /api/v3/allowlists/verify`:

        ```json
        POST /api/v3/allowlists
        {
          "regexs": ["https://example.com/", "trusted-site.org"]
        }
        ```
      </Accordion>
    </AccordionGroup>

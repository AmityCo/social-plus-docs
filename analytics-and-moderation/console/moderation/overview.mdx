...
<CardGroup cols={2}>
  ...
  <Card title="User Management" icon="users-cog" href="user-moderation">
...
```mermaid
graph TB
    A[Content Flagged] --> B{AI Analysis}
    B -->|High Risk| C[Auto-Remove]
    B -->|Medium Risk| D[Review Queue]
    B -->|Low Risk| E[Approve with Flag]
    ...
    H -->|Remove| J[Take Action]
    H -->|Escalate| K[Senior Review]
    
    J --> L[User Notification]
    L --> M[Appeal Window]
    M -->|Appeal| N[Appeal Review]
    M -->|No Appeal| O[Case Closed]
```

### **User Action Escalation**

<Steps>
  <Step title="Initial Warning">
    First violation receives educational warning with policy explanation
  </Step>
  
  <Step title="Content Restrictions">
    Repeated violations result in content approval requirements
  </Step>
  
  <Step title="Temporary Suspension">
    Serious or persistent violations lead to temporary account restrictions
  </Step>
  
  <Step title="Permanent Action">
    Severe violations or pattern of abuse results in permanent restrictions
  </Step>
</Steps>

...
A channel holds one pinned message at a time. Pinning another message replaces the current pin. Deleting the pinned message, or banning its author from the channel or globally, clears the pin.
...
- When a message filter is active, the pinned message appears only if it matches the filter and is on the current page. When it appears, it is still the first row.

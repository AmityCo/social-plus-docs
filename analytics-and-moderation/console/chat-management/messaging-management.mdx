...
<Info>
This page explains how Console moderators and administrators manage real‑time chat: broadcasting, monitoring live streams, filtering, enforcing policy, handling mentions, pinning livestream messages, and applying message-level actions.
</Info>
...
<CardGroup cols={3}>
  ...
  <Card title="Pin" icon="thumbtack">Keep one key message on top of a livestream chat</Card>
...
{/* Removed missing images: Broadcast Channel View / Broadcast Compose Trigger / Broadcast Message Composer */}
...
{/* Removed missing image: Mention Settings */}
...
<Tabs>
  ...
  <Tab title="Pin / Unpin">Pin one message above a livestream chat, or remove the current pin. Livestream channels only. See [Pinned Messages](#pinned-messages).</Tab>
</Tabs>
...
Pin a message to keep a rule reminder, a link, or an important update in view for every viewer of a livestream, no matter how fast the chat scrolls.

<Info>
Pinning is available only in livestream channels: chat channels attached to a livestream post. The pin action does not appear in other channel types or in the thread view.
</Info>

### Pin or unpin a message

<Steps>
  <Step title="Open the channel">Go to **Channels** and select the livestream channel.</Step>
  <Step title="Open Messages">Select the **Messages** tab.</Step>
  <Step title="Pin">Open the message's actions menu (**⋯**) and select **Pin message**. The console confirms with "Message pinned."</Step>
  <Step title="Unpin">Open the actions menu on the pinned message and select **Unpin message**. The console confirms with "Message unpinned."</Step>
</Steps>

### Behavior

A channel holds one pinned message at a time. Pinning a new message replaces the current pin.

```mermaid
stateDiagram-v2
    [*] --> NoPin
    NoPin --> Pinned: Pin message
    Pinned --> Pinned: Pin another message (replaces pin)
    Pinned --> NoPin: Unpin message
    Pinned --> NoPin: Message deleted (auto)
    Pinned --> NoPin: Author banned from channel or globally (auto)

    NoPin: No pinned message
    Pinned: One pinned message
```

- The pinned message shows as the first row of the Messages table, with a pin marker ("Pinned"). This happens even when the message is on an older page.
- When a message filter is active, the pinned message is not moved to the top. It appears only if it matches the filter.
- Deleted messages have no pin action.
- To show the pinned message in your app, see [Pin and Unpin Messages](/social-plus-sdk/chat/messaging-features/messages/pin-message).

### Permissions

The server authorizes pin and unpin with the `PIN_MESSAGE` permission. In the console's role editor, it appears under **Manage messages** as "Pin a message in a live channel". Without it, the action fails with an access-restricted error.
...
| No **Pin message** action | Not a livestream channel, thread view, or deleted message | Open the livestream channel's Messages tab |
...
<CardGroup cols={3}>
  ...
  <Card title="Actions" icon="shield">Clear • Delete • Blocklist • Mute • Ban • Pin</Card>
</CardGroup>

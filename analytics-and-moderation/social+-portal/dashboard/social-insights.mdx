...
Overall gauge (−100 to +100, where 0 is balanced). Correlate dips with deployment timestamps or incident reports. Topic sentiment bars isolate outlier issues masked by aggregate positivity.
...
|--------|------------|----------------|----------------|
| Total Mentions | Total topic references in filtered period (one thread counts once per topic it is classified under) | Measures conversation volume / engagement energy | Spike > X% without planned campaign |
...
| Overall Sentiment Score | Net sentiment across topic mentions, `(positive − negative) / total × 100`, on a −100 to +100 scale | Health & morale indicator | Drop of >5 pts d/d or >8 pts w/w |

---
title: "Flutter"
description: "This page contains an overview of all relevant changes made to the Flutter SDK"
rss: true
---

<Note>
  This page is generated from the Flutter SDK source changelog in `Amity-Social-Cloud-SDK-Flutter-Internal/CHANGELOG.md`. Entries preserve the source release wording and omit dates where the source changelog does not provide one.
</Note>

<Update label="7.8.8" tags={["New Releases", "Improvements"]}>

  ## 🚀 New Features

  - Feat: Intelligent post search and intelligent community search (ported from the Android SDK).
  - Feat: Network-level story creation setting (`allowAllUserToCreateStory`).

  ## 🐞 Bug Fixes

  - Fix: Persist story targets so the seen state survives a restart; order the global story ring newest-first; forward the paging token when the global ring is in SMART mode.
  - Fix: Surface an approved post in the community feed immediately.
  - Fix: Community keyword search no longer returns unrelated communities.
  - Fix: Keep optimistic (unsynced) messages visible on the first page of a new conversation.
  - Fix: Keep the analytics event syncer alive through transient errors, and ensure live-collection `dispose()` completes.

</Update>

<Update label="7.8.6" tags={["Improvements"]}>

- Fix: community list cross-contamination - CommunityObserveListUseCase changed from singleton to factory so each filter/sort combination gets its own StreamController

</Update>

<Update label="7.8.5" tags={["Improvements"]}>

- Fix: community list sort instability during loadNext() - 3 compounding bugs fixed:
  - Sort comparator typo (`a.position` vs `a.position`) in paging ID adapter
  - Paging IDs saved one-at-a-time (N Hive watch events per page) -> now batch-saved
  - `_onChanges()` called without await allowing concurrent executions -> serialised with _inFlight queue

</Update>

<Update label="7.8.4" tags={["Improvements"]}>

- Fix: community LiveCollection `avatarImage` always null - replaced broken unawaited forEach with `Future.wait` map

</Update>

<Update label="7.8.3" tags={["Improvements"]}>

- Fix: post live object `children` never emits - images/videos always empty on live stream
- Fix: LiveCollection double emit causing UI flicker on every feed update

</Update>

<Update label="7.8.2" tags={["Improvements"]}>

- Chore: Improve MQTT connection stabilities after app is reopened

</Update>

<Update label="7.8.1" tags={["Improvements"]}>

- Fix: Fix inconsistent reaction when receive event through RTE
- Fix: Racing condition on channel Live Collection when receive new message

</Update>

<Update label="7.8.0" tags={["Improvements"]}>

- Chore: Improve MQTT connection stabilities
- Fix: Fix optimistic update issue on story reaction
- Fix: Handle nullable user object when compose domain object

</Update>

<Update label="7.7.12" tags={["Improvements"]}>

- Fix: Fix channel livecollection not updating on receiving new value

</Update>

<Update label="7.7.11" tags={["Improvements"]}>

- Fix: Fix user property in follow

</Update>

<Update label="7.7.10" tags={["Improvements"]}>

- Fix: Fix search message returning empty value

</Update>

<Update label="7.7.9" tags={["Improvements"]}>

- Fix: Fix user avatar is missing when get user by id

</Update>

<Update label="7.7.8" tags={["Improvements"]}>

- Fix: Error when calling getStream

</Update>

<Update label="7.7.7" tags={["Improvements"]}>

- Fix: Incorrect global story target filtering

</Update>

<Update label="7.7.6" tags={["Improvements"]}>

- Fix: Handle edge case for story filtering.

</Update>

<Update label="7.7.5" tags={["Improvements"]}>

- Fix: Story targets now properly filtered to exclude expired stories.

</Update>

<Update label="7.7.4" tags={["Improvements"]}>

- Fix: Include missing user information in reaction object

</Update>

<Update label="7.7.3" tags={["Improvements"]}>

- Fix: Upgrade freezed package to latest version

</Update>

<Update label="7.7.2" tags={["Improvements"]}>

- Fix: SDK throws null exception when logout

</Update>

<Update label="7.7.1" tags={["Improvements"]}>

- Fix: Fix error on ChannelObserveTotalUnreadUseCase

</Update>

<Update label="7.7.0" tags={["New Releases"]}>

- Feat: Create messages with custom JSON data payloads - `createCustomMessage()`
- Feat: Update existing custom messages with new data - `editCustomMessage()`
- Feat: Search messages across channels with advanced filters - `searchMessage()`
- Feat: Search channels by name with type and membership filters - `searchChannels()`
- Feat: Retrieve list of archived channel IDs - `getArchivedChannelIds()`

</Update>

<Update label="7.6.1" tags={["Improvements"]}>

- Fix: Fix missing avatar url for current user object

</Update>

<Update label="7.6.0" tags={["New Releases", "Improvements"]}>

- Feat: Introduce getMyMembership api on community object
- Fix: Fix null data in community object

</Update>

<Update label="7.5.6" tags={["Improvements"]}>

- Fix: Add user matchType param

</Update>

<Update label="7.5.5" tags={["Improvements"]}>

- Fix: Fix invalid session state when token expired

</Update>

<Update label="7.5.4" tags={["Improvements"]}>

- Fix: Expose AccessTokenRenewal to public

</Update>

<Update label="7.5.3" tags={["New Releases", "Improvements"]}>

- Fix: Fail to renew token
- Feat: Support channel get total unreads

</Update>

<Update label="7.5.2" tags={["Improvements"]}>

- Fix: Fail to fetch community members
- Fix: Filter replies from First-level comments

</Update>

<Update label="7.5.1" tags={["New Releases"]}>

- Feat: Update dependency version

</Update>

<Update label="7.5.0" tags={["New Releases"]}>

- Feat: Add ability to flag message with reasons
- Feat: Performance improvements for live collections

</Update>

<Update label="7.4.6" tags={["New Releases", "Improvements"]}>

- Feat: Create channel with users
- Feat: Add isPublic, moderatorCount and notificationMode for AmityChannel
- Feat: Add Channel member livecollection on search and query member.
- Fix: Reaction issue after mqtt event

</Update>

<Update label="7.4.5" tags={["Improvements"]}>

- Fix: Get reaction return error on calling pagingData

</Update>

<Update label="7.4.4" tags={["Improvements"]}>

- Fix: Get followings/followers api not return value as expected.

</Update>

<Update label="7.4.3" tags={["Improvements"]}>

- Fix: Remove displaying log on null data

</Update>

<Update label="7.4.2" tags={["Improvements"]}>

- Fix: Story data field is not updated properly

</Update>

<Update label="7.4.1" tags={["Improvements"]}>

- Fix: Resolved issue where posts were duplicating after being edited

</Update>

<Update label="7.4.0" tags={["New Releases"]}>

- Feat: Update file path of locally created message
- Feat: Included audio and video metadata

</Update>

<Update label="7.3.0" tags={["Improvements"]}>

- Fix: Clear SDK related data in Hive database when logging out instead of deleting all Hive database data from device

</Update>

<Update label="7.2.0" tags={["New Releases"]}>

- Feat: Ability to Archive/unarchive a channel
- Feat: Query archived channel
- Feat: Filter excluded archived channels from channel query

</Update>

<Update label="7.1.0" tags={["New Releases"]}>

- Feat: post & comment as a brand
- Feat: urls in post & comment contents are highlighted now
- Feat: supported see more functionality if the content is too long in post & comment

</Update>

<Update label="7.0.0" tags={["New Releases", "Improvements"]}>

- Breaking Change: Removed AmityRegionalSocketEndpoint from AmityCoreClientOption.
- Deprecate: Deprecated newGetMessages(String subchannelId), newCreateMessage(String subchannelId) API in MessageRepository.
- Feat: Introduced getMessages(String subchannelId) and createMessage(String subchannelId) API in MessageRepository.

</Update>

<Update label="0.57.3" tags={["Improvements"]}>

- Fix: Fix issues when fetch following/follower user by getPagingData()

</Update>

<Update label="0.57.2" tags={["Improvements"]}>

- Fix: Fix issue where message, follower and following is not updated in live collection

</Update>

<Update label="0.57.1" tags={["Improvements"]}>

- Fix: Fix random crash when logout

</Update>

<Update label="0.57.0" tags={["New Releases"]}>

- Feat: Update Community membership Live Collection to support RTE
- Feat: Introduce Following / Follower Live Collection
- Feat: Introduce User RTE
- Feat: Introduce Message Preview on channel object

</Update>

<Update label="0.56.0" tags={["New Releases"]}>

- Feat: Introduce poll object as Linked object in post object

</Update>

<Update label="0.55.5" tags={["Improvements"]}>

- Fix: Resolve inconsistent token renewal module

</Update>

<Update label="0.55.4" tags={["Improvements"]}>

- Fix: Activate token watcher to do access token renewal automatically
- Fix: Fix race condition inside login api business logic

</Update>

<Update label="0.55.3" tags={["Improvements"]}>

- Chore: Protected internal function inside Live Collection

</Update>

<Update label="0.55.2" tags={["New Releases"]}>

- Feat: Change login api to session v5
- Feat: Introduce ability to renew access token automatically
- Feat: Accept session handler when login to handle access token renewal with auth token

</Update>

<Update label="0.55.1" tags={["Improvements"]}>

- Fix: Address invalid exception when logging in offline
- Fix: Resolve issue with broken emoji messages received via MQTT

</Update>

<Update label="0.55.0" tags={["New Releases", "Improvements"]}>

- Feat: Introduce Global featured posts
- Fix: Improve stability on MQTT reconnection
- Fix: Fix fail to fetch channels

</Update>

<Update label="0.54.5" tags={["New Releases", "Improvements"]}>

- Feat: Update Reaction Live Collection to support RTE
- Fix: Handle null event name on reactor response

</Update>

<Update label="0.54.4" tags={["New Releases"]}>

- Feat: Introduce SubChannel Repository in ChatClient

</Update>

<Update label="0.54.3" tags={["Improvements"]}>

- Fix: Fix Comment Live Collection doesn't emit data when client doesn't specify isDeleted on query builder
- Fix: Fix Pinned post Live Collection doesn't emit data

</Update>

<Update label="0.54.2" tags={["New Releases", "Improvements"]}>

- Feat: Update Comment Live Collection to use Query Stream
- Fix: Fix null channel.defaultSubChannelId

</Update>

<Update label="0.54.1" tags={["Improvements"]}>

- Fix: Ensure service locator ready when setup SDK client

</Update>

<Update label="0.54.0" tags={["New Releases"]}>

- Feat: Channel Live Object
- Feat: Message Live Object
- Feat: Message Live Collection
- Feat: User Live Object
- Feat: User Live Collection
- Feat: User Notification Setting
- Feat: Channel Notification Setting
- Feat: Introduce Video Message

</Update>

<Update label="0.53.0" tags={["New Releases", "Improvements"]}>

- Feat: Expose default subchannelId for channel object
- Deprecate: Channel creation with custom channelId

</Update>

<Update label="0.52.2" tags={["Improvements"]}>

- Fix: Fix reaction missing when receive post event through RTE

</Update>

<Update label="0.52.1" tags={["Improvements"]}>

- Fix: Exclude deleted posts from global feed and custom post ranking Live Collection

</Update>

<Update label="0.52.0" tags={["New Releases", "Improvements"]}>

- Feat: Introduce Subchannel
- Feat: Migrate Message to MessageV5
- Feat: Support auto subscribe to smartfeed RTE
- Feat: Introduce Channel Real-Time Events
- Feat: Introduce Subchannel Real-Time Events
- Fix: Fix MQTT connection stability on iOS devices

</Update>

<Update label="0.51.0" tags={["New Releases"]}>

- Feat: Introduce ability to observe client session state
- Feat: Introduce ability to observe Live Collection loading state
- Feat: Post Live Collection
- Feat: Global feed Live Collection
- Feat: Custom ranking global feed Live Collection
- Feat: Pinned post Live Collection

</Update>

<Update label="0.50.0" tags={["Improvements"]}>

- Fix: Fix invalid story reaction count

</Update>

<Update label="0.49.0" tags={["Improvements"]}>

- Fix: Handle story issues

</Update>

<Update label="0.48.0" tags={["New Releases", "Improvements"]}>

- Feat: Channel Live Collection
- Fix: Fix inconsistent behavior of Community Live Collection
- Feat: Support retain previous session after re-open the app

</Update>

<Update label="0.47.1" tags={["Improvements"]}>

- Fix: Community live collection page size issue.

</Update>

<Update label="0.47.0" tags={["New Releases"]}>

- Feat: Introduce 'includeDeleted' options to Channel Membership Query and Search, Community Membership Query and Search api.

</Update>

<Update label="0.46.2" tags={["Improvements"]}>

- Fix:  Handle spamming unexpected error log from nullable objects.
- Chore: Config Dio to do JSON Parser on the background thread.

</Update>

<Update label="0.46.1" tags={["Improvements"]}>

- Fix: Post Content being null after added or removing reaction from the Custom Post.

</Update>

<Update label="0.46.0" tags={["Improvements"]}>

- Fix: Community live collection return wrong order when query with displayName filter.

</Update>

<Update label="0.45.0" tags={["New Releases"]}>

- Feat: Ability to create and update custom post.

</Update>

<Update label="0.44.0" tags={["Improvements"]}>

- Fix: Getting comment relies in the query comments.

</Update>

<Update label="0.43.0" tags={["New Releases"]}>

- Feat: Add creator property as AmityUser to AmityReaction.

</Update>

<Update label="0.42.0" tags={["Improvements"]}>

- Fix: PostedCommunityMember is always null.

</Update>

<Update label="0.41.0" tags={["New Releases"]}>

- Feat: Global Story Target Added
- Feat: Story can be marked as view by AmityStory.analytics().markAsSeen()
- Feat: Story can be marked as Linked Clicked by AmityStory.analytics().markLinkAsClicked()
- Feat: Reached user for the AmityStory can be fetched.
- Feat: Story RTE Subscription added.
- Feat: Urls can be validated now by using AmityCoreClient().validateUrls(urls)
- Feat: Texts can be validated now by using AmityCoreClient().validateTexts(texts)

</Update>

<Update label="0.40.0" tags={["New Releases"]}>

- Feat: Comment on Story added added
- Feat: Reply to Comment on Story has been added
- Feat: Reaction on Story  has been added
- Feat: Reaction to comment on Story  has been added
- Feat: Story Traget Live collection has been added (Get Story Targets by tragets)
- Feat: Story Traget Live Object has been added (Get Story Target by traget)

</Update>

<Update label="0.39.0" tags={["New Releases"]}>

- Feat: Comment Live collection have been added
- Feat: Comment Live object has been added
- Feat: Reaction Live collection has been added
- Feat: Poll Live object has been added
- Feat: Community Live object has been added
- Feat: Community Live collection has been added
- Feat: Ability to create post with livestream id

</Update>

<Update label="0.38.1" tags={["Improvements"]}>

- Fix: Handle spamming unexpected error log from nullable objects.

</Update>

<Update label="0.38.0" tags={["Improvements"]}>

- Fix: Error of FeedType missmatched while creating the video story. Added ability to add FeedType while uploading video file.

</Update>

<Update label="0.37.1" tags={["Improvements"]}>

- Fix: Handle spamming unexpected error log from nullable objects.

</Update>

<Update label="0.37.0" tags={["New Releases"]}>

- Feat: The post count for a community can be fetched on the basis of AmityFeedType by using AmityCommunity.getPostCount.

</Update>

<Update label="0.36.0" tags={["New Releases"]}>

- Feat: Video Story can be created using AmitySocialClient.newStoryRepository().createVideoStory.
- Feat: Image Story can be created using AmitySocialClient.newStoryRepository().createImageStory.
- Feat: StoryLiveCollection can be used to observe Active stories.

</Update>

<Update label="0.35.0" tags={["New Releases", "Improvements"]}>

- Feat: Added Post Impression, now post can be marked as read using amityPost.analytics().markPostAsViewed(). Likewise post impression and reach can be get by using amityPost.impression and amityPost.reach
- Upgrade: Upgraded dart version from ">=2.15.1 <4.0.0" to ">=3.0.0 <4.0.0"
- Upgrade: Upgraded the dependencies except Socket as amity backend currently works on socket v2
- Fix: AmityFeedType filter not working on post live collection

</Update>

<Update label="0.34.0" tags={["New Releases"]}>

- Feat: Post Live Object can be used by AmitySocialClient.newPostRepository().live.getPost(postId)
- Feat: Post Live collection have been added
- Feat: Now user can get category by AmitySocialClient.newCommunityRepository().getCategory(categoryId)
- Feat: Now user can get global feed custom ranking by AmitySocialClient.newFeedRepository().getCustomRankingGlobalFeed()

</Update>

<Update label="0.33.0" tags={["New Releases", "Improvements"]}>

- Feat: Upgraded, Upgradable libraries that would not cuase breaking changes
- Refactor: AmitySocialClient.newCommunityRepository().getCurentUserRoles is depercated in favor of AmitySocialClient.newCommunityRepository().getCurrentUserRoles
- Fix: Crash when getting current user roles for community
- Fix: Crash when getting reations of post

</Update>

<Update label="0.32.0" tags={["Improvements"]}>

- Upgrade: amity_sdk_version

</Update>

<Update label="0.31.0" tags={["New Releases", "Improvements"]}>

- Fix: Exception when scrolling through the posts
- Feat: Auto Subscribe to the the network

</Update>

<Update label="0.30.0" tags={["New Releases"]}>

- Feat: Added Support for getting live and recorded streams
- Feat: Configuration for AmityVideoPlayer added as well

</Update>

<Update label="0.29.0" tags={["Improvements"]}>

- Fix: Width and Height coming null for video

</Update>

<Update label="0.28.0" tags={["New Releases", "Improvements"]}>

- Feat: Added tags option when creating new message
- Fix: removed flaggedByMe in favour of isFlaggedByme for users

</Update>

<Update label="0.27.0" tags={["New Releases", "Improvements"]}>

- Feat: Added comminity user rolers in comments
- Fix: Now video upload give back the upload progress

</Update>

<Update label="0.26.0" tags={["Improvements"]}>

- fix: File is now optional so in case if file for a post is deleted it will return nulll instead of crashing

</Update>

<Update label="0.25.0" tags={["Improvements"]}>

- fix: Get Post by ID not working

</Update>

<Update label="0.24.0" tags={["Improvements"]}>

- fix: DataType Filter is now working on user and community feeds

</Update>

<Update label="0.23.0" tags={["New Releases", "Improvements"]}>

- fix: Member can be removed from community and live channel
- fix: iOS can not receive notifications
- feat: Updated Dio to version 5

</Update>

<Update label="0.21.0" tags={["New Releases"]}>

- feat: Added create comment with Image
- feat: Added query comment with Image
- refactor: align function signature and namespace for Core module
- refactor: align function signature and namespace for Social module
- refactor: align function signature and namespace for Chat module

</Update>

<Update label="0.20.0" tags={["New Releases", "Improvements"]}>

- feat: Added user block/unblock
- feat: Added get blocked users
- fix: resolve issue with create commetn(Update comment count in post)

</Update>

<Update label="0.19.0" tags={["New Releases"]}>

- feat: Added RTE event for Post topic
- feat: Added RTE event for Comment topic
- feat: Added RTE event for Channel topic
- feat: Added RTE event for Community topic

</Update>

<Update label="0.18.0" tags={["New Releases"]}>

- feat: File uploading progress
- feat: Image uploading progress
- BREAKING FEAT: Updated Future Upload public API to Stream Upload API

</Update>

<Update label="0.17.1" tags={["Improvements"]}>

- fix: resolve issue with community query delete flag

</Update>

<Update label="0.17.0" tags={["New Releases", "Improvements"]}>

- feat: Community user search by keyword
- feat: Mention user in post creation
- feat: Mention user in comment creation
- fix: resolve issue with flag update

</Update>

<Update label="0.16.0" tags={["New Releases", "Improvements"]}>

- feat: Global user search
- feat: Comunity User Search
- feat: Global user search sorting
- fix: Added amityuser avatar file in compose usecase

</Update>

<Update label="0.15.0" tags={["New Releases", "Improvements"]}>

- feat: User mention in channel message
- feat: Channel mention in channel message
- feat: Channel message mention helper
- feat: Channel member search
- feat: isFlaggedByMe check for user
- fix: handled null check in live stream post

</Update>

<Update label="0.14.0" tags={["New Releases", "Improvements"]}>

- feat: Reply to channel message
- feat: Flag/Unflag channel message
- feat: Flag/Unflag channel message sender
- feat: Add/Remove tags on channel message
- feat: Mute/Unmute channel from client side
- fix: Added Amity Object extension method for reporting message and user

</Update>

<Update label="0.13.1" tags={["Improvements"]}>

- fix: Resolve issue Amity Message Update Query Builder

</Update>

<Update label="0.13.0" tags={["New Releases", "Improvements"]}>

- feat: Channel Message Query & Filter
- feat: Channel Message Update
- feat: Channel Message Delete
- fix: Add tags support in v4/post query

</Update>

<Update label="0.12.0" tags={["New Releases", "Improvements"]}>

- feat: Create/Update Channel
- feat: Mute/Unmute Channel Member
- feat: Ban/Unban Channel Member
- fix: Amity post my reaction after reaction update

</Update>

<Update label="0.11.0" tags={["New Releases", "Improvements"]}>

- feat: Channel Member Add/Remove roles and permission
- feat: Check user perssion for channel
- fix: Channel Query filere validation

</Update>

<Update label="0.10.0" tags={["New Releases", "Improvements"]}>

- feat: Channel Member Query
- fix: Update listner method for AmityChannelMember model

</Update>

<Update label="0.9.0" tags={["New Releases", "Improvements"]}>

- feat: Channel Query
- feat: Get Channel By Id
- fix: resolve issue with Message RTE payload

</Update>

<Update label="0.8.0" tags={["New Releases", "Improvements"]}>

- feat: Create image/file message
- feat: Channel repo setup
- feat: Join/Leave channel
- feat: Add/Remove message reactions
- feat: Message reactions query
- fix: change live collection stream to stream controller
- fix: Error handling in live collection

</Update>

<Update label="0.7.0" tags={["New Releases"]}>

- feat: Create and Query Chat Message
- feat: Chat Message Live Collection
- feat: Foundation for Chat SDK

</Update>

<Update label="0.6.0" tags={["New Releases"]}>

- feat: Flag/Unflag post and comment
- feat: init socket.io client
- feat: Bloom filter for hashflag functionlity

</Update>

<Update label="0.5.0" tags={["New Releases", "Improvements"]}>

- feat: CRUD Poll Post
- feat: Close Poll Post
- feat: Vote Poll Post
- fix: resolve the issue with Global Ban flag

</Update>

<Update label="0.4.0" tags={["New Releases", "Improvements"]}>

- feat: Post Query Reaction
- feat: Comment/Nested Comment Query Reaction
- fix: Blank Custom Avatar Url
- fix: Blank Unfollow Response
- refactor: Optimize import & public api doc

</Update>

<Update label="0.3.1" tags={["Improvements"]}>

- fix: GET post api issue

</Update>

<Update label="0.3.0" tags={["New Releases", "Improvements"]}>

- feat: follow/unfollow functionlity
- feat: accept/denied pending follow request functionlity
- feat: Query user follower/following
- fix: login issue without display name
- fix: nested comment loading issue

</Update>

<Update label="0.2.0" tags={["New Releases", "Improvements"]}>

- feat: Community Post Review feature
- feat: Query community by tag
- feat: add tags in community
- feat: add recommend and trending communities in public repo
- feat: Add Community Category in Create & Update Community
- fix: data type filtering in get post API
- fix: resolve edge case with page controller blank token
- fix: resolve issue with multiple post in community feed
- fix: updated post query for v4 api

</Update>

<Update label="0.1.3" tags={["Improvements"]}>

- fix: null category ID in Community Category hive entiry
- fix: resolve edge case with page controller blank token

</Update>

<Update label="0.1.2" tags={["Improvements"]}>

- chore: enable the lint core and analyze the code

</Update>

<Update label="0.1.1" tags={["Improvements"]}>

- fix: community category causes data corruption
- fix: post metadata is unable to update

</Update>

<Update label="0.1.0" tags={["New Releases", "Improvements"]}>

- feat: Add Metadata in post, comment and community
- feat: Streams for Post, Comment, Community and Community Member
- feat: Community CRUD and Query
- feat: Community Member CRUD and Query
- feat: Community Member Roles & Permissions
- feat: Community feed query
- feat: Add/Remove Post and Comment reactions
- fix: resolve memory leak issue in post and comment object

</Update>

<Update label="0.0.2" tags={["New Releases", "Improvements"]}>

- feat: Comment CRUD and Query
- fix: get_it instance issue

</Update>

<Update label="0.0.1" tags={["Improvements"]}>

- docs: updated the README file
- docs: updated the LICENSE file

</Update>

<Update label="0.0.1-gazelle.3" tags={["New Releases", "Improvements"]}>

- feat: added example for Amity Core client setup and login
- fix: updated plugin to null safety
- fix: update the code according to flutter analyze

</Update>

<Update label="0.0.1-gazelle.2" tags={["Improvements"]}>

- docs: updated the README file

</Update>

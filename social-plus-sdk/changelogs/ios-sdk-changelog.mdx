...
<Update label="8.8.0  (25/09/2026)" tags={["New Releases","Improvements"]}>
  ## 🚀 New Features
  ...
  - **Turn it on:** `AmityCommunityDataBuilder.setNeedApprovalOnCommentCreation(_:)` on create or update. Read it back with `AmityCommunity.isCommentReviewEnabled`.
  - **Approval status:** `AmityComment.approvalStatus` is an `AmityCommentApprovalStatus` (`pending`, `approved`, `rejected`).
  - **Create without a local copy:** `AmityCommentCreateOptions(isOptimistic: false)` skips the local comment, so the comment you get back carries the server's approval status. The default, `true`, keeps today's behavior.
  - **Review queue:** `AmityCommentRepository.getCommentReviewQueue(with:)` returns the community's pending comments as an `AmityCollection<AmityComment>`. Narrow it to one post or a date range with `AmityCommentReviewQueueQueryOptions`.
  - **Approve or decline:** `AmityCommentRepository.approveComment(withId:)` and `declineComment(withId:reason:)`. Declining deletes the comment. The optional `reason` (at most 250 characters) is kept for the moderation record only.
  - **Permission:** reviewers need `AmityPermission.reviewCommunityComment`.
  ...
  - **Fetch a pool:** `AmityCuratedContentRepository.getPool(topicId:limit:)` is an `async throws` call that returns an `AmityCuratedContentPool`. `limit` must be between 1 and 100.
  - **Analytics:** `AmityCuratedContentTopic.analytics` (`AmityCuratedContentAnalytics`) provides `markAsViewed()`, `markPostAsViewed(postId:)`, and `markPostClick(postId:)`.
  ...
  - **Pin and unpin:** `AmityMessageRepository.pinMessage(withId:)` and `unpinMessage(withId:)`.
  - **Read the pin:** `AmityChannel.pinnedMessage` holds the current pinned message, or `nil` when nothing is pinned.
  - **Permission:** `AmityPermission.pinMessage`. Channel moderators, and livestream hosts and co-hosts, have it by default. Channel role changes now update a member's permissions in real time, so a promoted or demoted member's pin permission changes immediately.
  - **Realtime:** the observed `AmityChannel` updates on every pin and unpin, driven by the `channel.messagePinned` and `channel.messageUnpinned` events.

  #### Typed Network Activity Observation

  Added `AmityClient.observeNetworkActivities(_:)`, a typed observer that supports multiple observers and replaces the previous single-slot callback. It returns a cancellable `AmityNetworkActivityToken` and delivers `AmityNetworkActivity` records for HTTP, upload, download, and MQTT traffic. Each record is labeled with an `AmityNetworkActivityType` and carries real timings and byte counts. The previous `observeNetworkActivities((URLRequest?, HTTPURLResponse?, Data?) -> Void)` overload is deprecated but still works.

  ## ✨ Improvements

  - Added `AmityPost.getLocalCommentCount(includePending:)`. By default, the count leaves out the current user's own comments that are still awaiting approval. Pass `true` to include them. `AmityPost.localCommentCount` is now deprecated and equals `getLocalCommentCount(includePending: true)`.
  ...
  <Card title="📘 API Reference — 8.8.0" icon="book-open" href="https://ios-sdk.docs.amity.co/8.8.0/documentation/amitysdk/" horizontal />
</Update>

<Update label="8.7.0  (11/09/2026)" tags={["New Releases","Improvements"]}>
  ## 🚀 New Features

  #### Configurable Search Fields

  Added a search user type and a `searchBy` option to control which fields user search matches.

  #### Livestream Viewer Configuration

  Added livestream viewer configuration.

  #### v4 Sessions

  Updated the `loginWithAccessToken` API endpoint to use v4 sessions.

  ## 🐞 Bug Fixes

  - Fixed the avatar relationship not clearing when `avatarFileId` is emptied.
  ...
  <Card title="📘 API Reference — 8.7.0" icon="book-open" href="https://ios-sdk.docs.amity.co/8.7.0/documentation/amitysdk/" horizontal />
</Update>

<Update label="8.6.0  (28/08/2026)" tags={["New Releases","Improvements"]}>
  ## 🚀 New Features

  #### Event Posts

  Added event-post support: an event shared to a community feed is delivered as a post (post `dataType` `event`), resolves through the event repository, and renders alongside regular posts.

  ## 🐞 Bug Fixes

  - Fixed a notification-tray seen-state issue.
  ...
  <Card title="📘 API Reference — 8.6.0" icon="book-open" href="https://ios-sdk.docs.amity.co/8.6.0/documentation/amitysdk/" horizontal />
</Update>

<Update label="8.5.4  (25/08/2026)" tags={["Improvements"]}>
  ## 🐞 Bug Fixes

  - Fixed permission values to match the backend.
  ...
  <Card title="📘 API Reference — 8.5.4" icon="book-open" href="https://ios-sdk.docs.amity.co/8.5.4/documentation/amitysdk/" horizontal />
</Update>

<Update label="8.5.3  (18/08/2026)" tags={["Improvements"]}>
  ## 🐞 Bug Fixes

  - Fixed the query stream retaining objects that have left their context.
  ...
  <Card title="📘 API Reference — 8.5.3" icon="book-open" href="https://ios-sdk.docs.amity.co/8.5.3/documentation/amitysdk/" horizontal />
</Update>

<Update label="8.5.2  (14/08/2026)" tags={["Improvements"]}>
  ## 🐞 Bug Fixes

  - Fixed the query stream reactor incorrectly processing items from a foreign entity.
  ...
  <Card title="📘 API Reference — 8.5.2" icon="book-open" href="https://ios-sdk.docs.amity.co/8.5.2/documentation/amitysdk/" horizontal />
</Update>

<Update label="8.5.1  (07/08/2026)" tags={["Improvements"]}>
  ## 🐞 Bug Fixes

  - Fixed channel unread-count handling and channel data persistence.
  ...
  <Card title="📘 API Reference — 8.5.1" icon="book-open" href="https://ios-sdk.docs.amity.co/8.5.1/documentation/amitysdk/" horizontal />
</Update>

<Update label="8.5.0  (31/07/2026)" tags={["New Releases","Improvements"]}>
  ## 🚀 New Features

  #### Report User

  Updated the report-user API to report a user profile with a reason.

  ## ✨ Improvements

  - Dropped the MQTT will message as part of the MQTT protocol alignment.

  ## 🐞 Bug Fixes

  - Fixed `getArchivedChannels()` to fetch and persist the channel list.
  - Fixed the sort order of the archived chats list.
  ...
  <Card title="📘 API Reference — 8.5.0" icon="book-open" href="https://ios-sdk.docs.amity.co/8.5.0/documentation/amitysdk/" horizontal />
</Update>

<Update label="8.4.0  (16/07/2026)" tags={["Improvements"]}>
  ## 🐞 Bug Fixes

  - Fixed issues with channel archiving.
  ...
  <Card title="📘 API Reference — 8.4.0" icon="book-open" href="https://ios-sdk.docs.amity.co/8.4.0/documentation/amitysdk/" horizontal />
</Update>

...
<Update label="8.8.0  (25/09/2026)" tags={["New Releases","Improvements"]}>
  ...
  Communities can now require review before a comment is publicly visible. Enable it with `AmityCommunityDataBuilder.setNeedApprovalOnCommentCreation(_:)` (read back via `AmityCommunity.isCommentReviewEnabled`), and check `AmitySocialSettings.isAllowEditCommentWhenReviewingEnabled` to know whether an author may edit a comment while it is pending. A new comment carries `AmityComment.approvalStatus` (`AmityCommentApprovalStatus`: `pending`, `approved`, `rejected`); pending and rejected comments are excluded from default queries and counts, but remain visible to their author and to reviewers. Reviewers with the new `AmityPermission.reviewCommunityComment` permission can call `AmityCommentRepository.getCommentReviewQueue(with:)` for a live, per-community queue of pending comments, and `approveComment(withId:)` / `declineComment(withId:reason:)` to action them.

  #### Curated Content

  Added `AmityCuratedContentRepository.getPool(topicId:limit:)`, which fetches the ranked, safety-gated pool of posts behind a console-configured curated-content topic. The response is a single bounded read (no pagination) and is viewer-specific, since block-list filtering is applied per request. Pair it with `AmityCuratedContentTopic.analytics` (`AmityCuratedContentAnalytics`) to record widget views, post views, and post clicks for a topic.

  #### Pin Message in Live Channels

  Added the ability to pin a message in a livestream (`live`) chat channel. `AmityMessageRepository.pinMessage(withId:)` and `unpinMessage(withId:)` set or clear the channel's single pinned message, exposed as `AmityChannel.pinnedMessage`; pinning a new message replaces the current pin. Gated by the new `AmityPermission.pinMessage` permission, and channel role/permission changes now update in real time so a promoted or demoted member's pin permission reflects immediately.

  #### Typed Network Activity Observation

  Added `AmityClient.observeNetworkActivities(_:)`, a typed, multi-observer replacement for the previous single-slot callback. It returns a cancellable `AmityNetworkActivityToken` and delivers `AmityNetworkActivity` records — covering HTTP, uploads, downloads and MQTT traffic, each labeled with an `AmityNetworkActivityType` and carrying real timings and byte counts. The old `observeNetworkActivities((URLRequest?, HTTPURLResponse?, Data?) -> Void)` overload is deprecated but still works.

  ## ✨ Improvements

  - Added `AmityPost.getLocalCommentCount(includePending:)`; `AmityPost.localCommentCount` is now deprecated in favor of it (equivalent to passing `includePending: true`).

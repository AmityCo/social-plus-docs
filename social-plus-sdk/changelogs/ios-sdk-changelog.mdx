...
<Update label="8.2.0  (05/06/2026)" tags={["New Releases", "Improvements"]}>
  ...
  Added `tags` parameter to `AmityCommunityQueryOptions`. Pass a list of tags to `getCommunities()` to narrow results to communities matching all specified tags — ideal for topic-based discovery flows.

  #### Visitor Daily Usage Limit
  Added detection for the visitor daily read quota (100 req/day, error code `400323`). When a visitor or bot exceeds this limit, the SDK detects the error and emits a deduplicated event via `AmityClient.getVisitorUsageLimitEvents() -> AnyPublisher<AmityVisitorUsageLimitEvent, Never>`. The session remains active so the user can still sign in to continue browsing.
  ...
  <AccordionGroup>
    <Accordion icon="gear" title="iOS SDK">
      | Config         | Version |
      | :------------- | :------ |
      | Minimum Target | 14.0    |
      | Xcode          | 26.0    |
    </Accordion>
    <Accordion icon="book" title="Dependencies">
      | Library          | Version               |
      | :--------------- | :-------------------- |
      | Alamofire        | 5.7.1                 |

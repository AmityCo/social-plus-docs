...
<Update label="7.27.1  (25/09/2026)" tags={["New Releases","Improvements"]}>
  ## 🚀 New Features

  #### Comment Approval for Communities

  A community can now require review before a new comment is visible. While a comment is pending, only its author, the community's reviewers, and network admins can see it. It stays out of public comment lists and comment counts until a reviewer approves it. See [Comment Review](/social-plus-sdk/social/content-management/comments/moderation/comment-review).

  - **Turn it on:** `isCommentReviewEnabled(true)` on `AmityCommunityCreator.Builder` or `AmityCommunityUpdate.Builder` when you create or update the community. Read it back with `AmityCommunity.isCommentReviewEnabled()`.
  - **Approval status:** `AmityComment.getApprovalStatus()` returns `AmityCommentApprovalStatus` (`PENDING`, `APPROVED`, `REJECTED`).
  - **Create without a local copy:** `.isOptimistic(false)` on the comment creator builder skips the local comment, so the comment you get back carries the server's approval status. The default, `true`, keeps today's behavior.
  - **Review queue:** `AmityCommentRepository.getCommentReviewQueue(communityId)` returns a builder for the community's pending comments. Narrow it to one post or a date range with `.post(postId)`, `.startDate()`, `.endDate()`, and `.timeZone()`.
  - **Approve or decline:** `AmityCommentRepository.approveComment(commentId)` and `declineComment(commentId, reason)`. Declining deletes the comment. The optional `reason` (at most 250 characters) is kept for the moderation record only.
  - **Permission:** reviewers need `AmityPermission.REVIEW_COMMUNITY_COMMENT`.
  - **Editing while pending:** `AmitySocialSettings.isAllowEditCommentWhenReviewingEnabled()` tells you whether an author may edit a pending comment.
  - **Realtime:** an approved comment appears as soon as it is approved, through the `comment.approved` event. A declined comment arrives as a deletion.

  #### Curated Content

  Fetch the posts selected for a topic configured in the Console, and build your own discovery surface around them. A pool is one bounded, ranked result with no pagination, and the SDK keeps the server's order. See [Topic-based Content Discovery](/social-plus-sdk/social/discovery-engagement/feed/curated-content).

  - **Fetch a pool:** `AmityCoreClient.newCuratedContentRepository().getPool(topicId, limit)` returns `Single<AmityCuratedContentPool>`. `limit` must be between 1 and 100.
  - **Analytics:** `AmityCuratedContentTopic.analytics()` provides `markAsViewed()`, `markPostAsViewed(postId)`, and `markPostClick(postId)`.

  #### Pin Message in Live Channels

  Pin one message in a live channel so it stays visible above the chat. Pinning a new message replaces the current pin. The server clears the pin when the message is deleted or its author is banned from the channel or globally. See [Pin and Unpin Messages](/social-plus-sdk/chat/messaging-features/messages/pin-message).

  - **Pin and unpin:** `AmityMessageRepository.pinMessage(messageId)` and `unpinMessage(messageId)`.
  - **Read the pin:** `AmityChannel.getPinnedMessage()` returns the current `AmityPinnedMessage`, or `null` when nothing is pinned.
  - **Permission:** `AmityPermission.PIN_MESSAGE`. Channel moderators, and livestream hosts and co-hosts, have it by default.
  - **Realtime:** the channel `Flowable` emits on every pin and unpin, driven by the `channel.messagePinned` and `channel.messageUnpinned` events.

  ## ✨ Improvements

  - `AmityPost.getLocalCommentCount()` now takes an optional `includePending` flag. By default, the count leaves out the current user's own comments that are still awaiting approval. Pass `true` to include them.

  ## 🐞 Bug Fixes

  - Fixed `AmityRoomStatus` falling back to `IDLE` for a room terminated by moderation. A terminated room now surfaces as `AmityRoomStatus.TERMINATED`.
  - Fixed a member's channel role change (`channel.roleAdded` / `channel.roleRemoved`) not reaching the client until the app was restarted, which could leave a stale pin permission.
  ...
  <Card title="📘 API Reference — 7.27.1" icon="book-open" horizontal href="https://android-sdk.docs.amity.co/7.27.1/">
  </Card>
</Update>

<Update label="7.26.0  (11/09/2026)" tags={["New Releases","Improvements"]}>
  ## 🚀 New Features

  #### Configurable Search Fields

  Added an opt-in `searchBy` option to user search (and channel-member search) so you can restrict which fields the keyword is matched against.

  - New enum `AmitySearchUserBy` (`DISPLAY_NAME`, `PROFILE_HANDLE`, `USER_ID`) in `com.amity.socialcloud.sdk.model.core.search`.
  - New builder method `AmityUserSearch.Builder.searchBy(List<AmitySearchUserBy>)`.
  - Omitting `searchBy` keeps the existing default matching, so existing integrations are unaffected.

  #### Video Rotation & Dimensions

  Video rotation and dimensions are now exposed, and `getVideoUrl` no longer hangs.

  #### Live Viewer Count Configuration

  Added `getLiveViewerCountConfig` to `AmityCoreClient`.

  #### v4 Sessions

  `loginWithAccessToken` now verifies the access token against v4 sessions and persists the caller's user type from the login response.
  ...
  <Card title="📘 API Reference — 7.26.0" icon="book-open" horizontal href="https://android-sdk.docs.amity.co/7.26.0/">
  </Card>
</Update>

<Update label="7.25.0  (28/08/2026)" tags={["New Releases","Improvements"]}>
  ## 🚀 New Features

  #### Channel Moderation Permissions

  Added the `BAN_USER_FROM_CHANNEL` and `MUTE_USER_INSIDE_CHANNEL` permissions for per-channel moderation.

  ## ✨ Improvements

  - Cleaned up the channel ban/mute permission set — retired unused names and added missing ones.
  ...
  <Card title="📘 API Reference — 7.25.0" icon="book-open" horizontal href="https://android-sdk.docs.amity.co/7.25.0/">
  </Card>
</Update>

<Update label="7.24.4  (25/08/2026)" tags={["New Releases","Improvements"]}>
  ## 🚀 New Features

  #### Event Posts

  Added event-post support: an event shared to a community feed is delivered as a post (post `dataType` `event`), resolves through `EventRepository`, and renders alongside regular posts. `event.isOriginPublic` is exposed for visibility handling.

  ## 🐞 Bug Fixes

  - Fixed locally-created chat messages being assigned the wrong segment and missing their channel id.
  - Fixed a failed send incorrectly determining the next message segment.
  - Fixed the sender's own message not being marked read on some paths.
  - Removed unused RTMP dependencies that broke 16 KB page-size alignment.
  ...
  <Card title="📘 API Reference — 7.24.4" icon="book-open" horizontal href="https://android-sdk.docs.amity.co/7.24.4/">
  </Card>
</Update>

<Update label="7.24.3  (18/08/2026)" tags={["Improvements"]}>
  ## 🐞 Bug Fixes

  - Improved real-time (MQTT) connection resilience: a transient failure on the first connect no longer stops real-time, and logout / token expiry now stops a client still retrying its first connect.
  - Fixed session handling so a failed token renewal is recoverable and the session stays alive through renewal.
  - Fixed chat-list message previews: previews now persist in step with their channel rows and fall back to the channel's current row when a snapshot is missing.
  ...
  <Card title="📘 API Reference — 7.24.3" icon="book-open" horizontal href="https://android-sdk.docs.amity.co/7.24.3/">
  </Card>
</Update>

<Update label="7.24.2  (14/08/2026)" tags={["Improvements"]}>
  ## 🐞 Bug Fixes

  - Fixed channel unread tracking to mark read against the channel id rather than the sub-channel id.
  - Fixed the local read position being lost when `membership.readToSegment` is stale.
  - Excluded left and deleted channels from the channel unread total.
  ...
  <Card title="📘 API Reference — 7.24.2" icon="book-open" horizontal href="https://android-sdk.docs.amity.co/7.24.2/">
  </Card>
</Update>

<Update label="7.24.0  (31/07/2026)" tags={["New Releases","Improvements"]}>
  ## 🚀 New Features

  #### Report User with Reason

  Added the ability to report a user with a report type, behavior reason, and comment, plus report/unflag actions on the user list.

  #### Exclude Own Events

  Added an `excludeOwnEvents` parameter to event queries so the current user's own events can be filtered out.

  #### Event Target Community Avatar

  Added the community avatar to an event's target community.

  ## ✨ Improvements

  - Migrated the MQTT client from protocol v3 to v5.
  - Removed the transitive `AD_ID` permission from the FCM push module.
  ...
  <Card title="📘 API Reference — 7.24.0" icon="book-open" horizontal href="https://android-sdk.docs.amity.co/7.24.0/">
  </Card>
</Update>

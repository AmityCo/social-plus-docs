...
<Update label="7.27.0  (25/09/2026)" tags={["New Releases","Improvements"]}>
  ## 🚀 New Features
  ...
  - **Turn it on:** from the Console. This release does not expose the community setting in the Android SDK.
  - **Approval status:** `AmityComment.getApprovalStatus()` returns `AmityCommentApprovalStatus` (`PENDING`, `APPROVED`, `REJECTED`).
  - **Review queue:** `AmityCommentRepository.getCommentReviewQueue(communityId)` returns a builder for the community's pending comments. Narrow it to one post with `.post(postId)`.
  ...
  <Card title="📘 API Reference — 7.27.0" icon="book-open" horizontal href="https://android-sdk.docs.amity.co/7.27.0/">
  </Card>
</Update>

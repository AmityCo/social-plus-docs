...
<Update label="7.16.0  (27/02/2026)" tags={["New Releases"]}>
  ...
  Add `excludingRoles` parameter to `AmityCommunityMembershipQuery.Builder` and `AmityCommunityMemberSearch.Builder`. Members holding any of the specified roles are excluded from the live collection result. This is a local-only filter — not sent to the backend.

  #### Time-Bounded Post Pagination
  Add `untilAt` parameter to `getPosts`, `getCommunityFeed`, and `getUserFeed` query builders. When provided, the SDK stops fetching and collecting posts beyond the given datetime boundary. This is a local-only filter — not sent to the backend.

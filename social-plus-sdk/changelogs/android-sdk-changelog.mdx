...
<Update label="7.27.0  (25/09/2026)" tags={["New Releases","Improvements"]}>
  ...
  Community moderators can now hold comments for review before they become public. A comment made in a review-enabled community is created with a `pending` status, visible only to its author, community reviewers and network admins, until a reviewer approves or declines it.

  - New enum `AmityCommentApprovalStatus` (`PENDING`, `APPROVED`, `REJECTED`) with `AmityComment.getApprovalStatus()`.
  - New permission `AmityPermission.REVIEW_COMMUNITY_COMMENT`.
  - `AmityCommentRepository.getCommentReviewQueue(communityId)` returns a builder for a paged queue of pending comments across the community. Narrow it to one post with `.post(postId)`.
  - `AmityCommentRepository.approveComment(commentId)` and `declineComment(commentId, reason)` resolve a pending comment.
  - Turn comment review on for a community from the Console. This release does not expose the community setting in the Android SDK.
  - `AmitySocialSettings.isAllowEditCommentWhenReviewingEnabled()` reports the network's edit-while-reviewing setting.
  - New realtime event `comment.approved` reveals an approved comment as soon as a reviewer acts, without waiting for a refresh.
  - A pending comment is excluded from the public comment list, the post's latest-comment previews, and the comment count until it is approved.

  #### Curated Content

  Read a moderated pool of posts for a content-widget placement, plus its view/click analytics.

  - `AmityCoreClient.newCuratedContentRepository().getPool(topicId, limit)` returns a ranked `AmityCuratedContentPool` for a topic; the server applies safety filtering and the viewer's block list, and the SDK preserves the returned order.
  - `AmityCuratedContentTopic.analytics()` exposes widget-view, post-view and post-link-click tracking.

  #### Pin Message in Live Channels

  A channel member with permission can now pin a message so it stays visible above the conversation.

  - `AmityMessageRepository.pinMessage(messageId)` / `unpinMessage(messageId)`, gated by the new `AmityPermission.PIN_MESSAGE`.
  - `AmityChannel.getPinnedMessage()` returns the current `AmityPinnedMessage`, if any.
  - The channel's `Flowable` emits immediately on pin/unpin, and also reacts to the realtime `channel.messagePinned` / `channel.messageUnpinned` events.
  ...
  - Fixed a channel permission change (a role granted or revoked via `channel.roleAdded` / `channel.roleRemoved`) not reaching the client until the app was restarted.

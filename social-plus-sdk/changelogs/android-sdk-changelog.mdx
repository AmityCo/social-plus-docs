...
<Update label="7.22.0  (05/06/2026)" tags={["New Releases", "Improvements"]}>
  ...
  Added `invalidateCache` option to `AmityCommunityFeedQuery` and post/feed live collection queries, allowing developers to force-refresh feeds by bypassing the local cache when stale data needs to be cleared.

  #### Community Tags
  Added community tags support, enabling communities to be tagged for better organization and discoverability.

  #### Visitor Limit
  Added visitor limit functionality to control content access for unauthenticated users when a limit is reached.

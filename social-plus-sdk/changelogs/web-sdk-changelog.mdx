...
<Update label="7.28.0  (25/09/2026)" tags={["New Releases","Improvements"]}>
  ...
  #### Pin Messages in Live Channels

  Added the ability to pin and unpin messages in live (livestream) channels. `MessageRepository.pinMessage` / `unpinMessage` set or clear a channel's single pinned message — pinning a new message replaces the current pin in one step, no unpin required first. The pin is exposed on `channel.pinnedMessage`, gated by the `PinMessagePermission` permission, and kept in sync in real time. `ChannelRepository.onChannelMessagePinned` / `onChannelMessageUnpinned` are also available as callbacks.

  #### Comment Approval in Communities

  Added moderation-gated comment approval for communities. When a community's `needApprovalOnCommentCreation` setting is enabled, new comments carry an `approvalStatus` and are held out of public collections and post comment counts until approved (the author can still see their own pending comment). Reviewers read the queue with `CommentRepository.getCommentReviewQueue` and act with `CommentRepository.approveComment` / `declineComment`. Subscribe to approvals with `CommentRepository.onCommentApproved`. Gated by the new `ReviewCommunityCommentPermission` permission.

  #### Local Comment Count

  Added `post.getLocalCommentCount(includePending)` to read a post's comment count with the current user's own optimistic, not-yet-confirmed comments factored in, without ever double-counting or losing a comment once the server confirms it. The existing `post.localCommentCount` field is unchanged but now deprecated in favor of the new method.

  #### Curated Content

  Added `CuratedContentRepository.getPool(topicId, limit?)`, returning the curated content pool for a topic: the topic plus its ranked posts. Added `topic.analytics` helpers — `markAsViewed`, `markPostAsViewed`, and `markPostClick` — to record widget and post-level engagement.

  #### Local Event on Channel Creation

  `createChannel` now fires a local `channel.created` event right after a successful create, so a newly created channel appears immediately in a running `ChannelRepository.getChannels` collection instead of waiting on the first message.

  ## 🐞 Bug Fixes

  - Fixed `ChannelRepository.getChannel` not emitting an update when a member's role changed, which could leave a stale pin-message permission cached for that member.
  ...
  - Fixed a deleted message's text lingering in the channel list preview when the "don't include deleted messages" preview setting is enabled.
  - Fixed `message.creator.avatar` always resolving to `undefined` even though the type declared it as available.

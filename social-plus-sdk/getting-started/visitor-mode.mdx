...
| Error Constant | Code | Trigger |
|---|---|---|
| `VISITOR_USAGE_LIMIT_EXCEEDED` | `400323` | Visitor/bot has exhausted their daily read quota |
| `VISITOR_PERMISSION_DENIED` | `403999` | Visitor attempted a write operation |
| `BOT_PERMISSION_DENIED` | `403998` | Bot attempted a restricted operation |
...
// Subscribe to usage limit events after visitor login
const unsubscribe = onVisitorUsageLimitReached(() => {
    // Navigate to sign-in or show custom error UI
...
<Card title="UIKit: Usage Limit Page & Behavior Overrides" icon="mobile-screen" href="/uikit/getting-started/visitor-access">
UIKit subscribes to this event automatically and shows a full-screen `VisitorUsageLimitPage`. See the UIKit Visitor Access guide for the error page, `AmityGlobalBehavior` overrides, and per-platform availability.
</Card>
...
<Accordion title="Usage limit error page not showing (or showing when it shouldn't)">
**Symptoms**: Visitor exceeds quota but no error page appears, or signed-in users see the error page unexpectedly

**Solutions**:
1. Verify UIKit version is **4.17.0+ (Web), 4.18.0+ (Android), 4.22.0+ (iOS)** — older UIKit versions do not have the usage limit page
2. Confirm the user type is `VISITOR` or `BOT` — signed-in users never trigger this flow
3. If using a custom `handleVisitorUsageLimitReached` override, ensure it is correctly wired in `AmityGlobalBehavior`
4. Check that your override is not swallowing the event silently
5. If the error page persists after sign-in: ensure you are calling `registerDevice()` with the signed-in `userId` — this re-initializes the SDK session and resets the state
</Accordion>
</AccordionGroup>

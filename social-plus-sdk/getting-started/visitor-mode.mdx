  ...
  <Tab title="User Discovery">
    **User Listing/Search**: Hidden from results
    ...
    const unsubscribe = UserRepository.searchUsers(
      { displayName: 'John' },
      ({ data: users }) => {
        // users contains only SIGNED_IN users, no VISITOR or BOT users
      },
    );
    ```
...
<Info>
  The quota is shared across all read endpoints — feed, events, communities, user profiles, and so on — and its size is a network-level setting. The counter resets daily. Monthly overages are a billing concern only; the SDK never receives a monthly-limit error.
</Info>
...
social.plus automatically removes the data of inactive guest users, so transient visitor records don't accumulate. This runs on the backend with no integrator action required, and it supports privacy compliance (such as GDPR). Actively-returning visitors keep their data, and a visitor who signs up (converts) keeps their history.

...
Obtain `authSignature` via the API implemented in the previous step, and provide the corresponding values to the `loginAsVisitor()` function.
...
<Tabs>
<Tab title="iOS">
{/* doc-as-test: skip (UIKit behavior customization example; UIKit is outside the SDK-first audit scope) */}
```swift
// Set behavior
...
{/* doc-as-test: skip (UIKit JSX customization example; UIKit is outside the SDK-first audit scope) */}
```typescript

...
await client.login({
  ...
  authToken: 'backend-generated-token'
});
```
...
```kotlin Android
// Simple visitor login (development)
AmityCoreClient.loginAsVisitor()
    .submit()
    .doOnComplete {
...
<CodeGroup>
```swift iOS
let userType = client.getCurrentUserType()
switch userType {
case .signedIn:
...
switch (userType) {
    case AmityUserType.SIGNED_IN:
        console.log('User is authenticated');
        break;
    case AmityUserType.VISITOR:
        console.log('User is a visitor');
        break;
    case AmityUserType.BOT:
...
const users = await queryUsers({ displayName: 'John' });
...
class AmityGlobalBehavior {
  function handleVisitorUserAction() {
    // Default: Show toast "Create an account or sign in to continue."
    // Overridable for custom behavior
  }

  function handleNonMemberAction() {
    // Default: Show toast "Join community to interact."
    // Overridable for custom behavior
  }

  function handleNonFollowerAction() {
    // Default: Show toast "Follow user to interact."
    // Overridable for custom behavior
  }
}
```
...
const customBehavior = new AmityGlobalBehavior();

customBehavior.handleVisitorUserAction = () => {
  // Custom implementation
  showCustomModal({
    title: 'Join Our Community',
    message: 'Sign up to interact with posts and connect with others',
    primaryAction: () => navigateToSignUp(),
    secondaryAction: () => navigateToLogin()
  });
};

// Apply custom behavior to UIKit
AmityUIKit.setBehavior(customBehavior);
```
...
const handleVisitorAction = (action: string) => {
  ...
  if (userType === AmityUserType.VISITOR) {
    showUpgradePrompt({
      action,

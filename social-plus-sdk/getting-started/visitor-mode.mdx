...
<Warning>
**Secure Mode Requirement:** Contact social.plus support to enable secure visitor mode for your network. Once enabled, visitor logins without valid auth signatures will be rejected.
</Warning>

### Getting Your Visitor Secret

After secure mode is enabled for your network, retrieve your `SOCIAL_PLUS_APP_SECRET` from the Console:
...
Scroll to the **Visitor Secret** section (visible only after secure mode is enabled)
</Step>

<Step title="Copy Secret">
Copy your `SOCIAL_PLUS_APP_SECRET` and store it securely in your backend environment variables
</Step>
</Steps>

<Warning>
**Security Best Practice:** Never expose your `SOCIAL_PLUS_APP_SECRET` in client-side code, mobile apps, or version control. This secret must remain on your backend server only.
</Warning>

...
On Android the check is a `Flowable` and re-emits when the user's channel membership changes. On iOS the check is one-shot: re-run it when the channel, room, or the current user's channel membership updates, and after a `403`. Role grants and revokes arrive over the `channel.roleAdded` / `channel.roleRemoved` realtime events, which update the current user's channel permissions — so re-checking on a membership update reflects a mid-stream role change with no user action, and the `403` re-check is a backstop.
...
| Condition | Result |
| --- | --- |
| Caller lacks `PIN_MESSAGE` and is not an admin with the moderate-channel permission | `403` permission denied. Role changes also arrive over the `channel.roleAdded` / `channel.roleRemoved` realtime events, which update the caller's channel permissions — so a revoked role is usually reflected before a pin attempt. The TypeScript SDK additionally refreshes the caller's permissions on the `403` itself, as a backstop. |

...
<Tabs>
  <Tab title="Basic Message Querying">
    **Retrieve messages from a subchannel with optional filtering**
    ...
    token = messageRepository.getMessages(options: messageQueryOptions).observe({ collection, error in
        if let error {
            print("Error: \(error).")
            return
        }
        for message in collection.snapshots {
  ...
  <Tab title="Jump to Specific Message">
    **Navigate directly to a specific message in the conversation**
    ...
    func queryMessages(queryOptions: AmityMessageQueryOptions) {
        ...
        token = messageRepository.getMessages(options: queryOptions).observe({ collection, error in
            if let error = error {
                print("Error: \(error).")
                return
            }
    ...
    {/* doc-as-test: skip(android-ui: requires PagingDataAdapter/RecyclerView UI components) */}
    ```kotlin Android
    fun jumpToTarget(
        messageRepository: AmityMessageRepository,
        subChannelId: String,

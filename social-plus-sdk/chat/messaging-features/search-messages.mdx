...
| --- | --- | --- | --- |
| `query` | Channels, Messages | Required for message search; optional for channel search | Keyword to search for. |
| `exactMatch` | Channels, Messages | No | When `true`, match the keyword exactly rather than as a partial/fuzzy match. Defaults to `false`. |
| `types` | Channels, Messages | No | For channels, restrict to channel types (community, live, conversation, broadcast). For messages, restrict to message content types (text, image, file, video, audio, custom). |
| `isMemberOnly` / `memberOnly` | Channels | No | Limit channel results to channels the current user is a member of. |
| `tags` | Channels, Messages | No | Include only results carrying the given app-defined tags. |
| `channelId` | Messages | No | Scope message results to a single channel. |
| `messageFeedId` | Messages | No | Scope message results to a single message feed (sub-channel). |
| `userIds` | Messages | No | Restrict message results to messages sent by the given users. |
| `sortBy` | Channels, Messages | No | Sort field. Channels: `relevance` or `lastActivity`. Messages: `relevance` or `createdAt`. Defaults to `relevance`. |
| `orderBy` | Channels, Messages | No | Result order, `asc` or `desc`. Defaults to `desc`. |
| `limit` | Channels, Messages | No | Page size. Clamped to the 1–100 range where the platform exposes it; defaults to 20. |

...
| Notification mode | `notificationMode` | `setNotificationMode(_:)` on `AmityChannelUpdateOptions` | `.notificationMode(...)` on the edit builder | `.notificationMode(...)` |

## Parameters

| Parameter | Required | Platforms | Description |
| --- | --- | --- | --- |
| `channelId` | Yes | TypeScript, iOS, Android, Flutter | Channel ID to update. |
| `displayName` | No | TypeScript, iOS, Android, Flutter | Replacement channel display name. |
| `avatarFileId` / `avatar` | No | TypeScript, iOS, Android, Flutter | Replacement avatar file reference where supported. |
| `tags` | No | TypeScript, iOS, Android, Flutter | Replacement tag list for the channel. |
| `metadata` | No | TypeScript, iOS, Android, Flutter | Replacement metadata stored with the channel. |
| `notificationMode` | No | TypeScript, iOS, Android, Flutter | Per-channel notification mode: `default`, `silent`, or `subscribe`. Exposed on all four platforms. |
...
The channel update API exposes notification mode on all four platforms. The available values are `default`, `silent`, and `subscribe`. Use the dedicated [channel notification settings](../../push-notifications/settings/channel-settings) for broader notification configuration.
...
```swift iOS
let options = AmityChannelUpdateOptions(channelId: "channel-id")
options.setNotificationMode(.silent)
...
val disposable = channelRepository
    .editChannel(channelId = channelId)
    .notificationMode(AmityChannelNotificationMode.SILENT)
    .build()
    .apply()
    .subscribe(
        { channel -> showSuccessMessage(channel.getChannelId()) },

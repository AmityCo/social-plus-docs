...
Call `markAsRead()` on a channel object to reset its unread count to zero. Call this when the user opens or finishes viewing a channel.

<CodeGroup>
```swift iOS
// Mark all messages in the channel as read (async/throws)
func markChannelAsRead(channel: AmityChannel) async {
    do {
...
fun markChannelAsRead(channel: AmityChannel) {
    channel.markAsRead()
        .doOnComplete {
            Log.d("UnreadCount", "Channel marked as read")
        }
        .doOnError { error ->
            Log.e("UnreadCount", "Failed to mark channel as read", error)
        }
        .subscribe()
}
```
...
```typescript TypeScript
async function markChannelAsRead(channel: Amity.Channel): Promise<void> {
    await channel.markAsRead();
}
```

```dart Flutter
// Flutter does not currently expose a channel-level markAsRead.
// Use message.markRead() on AmityMessage objects to track read state
// at the message level — see Message Read Status docs.
```
</CodeGroup>

<Note>
**Flutter**: Channel-level `markAsRead()` is not yet part of the Flutter SDK public API. Use `message.markRead()` at the message level instead (see [Message Read Status](/social-plus-sdk/chat/engagement-features/unread-status/message-read-status)).
</Note>

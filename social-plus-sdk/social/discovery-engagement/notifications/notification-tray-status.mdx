...
<Note>
**Not real-time.** Seen status is derived from the same server-cached tray data as [notification items](./notification-items#data-freshness), so it does not update the instant a new notification is triggered. Expect a short delay before `isSeen` / `lastTrayOccurredAt` reflect a new occurrence — an unread indicator built on this status is eventually consistent, not live.
</Note>

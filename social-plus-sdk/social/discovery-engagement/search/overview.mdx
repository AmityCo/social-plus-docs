...
graph TB
    ...
    C --> D[Lexical Matching 30%]
    C --> E[Semantic Understanding 70%]
...
<Tabs>
  <Tab title="SDK Integration">
    Use the built-in SDK functions for seamless integration:
    - `semanticSearchPosts()` - Search for posts with context awareness
    - `semanticSearchCommunities()` - Find relevant communities intelligently
  </Tab>
  
  <Tab title="Direct API">
    Make direct HTTP API calls for custom implementations:
    - [Posts Search API](https://api-docs.amity.co/#/Semantic%20Search/get_api_v1_semantic_search_posts)
    - [Communities Search API](https://api-docs.amity.co/#/Semantic%20Search/get_api_v1_semantic_search_communities)
  </Tab>
</Tabs>
...
You can set minimum score thresholds to control result quality and quantity:

<CodeGroup>
  ```javascript Example: Conservative Filtering
  const searchResults = await semanticSearchPosts({
    query: "red fruits",
    minScore: 0.7  // Only highly relevant results
  });
  ```

  ```javascript Example: Broad Discovery
  const searchResults = await semanticSearchPosts({
    query: "red fruits", 
    minScore: 0.3  // Include moderately relevant results
  });
  ```
</CodeGroup>
...
<CardGroup cols={2}>
  <Card title="Search Posts" icon="file-text" href="intelligent-search-post.md">
    Learn how to implement semantic search for posts with filtering and customization options
  </Card>
  
  <Card title="Search Communities" icon="users" href="intelligent-search-community.md">

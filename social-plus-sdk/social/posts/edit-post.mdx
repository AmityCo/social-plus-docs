...
<CardGroup cols={2}>
  <Card title="Supported Post Types" icon="edit">
    - Text posts - Image posts - Video posts - File posts
  </Card>
  <Card title="Key Features" icon="sparkles">
    - Edit timestamp tracking - Permission-based editing - Media file management
    - Content validation
  </Card>
</CardGroup>
...
func editTextPost(postId: String, newTitle: String? = nil, newText: String) {
    ...
    if let title = newTitle {
        textPostBuilder.setTitle(title)
    }
...
postRepository.editPost(postId = "postId")
    ...
    .apply()
    .subscribe(
        { post -> println("Post updated: ${post.getPostId()}") },
...
```kotlin
postRepository.editPost(postId = "postId")
    .image(images = listOf(updatedImage)) // AmityImage list
    .build()
    .apply()
    .subscribe(
        { post -> println("Post updated") },
        { error -> println("Error: ${error.message}") }
    )
```
...
```kotlin
postRepository.editPost(postId = "postId")
    .file(files = listOf(updatedFile)) // AmityFile list
    .build()
    .apply()
    .subscribe(
        { post -> println("Post updated") },
...
```kotlin
postRepository.editPost(postId = "postId")
    .video(videos = listOf(updatedVideo)) // AmityVideo list
    .build()
    .apply()
    .subscribe(
        { post -> println("Post updated") },
...
const editPost = async function (
  ...
  const { data: post } = await PostRepository.editPost(postId, editPost);
...
class PostEditManager {
  Future<void> editTextPost(String postId, {String? newTitle, String? newText}) async {
    try {
      final updateBuilder = AmitySocialClient.newPostRepository()
          .updatePost(postId)
          .metadata({'editReason': 'content_update'});

      if (newText != null) {
        updateBuilder.text(newText);
      }
  ...
  Future<bool> editPostSafely(String postId, {String? newTitle, String? newText}) async {
    try {
      // Validate input
...
<CardGroup cols={2}>
  <Card title="Content Moderation" icon="gavel">
    Enable moderators to edit posts for compliance while maintaining edit
    history and transparency through timestamps.
  </Card>
  <Card title="Collaborative Editing" icon="users">
    Allow team members to collaboratively improve posts with proper permission
    controls and change tracking.
  </Card>
  <Card title="Content Updates" icon="refresh">
    Keep evergreen content current by allowing authors to update posts with new
    information while preserving engagement.
  </Card>
  <Card title="Error Correction" icon="spell-check">
    Quick fixes for typos and factual errors while maintaining the original
    post's context and replies.
  </Card>
</CardGroup>

<Note>
  **File Upload Limits**: You can upload a maximum of 10 images/files in a
  single post. Ensure your builder type matches the original post type when
  editing.
</Note>

<Warning>
  **Important**: When editing posts with media attachments, you must first
  upload new files before updating the post. The edit operation will replace
  existing attachments based on the new attachment list provided.
</Warning>

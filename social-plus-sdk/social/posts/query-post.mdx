...
| `untilAt`        | DateTime | Time boundary for pagination (local-only). Stops fetching and collecting posts beyond this datetime. | See [Time-Bounded Pagination](#time-bounded-pagination-with-untilat) |
...
<Tabs>
  ...
  <Tab title="TypeScript">
    ...
    class PostQueryManager {
      ...
      // Basic community posts query
      queryPosts(params: PostQueryParams): Amity.LiveCollection<Amity.Post> {
      ...
      async queryCommunityPosts(
        ...
        return new Promise((resolve, reject) => {
          liveCollection.once('dataUpdated', (posts: Amity.Post[]) => {
            if (options.limit) {
      ...
      ): Amity.LiveCollection<Amity.Post> {
      ...
      observePosts(
        communityId: string,
        onUpdate: (posts: Amity.Post[]) => void,

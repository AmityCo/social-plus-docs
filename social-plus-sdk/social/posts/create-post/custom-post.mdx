...
val customData = JsonObject().apply {
    addProperty("title", "Custom Post Title")
    addProperty("description", "Custom post description")
}

postRepository.createCustomPost(
    target = target,
    customDataType = "amity.custom",
    data = customData
)
    .doOnSuccess { post: AmityPost -> println("Created: ${post.getPostId()}") }
...
try {
    final post = await postRepository
        .createPost()
        .targetCommunity('flutter_jobs_community')
        .custom('custom.job', jobData)

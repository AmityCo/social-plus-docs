...
<Tab title="Android">
Create live streams and stream posts using the video client and post builder APIs.
...
<Tab title="Flutter">
Create a live stream post once you have a stream ID from Flutter's video SDK module.

<Warning>
  **Flutter SDK gap**: `AmityStreamRepository` is available on iOS/Android but is not exported from the main Flutter `amity_sdk` barrel. Tracking: `TICKET-FLUTTER-004`.
</Warning>
...
<Tab title="Android">
<Warning>
  **Android SDK gap**: `AmityException.StreamLimitReachedException` is not available in the Android SDK, and there is no equivalent `AmityError` entry yet. Tracking: `TICKET-ANDROID-001`.
</Warning>
...
fun handleStreamError(error: Throwable) {
    if (error is AmityException && error.code == AmityError.PERMISSION_DENIED.code) {
        println("No permission to create streams")
    } else {
        // Stream-limit-specific handling is pending SDK support.

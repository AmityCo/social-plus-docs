...
fun processChildPosts(children: List<AmityPost>) {
    children.forEach { childPost ->
        when (childPost.dataType) {
            ...
            'poll' -> processPollPost(childPost)
...
```typescript
// Access parent post and its children
function processPostContent(post: Amity.Post): void {
  // Parent post contains text content and metadata
...
function processChildPosts(children: Amity.Post[]): void {
  children.forEach(childPost => {
    switch (childPost.dataType) {
      case 'image':
        processImagePost(childPost);
...
class PostContentManager {
  ...
  private extractParentContent(post: Amity.Post): ParentContent {
    const textData = post.data as AmityTextPostData;
    return {
...
class PostContentProcessor extends ChangeNotifier {
  ...
  Future<ProcessedPostContent> processPost(String postId) async {
    ...
    final repository = AmitySocialClient.newPostRepository();
...
```typescript
// Basic image post processing
function processImagePost(post: Amity.Post): void {
...
class FileViewController: UIViewController {
    ...
    private let fileManager = FilePostManager(client: AmityClient(apiKey: "your-api-key", region: .sg))
...
```typescript
// Basic file post processing
function processFilePost(post: Amity.Post): void {
...
```typescript
// Basic video post processing
function processVideoPost(post: Amity.Post): void {
...
class LiveStreamViewController: UIViewController {
    ...
    private let streamManager = LiveStreamManager(client: AmityClient(apiKey: "your-api-key", region: .sg))
...
```typescript
// Basic live stream post processing
function processLiveStreamPost(post: Amity.Post): void {
...
class PollPostManager(private val client: AmityClient) {
    ...
    fun getPollFromPost(post: AmityPost): AmityPoll? {
        ...
        return post.getChildren()
            ?.firstOrNull { it.getDataType() == 'poll' }
            ?.let { pollChild ->
...
// AmityDataType not exported from TS SDK — use Amity.Post dataType string directly ('poll', 'image', etc.)
...
class PollPostManager {
    private client: any;
    private pollRepository: typeof PollRepository;
    
    constructor(client: any) {
        this.client = client;
        // PollRepository is a TS namespace (not a class); call its static methods directly
        this.pollRepository = PollRepository;
    }
    
    getPollFromPost(post: Amity.Post): Amity.Poll | null {
        // Check if post has poll child
        const pollChild = post.children?.find(child => child.dataType === 'poll');
        
        if (pollChild && pollChild.data) {
            return pollChild.data as Amity.Poll;
        }
        
        return null;
    }
    
    async displayPollPost(post: Amity.Post, container: HTMLElement): Promise<void> {
    ...
    private createPollElement(poll: Amity.Poll): HTMLElement {
    ...
    private createOptionElement(answer: any, poll: Amity.Poll): HTMLElement {
    ...
    private createStatsElement(poll: Amity.Poll): HTMLElement {
    ...
    private setupRealTimeUpdates(pollId: string, pollElement: HTMLElement): void {
        // Subscribe to real-time poll updates
        this.pollRepository.getPoll(pollId).subscribe((poll: Amity.Poll) => {
            this.updatePollElement(pollElement, poll);
        });
    }
    
    private updatePollElement(pollElement: HTMLElement, poll: Amity.Poll): void {
    ...
    private displayRegularPost(post: Amity.Post, container: HTMLElement): void {
...
class PollPostManager {
  ...
  AmityPoll? getPollFromPost(AmityPost post) {
    ...
    final pollChild = post.children?.firstWhere(
      (child) => child.dataType == 'poll',
      orElse: () => null,
    );
    
    if (pollChild?.data is AmityPoll) {
...
<Note>
For comprehensive poll creation and management features, refer to the [Polls Guide](/social-plus-sdk/core-concepts/content-handling/poll). Poll posts automatically inherit all poll features including expiration, vote limits, and moderation controls.
</Note>
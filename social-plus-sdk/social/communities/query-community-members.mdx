...
| `excludingRoles` | Array | Exclude members who hold any of the specified roles. Local-only filter — not sent to backend. | None. Available on **iOS** and **Android** only. |
...
- **Local-only filter** — `excludingRoles` is never sent to the backend API. All members are still fetched and persisted to the local cache; the exclusion only affects what the live collection emits.
- **ANY-match semantics** — A member is excluded if they hold **any** role from the exclusion list.
- **Backward compatible** — When `excludingRoles` is empty or not provided, all members are returned as before.
...
- **Real-time updates** — If a member gains or loses an excluded role in real-time, the live collection updates automatically.
...
<Tabs>
  <Tab title="iOS">
    ```swift
    // Get all community members except moderators
    let query = AmityCommunityMemberQuery()
    query.membershipFilter = [.member]
    query.excludingRoles = ["community-moderator", "channel-moderator"]
    ...
    membersToken = collection.observe { [weak self] result in
        ...
        case .initial(let members):
            ...
            print("Non-moderator members: \(members.count)")
    ...
    // Search members excluding specific roles
    let searchQuery = AmityCommunityMemberQuery()
    searchQuery.keyword = "john"
    searchQuery.excludingRoles = ["community-moderator"]

    let searchCollection = communityRepository.searchMembers(
        communityId: "community-123",
        query: searchQuery
    )
    ```
  ...
  <Tab title="Android">
    ...
    // Get all community members except moderators
    val query = AmityCommunityMemberQuery.Builder()
        .membershipFilter(listOf(AmityCommunityMembershipFilter.MEMBER))
        .excludingRoles(listOf("community-moderator", "channel-moderator"))
        .sortBy(AmityCommunityMemberSortOption.FIRST_CREATED)
        .build()

    client.community()
        .membership("community-123")
        .getMembers(query)
        .subscribe { membersList ->
            // Members list will NOT include any moderators
            val members = membersList.data
            println("Non-moderator members: ${members.size}")
        }

    // Search members excluding specific roles
    val searchQuery = AmityCommunityMemberSearch.Builder()
        .keyword("john")
        .excludingRoles(listOf("community-moderator"))
        .build()
    ...
    client.community()
        ...
        .subscribe { searchResults ->
            // Search results exclude moderators
        }
    ```
  </Tab>
</Tabs>

### Combining `roles` and `excludingRoles`

When both parameters are set, `roles` (inclusion) is applied first, then `excludingRoles` (exclusion). For example, to get all premium members who are **not** moderators:

<Tabs>
  <Tab title="iOS">
    ```swift
    let query = AmityCommunityMemberQuery()
    query.roles = ["premium"]                           // Include only premium members
    query.excludingRoles = ["community-moderator"]      // Then exclude moderators
    ```
  </Tab>
  ...
  <Tab title="Android">
    ```kotlin
    val query = AmityCommunityMemberQuery.Builder()
        .roles(listOf("premium"))                       // Include only premium members
        .excludingRoles(listOf("community-moderator"))  // Then exclude moderators
        .build()
    ```
  </Tab>
</Tabs>

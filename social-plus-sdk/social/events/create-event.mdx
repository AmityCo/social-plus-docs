...
let event = try await AmityEventRepository().createEvent(options: options)
```
...
let imageFile = try await AmityFileRepository().uploadImage(imageData!)
...
```typescript TypeScript
import { EventRepository, FileRepository, AmityEventType, AmityEventOriginType } from '@amityco/ts-sdk';

// First upload the cover image
const { data: imageFile } = await FileRepository.uploadImage(imageBlob);
...
let canCreateEvent = await AmityClient(apiKey: "your-api-key", region: .sg).hasPermission(
    .createEvent
)

// Check community permission for community events
let canCreateInCommunity = await AmityClient(apiKey: "your-api-key", region: .sg).hasPermission(
    .createEvent,
    communityId: "community-123"
)

// Check livestream event permission
let canCreateLivestream = await AmityClient(apiKey: "your-api-key", region: .sg).hasPermission(
...
try {
    const { data: event } = await EventRepository.createEvent(options);
    // Handle success
} catch (error) {

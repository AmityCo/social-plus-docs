...
| `isOriginPublic` | For a community-origin event, whether the origin community is public. User-origin events have no origin community; use `originType` when applying the event-post target rules — see [Target Rules](../content-management/posts/creation/event-post#target-rules). Android exposes it as `isOriginPublic()`. |
...
<CardGroup cols={3}>
  ...
  <Card title="Publish as Post" href="../content-management/posts/creation/event-post" icon="calendar-star">
    Attach an event to a post so it renders as an event card in feeds.
  </Card>

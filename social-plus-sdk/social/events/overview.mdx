...
| `isOriginPublic` | Whether the origin community is public. Combined with `originType`, this is the signal that determines where the event may be published as a post — see [Target Rules](../content-management/posts/creation/event-post#target-rules). Android exposes it as `isOriginPublic()`. |

...
try {
    ...
    final rsvp = await event.createRSVP(status: AmityEventResponseStatus.GOING);
    print("RSVP created: ${rsvp.status}");
} catch (error) {
    print("Failed to RSVP: $error");
}
...
```dart Flutter
final event = await AmityEventRepository().getEvent(id: "event-123");
final rsvp = await event.createRSVP(status: AmityEventResponseStatus.INTERESTED);
```
</CodeGroup>
...
```dart Flutter
final event = await AmityEventRepository().getEvent(id: "event-123");
final rsvp = await event.createRSVP(status: AmityEventResponseStatus.NOT_GOING);
```
</CodeGroup>
...
try {
    final event = await AmityEventRepository().getEvent(id: "event-123");
    final updatedRSVP = await event.updateRSVP(status: AmityEventResponseStatus.NOT_GOING);
    print("RSVP updated to: ${updatedRSVP.status}");
} catch (error) {
    print("Failed to update RSVP: $error");
}
...
try {
    final event = await AmityEventRepository().getEvent(id: "event-123");
    final myRSVP = await event.getMyRSVP();
...
```dart Flutter
final event = await AmityEventRepository().getEvent(id: "event-123");
final attendees = event.getRSVPs(status: AmityEventResponseStatus.GOING);

attendees.listen((responses) {
    for (var rsvp in responses) {
        print("Attendee: ${rsvp.user.displayName}");
        print("Responded at: ${rsvp.respondedAt}");
    }
    print("Total going: ${responses.length}");
});
```
</CodeGroup>
...
```dart Flutter
final event = await AmityEventRepository().getEvent(id: "event-123");
final interested = event.getRSVPs(status: AmityEventResponseStatus.INTERESTED);

interested.listen((responses) {
    print("Interested count: ${responses.length}");
    for (var rsvp in responses) {
        print("- ${rsvp.user.displayName}");
    }
});
...
```dart Flutter
final event = await AmityEventRepository().getEvent(id: "event-123");
final notGoing = event.getRSVPs(status: AmityEventResponseStatus.NOT_GOING);

notGoing.listen((responses) {
    print("Not going count: ${responses.length}");
});
```
</CodeGroup>
...
```dart Flutter
try {
    await event.createRSVP(status: AmityEventResponseStatus.GOING);
} catch (error) {
    if (error is AmityAlreadyExistsException) {
        // Use update instead
        await event.updateRSVP(status: AmityEventResponseStatus.GOING);
    } else if (error is AmityEventEndedException) {
        showError("Cannot RSVP to ended event");
    } else if (error is AmityEventCancelledException) {
        showError("This event has been cancelled");
    } else {
        showError("Failed to RSVP: $error");
    }
}
```
</CodeGroup>

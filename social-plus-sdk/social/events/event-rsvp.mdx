...
const unsubscribe = EventRepository.getEvent(
    ...
    async ({ data: event, loading, error }) => {
        if (event && !loading) {
            const rsvp = await event.createRSVP(AmityEventResponseStatus.NotGoing);
            console.log("RSVP created:", rsvp?.status);
        }
    }
);
...
const unsubscribe = EventRepository.getEvent(
    ...
    async ({ data: event, loading, error }) => {
        if (event && !loading) {
            try {
                const updatedRSVP = await event.updateRSVP(AmityEventResponseStatus.NotGoing);
...
// Call unsubscribe() and rsvpUnsubscribe() when done to clean up
```
</CodeGroup>
...
const unsubscribe = EventRepository.getEvent(
    ...
    ({ data: event, loading, error }) => {
        if (event && !loading) {
            rsvpUnsubscribe = event.getRSVPs(
                { status: AmityEventResponseStatus.NotGoing },
                ({ data: responses, loading, error }) => {
                    if (responses) {
...
| Event reminder | All "Going" users notified |

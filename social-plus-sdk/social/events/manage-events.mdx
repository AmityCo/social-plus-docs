...
| `excludeOwnEvents` | Query | TypeScript | Exclude events created by the current user — useful for discovery feeds. Defaults to `false`. |
...
For discovery surfaces such as an **Explore** or **Happening now** feed, pass `excludeOwnEvents: true` to omit events created by the current user. The flag applies to the TypeScript `getEvents` query and is a no-op when no authenticated user is resolved. It composes with the other filters, so you can combine it with `originType`, `status`, and sorting.

```typescript TypeScript
import { AmityEventStatus, EventRepository } from '@amityco/ts-sdk';
...
| Exclude own events | `excludeOwnEvents` | — | — |

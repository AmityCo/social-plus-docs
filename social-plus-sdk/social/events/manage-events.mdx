...
```dart Flutter
final eventLiveObject = AmityEventRepository().getEvent(id: "event-123");

eventLiveObject.listen((event) {
    print("Event: ${event.title}");
    print("Status: ${event.status}");
    print("RSVP Count: ${event.rsvpCount}");
});
...
```dart Flutter
final eventsCollection = AmityEventRepository().getEvents(
    builder: AmityEventQueryBuilder()
        .originType(AmityEventOriginType.COMMUNITY)
        .originId("community-123")
        .sortBy(AmityEventSortOption.START_TIME)
...
eventsCollection.listen((events) {
...
```dart Flutter
final upcomingEvents = AmityEventRepository().getEvents(
    builder: AmityEventQueryBuilder()
        .originType(AmityEventOriginType.COMMUNITY)
        .originId("community-123")
        .status(AmityEventStatus.SCHEDULED)
...
```dart Flutter
final myEvents = AmityEventRepository().getEvents(
    builder: AmityEventQueryBuilder()
        .userId("user-123")
        .onlyAttendee(true)
        .sortBy(AmityEventSortOption.START_TIME)
...
```dart Flutter
final virtualEvents = AmityEventRepository().getEvents(
    builder: AmityEventQueryBuilder()
        .originType(AmityEventOriginType.COMMUNITY)
        .originId("community-123")
        .type(AmityEventType.VIRTUAL)
...
```dart Flutter
try {
    final updatedEvent = await AmityEventRepository().updateEvent(
        id: "event-123",
        options: AmityEventUpdateOptions()
            .title("Updated Event Title")
            .description("Updated description with new details")
...
```dart Flutter
// Upload new image
final newImageFile = await AmityFileRepository().uploadImage(newImage);
...
```dart Flutter
try {
    await AmityEventRepository().deleteEvent(id: "event-123");
    print("Event deleted successfully");
} catch (error) {
    print("Failed to delete event: $error");
}
...
```dart Flutter
// Check if user can manage community events
final canManage = await AmityClient().hasPermission(
    AmityPermission.MANAGE_COMMUNITY_EVENTS,
    communityId: event.originId
);

// Check if user can delete any event
final canDelete = await AmityClient().hasPermission(
    AmityPermission.DELETE_EVENT
);

// Check if user is event creator
final isCreator = event.userId == AmityClient().currentUserId;
```
</CodeGroup>
...
```dart Flutter
try {
    await AmityEventRepository().deleteEvent(id: eventId);
} catch (error) {

...
| `excludeOwnEvents` | Query | TypeScript, Android | Exclude events created by the current user — useful for discovery feeds. Defaults to `false`. |
...
const unsubscribe = EventRepository.getEvents(
  {
    ...
    excludeOwnEvents: true, // Exclude the current user's own events from the Explore, Happening, and Recommended surfaces
  },
...
AmitySocialClient.newEventRepository()
    ...
    .orderBy(AmityEventOrderOption.ASCENDING)
    .excludeOwnEvents(true) // For showing on happening event and recommended event
    .build()
    .query()
    .subscribe(
...
| Exclude own events | `excludeOwnEvents` | — | `.excludeOwnEvents(...)` |

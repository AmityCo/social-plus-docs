...
<CodeGroup>
```swift iOS
let eventObject = AmityEventRepository().getEvent(id: "event-123")
...
let eventsCollection = AmityEventRepository().getEvents(options: options)
...
let upcomingEvents = AmityEventRepository().getEvents(options: options)
```
...
let myEvents = AmityEventRepository().getEvents(options: options)
```
...
let virtualEvents = AmityEventRepository().getEvents(options: options)
```
...
Task { @MainActor in
    do {
        ...
        let updatedEvent = try await AmityEventRepository().updateEvent(
            id: "event-123",
            options: options
        )
...
let newImageFile = try await AmityFileRepository().uploadImage(newImageData!)
...
let canManage = await AmityClient(apiKey: "your-api-key", region: .sg).hasPermission(
    .manageCommunityEvents,
    communityId: event.originId
)

// Check if user can delete any event
let canDelete = await AmityClient(apiKey: "your-api-key", region: .sg).hasPermission(
    .deleteEvent
)

// Check if user is event creator
let isCreator = event.userId == AmityClient(apiKey: "your-api-key", region: .sg).currentUserId
```
...
```swift iOS
do {
    try await AmityEventRepository().deleteEvent(id: eventId)

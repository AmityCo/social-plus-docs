...
<Warning>
  **TypeScript SDK gap**: `FeedRepository.getCommunityFeed` is not yet available in the TypeScript SDK. iOS, Android, and Flutter all expose it. TS `FeedRepository` only has `queryGlobalFeed`. Tracking: see `.docs-ops/evals/sdk-tickets-to-file.md`.
</Warning>
```typescript TypeScript
...
const communityPostsParam: Amity.CommunityFeedLiveCollection = {
  communityId: 'communityId',
  sortBy: 'lastCreated',
  tags: ['product']
};

// Community feed not in TS SDK — see Warning above
// const communityPostsParam_placeholder = communityPostsParam; // placeholder — community feed not yet in TS SDK

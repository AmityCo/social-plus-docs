---
title: "Post Review"
description: "Approve or decline posts in a community review queue using PostRepository.approvePost() and PostRepository.declinePost()."
---

Use `PostRepository.approvePost()` and `PostRepository.declinePost()` to move posts between the `reviewing`, `published`, and `declined` feed states. Post review is a community-level setting enabled by administrators; once enabled, all new posts require moderator approval before becoming visible.
...
token = feedRepository.getCommunityFeed(withCommunityId: "<community-id>",
                                sortBy: .firstCreated,
                                includeDeleted: true,
                                feedType: .reviewing).observe { collection, error in
    // Handle result here
}
```

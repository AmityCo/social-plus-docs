---
title: "Edit Posts"
description: "Update existing posts (text, image, video, file) using PostRepository.editPost(). Users can edit their own posts; moderators and admins can edit any community post."
---

Use `PostRepository.editPost()` (or `post.edit()` in Flutter) to update text, image, video, or file posts. Users can edit their own posts; community moderators and admins can edit any post in their community. On completion, `editedAt` is updated to reflect the change.
...
<Warning>
Upon completing an edit operation, the SDK updates the `editedAt` property to the current time. Use this timestamp in your UI to indicate that a post has been edited.
</Warning>
...
func updateTextPostExample() async throws {
  ...
  builder.setText("Updated text with https://www.amity.co")
  ...
  let links = [
    ...
    // Define text product tags with position info
    let productTags = [
        AmityTextProductTag(productId: "productId1", index: 18, length: 18)
    ]
    
    // Update a post from the builder
    do {
        let post = try await postRepository.editPost(
            withId: "<post-id>",
            builder: builder,
            metadata: nil,
            mentionees: nil,
...
async function updatePost() {
  const updatedPost = {
    data: {
      text: 'Updated text with https://www.amity.co',
    },
    links: [
      {
        index: 18,
...
func updateFilePostExample() async throws {
    guard let post = postRepository.getPost(withId: "postId").snapshot else { return }
...
func updateImagePostExample() {
    ...
    Task { @MainActor in
        do {
            ...
            // Attachment product tags: map fileId to media product tags
            ...
            let _ = try await postRepository.editPost(
                withId: "postId",
...
async function updatePost() {
  ...
  const updatedPost = {
    ...
    attachments: [
      { type: PostContentType.IMAGE, fileId: 'image-file-id-1' },
      { type: PostContentType.IMAGE, fileId: 'image-file-id-2' },
    ],
    attachmentTags,
    tags: ['tag1', 'tag2'],
  };
...
async function updateVideoPost() {
  // Build attachment product tags
  const attachmentTags = new AmityAttachmentProductTags();
  attachmentTags.add("video-file-id-1", { productId: "product-1" });
  attachmentTags.add("video-file-id-2", { productId: "product-2" });
  ...
  const updatedPost = {
    ...
    attachments: [
      { type: PostContentType.VIDEO, fileId: 'video-file-id-1' },
      { type: PostContentType.VIDEO, fileId: 'video-file-id-2' },
    ],
    attachmentTags,
    tags: ['tag1', 'tag2'],
  };

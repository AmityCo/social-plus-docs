---
title: "Audio Posts"
description: "Create posts with audio files. Supports MP3, WAV, AAC, M4A, OGG, Opus, FLAC. Up to 10 files, 1GB each, 1 hour max duration."
---

Use `PostRepository.createPost()` (TypeScript) or the platform post repository to attach audio files to a post after uploading them. Each audio file becomes a child post that users can react to individually. Supports up to 10 files, 1GB each, with a 1-hour duration limit.
...
<Warning>
Audio files must be uploaded first before creating the post. See [Audio Handling](/social-plus-sdk/core-concepts/content-handling/files-images-and-videos/file) for upload instructions.
</Warning>
...
<CardGroup cols={3}>
  <Card title="Audio Upload" href="/social-plus-sdk/core-concepts/content-handling/files-images-and-videos/file" icon="upload">

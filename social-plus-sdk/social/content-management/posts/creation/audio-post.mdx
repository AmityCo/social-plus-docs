...
| `productTags` | No | Products named in the post text. iOS only. |
| `attachmentProductTags` | No | Products shown on an audio attachment. An `AmityAttachmentProductTags` container keyed by `fileId`. TypeScript and iOS only. |

<Note>
**Product tags.** iOS accepts both inputs on `createAudioPost`. TypeScript accepts `attachmentProductTags` only. The Android `createAudioPost` accepts neither: create the post first, then add tags with `editPost(postId).taggedProducts(...)`. See [Edit Post](../moderation/edit-post). For tagging limits and tap-through behavior, see [Product Tagging](/uikit/components/social/posts#product-tagging).
</Note>

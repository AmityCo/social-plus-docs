---
title: "Custom Posts"
description: "Create custom post types with a structured JSON payload up to 100KB. The dataType field must start with 'custom.' (e.g., 'custom.product')."
---

Use `AmityCustomPostBuilder` (iOS/Android/Flutter) or `PostRepository.createPost()` with a `dataType` field (TypeScript) to post structured JSON data for custom content types such as product listings, events, or real estate entries. The `dataType` must follow the pattern `custom.*` and the payload must be under 100KB.
...
|-----------|------|----------|-------------|
| `dataType` | String | ✅ | Custom type identifier (must start with "custom") |
| `data` | Object | ✅ | Free-form JSON object with your custom structure |
...
<CardGroup cols={3}>
  ...
  <Card title="Post Management" href="../moderation/edit-post" icon="settings">

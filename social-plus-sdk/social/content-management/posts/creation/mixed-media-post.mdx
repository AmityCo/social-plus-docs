...
| --- | --- | --- | --- |
| `attachments`, uploaded media sets, or file IDs | Yes | TypeScript, iOS, Android | Uploaded media items to combine in one post. Supported attachment types depend on SDK. |
| `text` | No | TypeScript, iOS, Android | Caption text shown with the mixed media post. |
...
| `productTags` | No | iOS | Products named in the parent post text. Text product tags. |
| `attachmentProductTags` | Yes on TypeScript | TypeScript, iOS | Products shown on child attachments. An `AmityAttachmentProductTags` container keyed by `fileId`. |

<Note>
**Product tags (SDK API).** A mixed-media post has a text parent and one child per attachment. iOS accepts both inputs on `createMixedMediaPost`, both optional. TypeScript accepts `attachmentProductTags` for the children only, and its type declares the field as required: pass an empty `new AmityAttachmentProductTags()` when the post has no product tags. The public Android and Flutter mixed-media creators expose neither field, so this page does not promise a post-creation workaround. UIKit's product-tag UI is a separate surface and may support fewer post types; see [UIKit Product Tagging](/uikit/components/social/product-tagging). For tagging limits and tap-through behavior, see [Product Tagging](/uikit/components/social/posts#product-tagging).
</Note>
...
Mixed-structure posts can still be **read** from Flutter: pass `includeMixedStructure: true` to [`semanticSearchPosts()`](/social-plus-sdk/social/discovery-engagement/search/intelligent-search-post). The limitation is on creation only.

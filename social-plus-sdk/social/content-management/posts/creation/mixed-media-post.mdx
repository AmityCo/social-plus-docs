...
| `productTags` | No | iOS | Products named in the post text. Text product tags. |
| `attachmentProductTags` | Yes on TypeScript | TypeScript, iOS | Products shown on individual attachments. An `AmityAttachmentProductTags` container keyed by `fileId`. |

<Note>
**Product tags.** iOS accepts both inputs on `createMixedMediaPost`, both optional. TypeScript accepts `attachmentProductTags` only, and its type declares the field as required: pass an empty `new AmityAttachmentProductTags()` when the post has no product tags. The Android `createMixedAttachmentPost` accepts neither: create the post first, then add tags with `editPost(postId).taggedProducts(...)`. See [Edit Post](../moderation/edit-post). For tagging limits and tap-through behavior, see [Product Tagging](/uikit/components/social/posts#product-tagging).
</Note>

...
| `attachmentProductTags` | No | TypeScript, Android, iOS | Products tagged directly on the video attachments (media product tags). |

<Note>
**Product tags.** `productTags` tags products mentioned in the post **text**, while `attachmentProductTags` tags products directly on individual **media attachments**. Both are exposed on TypeScript, iOS, and Android; the Flutter post builder does not expose them. For tagging limits and tap-through behavior, see [Product Tagging](/uikit/components/social/product-tagging).
</Note>

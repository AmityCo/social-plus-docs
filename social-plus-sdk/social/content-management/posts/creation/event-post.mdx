...
There are two complementary accessors: a **snapshot** for first paint, and a **live observer** to keep the card in sync.

| Accessor | Role | Availability |
| --- | --- | --- |
| `post.getEventInfo()` | Synchronous snapshot from the sibling `events[]` collection cached with the post response. No network call, no subscription. Not a live observable. | 🚧 Planned — not yet implemented on any platform. Android will surface it via the sealed `AmityPost.Data.EVENT` variant rather than a top-level method. |
...
Until the snapshot accessor lands, resolve the event through the repository alone. Read the `eventId` from the **child** post — on Android the parent post's data is `TEXT` and the `EVENT` data sits on the child.
...
Once shipped, `getEventInfo()` will return `nil` / `undefined` when the post is not an event post or when the event entry is not in the payload (for example, the event was deleted).

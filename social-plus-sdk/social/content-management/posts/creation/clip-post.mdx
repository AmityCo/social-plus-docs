...
| `attachmentProductTags` | No | Products shown on the clip. An `AmityAttachmentProductTags` container keyed by `fileId`. TypeScript and iOS only. |

<Note>
**Product tags.** iOS accepts both inputs on `createClipPost`. TypeScript accepts `attachmentProductTags` only. The Android `createClipPost` accepts neither: create the post first, then add tags with `editPost(postId).taggedProducts(...)`. See [Edit Post](../moderation/edit-post). For tagging limits and tap-through behavior, see [Product Tagging](/uikit/components/social/posts#product-tagging).
</Note>

---
title: "Clip Posts"
description: "Create short-form video posts. Supports videos up to 15 minutes and 2GB. Use PostRepository.createClipPost() or AmityVideoPostBuilder."
---

Use `PostRepository.createClipPost()` (TypeScript) or the `clip()` builder method (iOS/Android/Flutter) to post short-form video content. Upload the video first to obtain a file ID or clip data, then create the post targeting a user or community.
...
async function createClipPost(): Promise<Amity.Post> {
  const newPost: Parameters<typeof PostRepository.createClipPost>[0] = {
    data: {
      text: 'hello!', // optional text post
    },
    attachments: [
      {
        fileId: 'videoFileId1',
        type: PostContentType.CLIP,
        displayMode: 'fill', // optional, 'fill' or 'fit'
        isMuted: false, // optional default is false
      },
    ],
    targetType: 'user', // 'community' or 'user' depends on post target
    targetId: 'userId1', // userId or communityId
  };

  const { data: post } = await PostRepository.createClipPost(newPost);
...
<CardGroup cols={3}>
  <Card title="Video Upload" icon="upload" href="/social-plus-sdk/core-concepts/content-handling/files-images-and-videos/video-handling">

---
title: "Query Posts"
description: "Query posts by target, type, tags, feed status, and time boundary using PostRepository.getPosts(). Results are returned as live collections."
---

Use `PostRepository.getPosts()` (or `feedRepository.getCommunityFeed()`) to query posts by community or user, filtering by post type, tags, deletion status, feed type (`published`, `reviewing`, `declined`), and mixed media structure. Results are live collections that update in real time.
...
| `untilAt` | DateTime | ❌ | Time boundary for pagination (local-only). Stops fetching and collecting posts beyond this datetime. The comparison is exclusive. Available on **iOS** and **Android** only. See [Time-Bounded Pagination](#time-bounded-pagination-with-untilat). |
...
<Info>
  By default, a post query serves cached posts immediately and clears the cache _after_ the first page is fetched.
</Info>

Use `invalidateCache(true)` when you want a community or user feed to always reflect the latest server state on load. For example, after a pull-to-refresh or when returning to a feed that may have changed. The option defaults to `false`.
...
The `untilAt` parameter enables time-bounded pagination for post queries (including community feed queries via `targetType: 'community'`). When provided, the SDK stops fetching and collecting posts beyond the specified datetime boundary.

- **Automatic pagination stop** — Once the boundary is reached, no further pages are fetched and `hasNextPage` becomes `false`.
- **Real-time filtering** — New posts arriving via real-time events are also checked against the boundary.
...
<Note>
  **Limitations**: `untilAt` does not affect posts already persisted in the local database — it only controls what appears in the live collection result. When not provided, pagination behaves normally with no time boundary.
</Note>

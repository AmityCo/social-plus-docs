...
| --- | --- | --- | --- | --- |
| Query posts | Target filter | TypeScript, iOS, Android, Flutter | Context-dependent | Target type and target ID for user or community feeds. |
| Query posts | Data type filter | TypeScript, iOS, Android, Flutter | No | Post content types to include, such as image, video, file, poll, or custom. |
| Query posts | Deleted-state filter | TypeScript, iOS, Android, Flutter | No | Include or exclude deleted posts where supported. |
| Query posts | Review/feed status | TypeScript, iOS, Android, Flutter | No | Published, reviewing, or declined state filters where exposed by the SDK. |
| Query posts | `tags` | TypeScript, iOS, Android, Flutter | No | Tags to match when building tag-filtered feeds. |
| Query posts | `includeMixedStructure` | TypeScript, iOS, Android; Flutter semantic search only | No | Include mixed-structure posts alongside media-type filters where supported. |
| Query posts | `untilAt` | TypeScript, iOS, Android | No | Time boundary for pagination where supported. |
| Android cache invalidation | `invalidateCache` | Android | No | Android-only option to skip stale paging cache for a query. |
...
| Mixed media matching | `includeMixedStructure` | `includeMixedStructure` | `.includeMixedStructure()` | Not exposed on `getPosts()`; available on [`semanticSearchPosts()`](/social-plus-sdk/social/discovery-engagement/search/intelligent-search-post) |

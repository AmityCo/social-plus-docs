...
| --- | --- | --- | --- | --- |
| Inspect post content | Post object | Yes | TypeScript, iOS, Android, Flutter | Post returned from a get or query API. Use SDK helper methods or typed data accessors on that object. |
...
- Use `localCommentCount` where exposed when your UI should include replies in the visible comment count. With comment approval (iOS/TS), this is deprecated in favor of `getLocalCommentCount(includePending:)` — it equals `getLocalCommentCount(includePending: true)`; pass `includePending: false` for the shared, approved-only count.

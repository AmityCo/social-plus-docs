...
func getRoomPlaybackUrls(from post: AmityPost) {
    ...
    case .live:
        ...
        if let liveUrl = roomData.livePlaybackUrl {
            playbackUrls.append(liveUrl)
        }
    case .recorded:
        // Get recorded playback URLs after broadcast ends
        let recordedInfos = roomData.recordedData
        playbackUrls = recordedInfos.compactMap { $0.playbackUrl }
    default:
...
{/* doc-as-test: skip (truly-illustrative: present() is a UIViewController instance method — requires UIViewController subclass context, cannot compile in isolation) */}
```swift iOS
...
func playRoom(from post: AmityPost) {
    ...
    if roomData.status == .live, let liveUrl = roomData.livePlaybackUrl {
        playRoomVideo(url: liveUrl)
    } else if roomData.status == .recorded {
        if let firstRecordedUrl = roomData.recordedData.first?.playbackUrl {
...
{/* doc-as-test: skip(android-ui: requires media3/ExoPlayer not in compile classpath) */}
```kotlin Android
...
<Info>
For more information about live streaming, please refer to [Video Streaming](/social-plus-sdk/video-new/broadcasting/overview).
</Info>

<CodeGroup>

{/* doc-as-test: skip (truly-illustrative: references outer-scope 'post: AmityPost' variable — @_hasMissingDesignatedInitializers, cannot construct in type-check context) */}
```swift iOS
...
{/* doc-as-test: skip(android-ui: requires AmityVideoPlayer not in public API) */}
```kotlin Android
...
async function getStream() {
  const stream = await StreamRepository.getStreamById('streamId');

...
Posts are the primary content objects in social.plus. A post belongs to a target, such as a user feed or community feed, and can contain text, uploaded media, poll data, live or room references, or custom data depending on the SDK.
...
| Event | Supported by TypeScript, iOS, and Android. References an existing event via `data.eventId`. Reads must go through `GET /api/v5/posts` — event posts are excluded from unfiltered `v3` and `v4` results. See [Event Posts](./creation/event-post). |
...
| --- | --- |
| TypeScript | Typed values are `text`, `image`, `video`, `file`, `audio`, `event`, and `mixed`. |
| iOS | Exposes `structureType` as a string on `AmityPost`. |
| Android | Exposes `getStructureType()` with values including `TEXT`, `IMAGE`, `VIDEO`, `AUDIO`, `FILE`, `LIVESTREAM`, `POLL`, `CLIP`, `ROOM`, `EVENT`, `MIXED`, and `UNKNOWN`. |
...
<Note>
  **Deprecated with comment approval.** In the comment-approval SDKs (iOS and
  TypeScript), `localCommentCount` is deprecated in favor of
  `getLocalCommentCount(includePending:)`. The old property equals
  `getLocalCommentCount(includePending: true)` — it includes the author's own
  pending comment; pass `includePending: false` for the shared, approved-only
  count. Move new code to `getLocalCommentCount(includePending:)` and choose the
  flag explicitly. See [Comment
  Review](/social-plus-sdk/social/content-management/comments/moderation/comment-review#related-sdk-functions)
  for how the two counts behave under review.
</Note>
...
<CardGroup cols={3}>
  ...
  <Card title="Event Posts" icon="calendar-star" href="./creation/event-post">
    Publish an event as a post so it renders as a card in feeds.
  </Card>

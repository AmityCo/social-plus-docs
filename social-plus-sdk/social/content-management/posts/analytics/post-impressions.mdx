...
description: "Track post views and reach with impression analytics. Mark posts as viewed and query which users have seen a post."
---

Use `post.analytics.markAsViewed()` to record when a user sees a post, then read `impression` (total views) and `reach` (unique viewers) from the post object. You can also query the list of users who have viewed a specific post.
...
|-----------|------|----------|-------------|
| `viewedType` | AmityViewedType / string | Yes | Type of content viewed (use `.post` / `'post'` for posts) |
| `viewedId` / `viewId` | String | Yes | ID of the post to query reached users for (`viewedId` on iOS/Android/Flutter; `viewId` on TypeScript) |
...
const GetReachedUsers: FC<{ postId: string }> = ({ postId }) => {
  const [users, setUsers] = useState<(Amity.User | undefined)[]>([]);
  const [onNextPage, setOnNextPage] = useState<(() => void) | undefined>();

  useEffect(() => {
    const unsubscribe = UserRepository.getReachedUsers(
      { viewId: postId, viewedType: 'post', limit: 10 },
      ({ data, hasNextPage, onNextPage: nextPageFn, loading }) => {
        if (!loading) {
          setUsers(data);
          setOnNextPage(hasNextPage ? () => nextPageFn : undefined);
        }
      },
    );

...
<Warning>
  **TypeScript SDK gap**: `UserRepository.getViewedUsers` is not yet available in the TypeScript SDK. Android and Flutter expose this for tracking post viewers. Tracking: see `.docs-ops/evals/sdk-tickets-to-file.md`.
</Warning>
```typescript TypeScript
...
const GetViewedUsers: FC<{ postId: string }> = ({ postId }) => {
  ...
  useEffect(() => {
    ...
    // FIXME: getViewedUsers not in TS SDK (Android/Flutter only) — see SDK gap ticket
    // (method not available in TS SDK — see Warning above)

...
description: "Retrieve one or more story targets using StoryRepository.getStoryTarget() or StoryRepository.getStoryTargets(). Use hasUnseen() to show story ring indicators."
---

Use `StoryRepository.getStoryTarget(targetType, targetId)` to observe a single community story target as a live object, or `StoryRepository.getStoryTargets(targets)` to observe up to 10 targets simultaneously. Check `target.hasUnseen()` to drive story ring indicators in your UI.

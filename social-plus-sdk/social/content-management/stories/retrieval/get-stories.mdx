---
title: "Get Stories"
description: "Retrieve stories by ID, active stories for a target, or stories across multiple targets using StoryRepository methods. Results are live objects."
---

Use `StoryRepository.getStory(storyId)` for a single story, `StoryRepository.getActiveStoriesByTarget()` for a community's current stories, or `StoryRepository.getStoriesByTargets()` to fetch stories from up to 10 targets simultaneously. All results are live objects or live collections that update automatically.

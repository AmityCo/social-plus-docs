...
description: "Track story views and reach using story.analytics.markAsSeen(). Stories must be in SYNCED state before analytics calls. Link clicks are tracked separately."
---

Use `story.analytics.markAsSeen()` to record a view impression, and `story.analytics.markLinkAsClicked()` to track hyperlink clicks. Both calls require the story to be in `SYNCED` state. Query users who viewed a story with `UserRepository.getReachedUsers(viewedType: .STORY, viewedId: storyId)`.

---
title: "Create Story"
description: "Create image or video stories in a community using StoryRepository.createImageStory() or StoryRepository.createVideoStory(). Stories support hyperlinks and custom metadata."
---

Use `StoryRepository.createImageStory()` or `StoryRepository.createVideoStory()` to publish ephemeral content to a community target. Image stories support files up to 1GB; video stories support files up to 2GB with a 90-second duration limit. Add interactive hyperlinks via `storyItems` and track sync state (`SYNCING`, `SYNCED`, `FAILED`) for upload feedback.

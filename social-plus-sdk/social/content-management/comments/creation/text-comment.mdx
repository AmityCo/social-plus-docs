...
```dart Flutter
The current Flutter comment examples in this documentation do not expose a documented `links` field on `AmityCommentCreationData` yet.

Use the standard text comment API for now, and add a Flutter code example here once the SDK surface is confirmed.
```
</CodeGroup>

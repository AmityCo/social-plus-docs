...
<Note>
`links` is optional. If omitted, SDKs do not send the `links` field. For update APIs, passing an empty array clears links while `nil`/`null` keeps existing links unchanged.
</Note>
...
### Read Links from a Comment

Use the comment model to access parsed links returned by the server.

<CodeGroup>
```swift iOS
// AmityComment.links returns [AmityLink] and defaults to [] when no links exist.
let links = comment.links
```

```kotlin Android
// getLinks() returns List<AmityLink>?; null means no links field in payload.
val links = comment.getLinks()
```

```typescript TypeScript
// links is optional on comment objects.
...
You can also include `links` in replies using the same payload shape as top-level comments.
...
```dart Flutter
void createComment(String postId, String commentId) {
  AmitySocialClient.newCommentRepository()
      .createComment()

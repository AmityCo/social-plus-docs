...
<Note>
  In a community that requires comment review, the created comment comes back with a `pending` approval status: only its author and the community's reviewers can see it, and it does not move the post's comment count for anyone else until it is approved. See [Approval Lifecycle](../moderation/comment-review#approval-lifecycle) for when a comment is held and who is exempt. Where review may apply, create with optimistic creation turned off so the returned comment carries the server's approval status — see [Read Approval Status on a Comment](../moderation/comment-review#read-approval-status-on-a-comment).
</Note>

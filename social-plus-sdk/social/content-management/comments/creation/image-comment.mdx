---
title: "Image Comment"
description: "Create image comments by uploading files first to get a fileId, then passing it to createComment(). Supports up to 10 images per comment."
---

Create image comments by uploading image files first to obtain a `fileId`, then passing it to `createComment()`. Image comments use the same optimistic update mechanism as text comments: the comment appears immediately in the UI while the SDK handles the upload and creation in the background.
...
void createComment(String postId) {
  AmitySocialClient.newCommentRepository()
      .createComment()
      .post(postId) // or .story(storyId)
      .create()
      .attachments([CommentImageAttachment(fileId: 'imageFileId')])

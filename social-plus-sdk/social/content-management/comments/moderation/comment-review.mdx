...
Where review may apply, create comments with optimistic creation turned off. An optimistic create writes a local comment before the server answers, and that comment's `approvalStatus` is only a local guess until the server decides. With optimistic creation off, nothing is written locally until the server confirms the comment, so the comment you receive carries the server's status.
...
| Create a comment or reply | Comes back `pending` where review applies. Create with optimistic creation off, so a local comment made on a guess cannot appear in a collection before the server decides. A reply to a pending comment is refused. | [Create Comment](/social-plus-sdk/social/content-management/comments/creation/create-comment) · [Read Approval Status on a Comment](#read-approval-status-on-a-comment) |

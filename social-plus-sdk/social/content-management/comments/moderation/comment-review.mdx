...
| Android | Not available yet. Android always writes a local comment first, and that local comment is treated as approved until the server answers, so in a community that requires review it can briefly appear in the author's comment list. |
...
<Note>
  ...
  that off. Before you show an edit control on a pending comment, read
  `isAllowEditCommentWhenReviewingEnabled` from the social settings:
  `AmitySocialClient.getSettings()` on Android, `client.getSocialSettings()` on
  iOS, and `Client.getSocialSettings()` on TypeScript. When it is `false`,
  editing a pending comment fails with `403` / `400300`.
</Note>
...
| Create a comment or reply | Comes back `pending` where review applies. On TypeScript and iOS, create with optimistic creation off, so a local comment made on the SDK's cached guess cannot appear in a collection before the server decides. Android has no such option yet. A reply to a pending comment is refused. | [Create Comment](/social-plus-sdk/social/content-management/comments/creation/create-comment) · [Read Approval Status on a Comment](#read-approval-status-on-a-comment) |

...
| -------- | ------------ | ----------- | ---------- | ------------- |
| Pending  | `"pending"`  | `.pending`  | `PENDING`  | Held for review. Its author, the community's reviewers, and network admins can fetch it by ID; everyone else gets `404`. In comment lists, only its author sees it; reviewers find it in the [review queue](#query-the-review-queue). It accepts no replies, reactions, or flags. |
...
| Who sees it | Everyone | No one: the comment is deleted | Author, reviewers, and network admins only |

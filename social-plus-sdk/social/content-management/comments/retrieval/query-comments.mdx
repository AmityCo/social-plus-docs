...
<Note>
  In a community that requires comment review, the SDKs that ship comment review leave pending comments out of comment queries, including the author's own, until a reviewer approves them. See [Comment Queries Show Approved Comments Only](../moderation/comment-review#comment-queries-show-approved-comments-only).
</Note>

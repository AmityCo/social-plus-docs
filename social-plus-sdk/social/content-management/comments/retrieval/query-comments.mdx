...
| **`sortBy`** | Enum | Sort order for results. Options: `firstCreated` (ascending/oldest first) or `lastCreated` (descending/newest first). Flutter uses `.sortBy(AmityCommentSortOption)`, iOS uses `orderBy` in `AmityCommentQueryOptions`. Android and TypeScript do not expose this parameter — ordering is determined by the SDK internally. |

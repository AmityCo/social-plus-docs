...
description: "Query comments by reference ID and type. Filter by parentId, deleted status, and sort order. Returns a Live Collection with real-time updates."
---

Use `CommentRepository.getComments()` to retrieve comments for a post, story, or custom content item. Pass a `parentId` to filter by thread, set `includeDeleted` for moderation views, and choose sort order. Results are returned as Live Collections that update automatically when comments change.
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="heart"
    href="/social-plus-sdk/social/content-management/comments/overview"
  >

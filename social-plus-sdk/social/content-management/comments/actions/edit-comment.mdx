...
<Warning>
  In a community that requires comment review, an edit that actually changes the comment, made by anyone who cannot review the community's comments, sends it back to `pending`: it leaves public comment lists and the post's comment count until a reviewer approves it again. An edit that changes nothing leaves the comment approved. Read `approvalStatus` on the returned comment instead of assuming the edit is live — see [How the Status Changes](../moderation/comment-review#how-the-status-changes) for which edits count and who is exempt.

  On a network where `isAllowEditCommentWhenReviewingEnabled` is `false`, editing a comment that is already pending fails with `403` / `400300`.
</Warning>
...
| --- | --- | --- | --- | --- |
| Update text | `commentId` | Yes | TypeScript, iOS, Android, Flutter | Comment ID to update. |
| Update text | `text` | Yes | TypeScript, iOS, Android, Flutter | Replacement text body for the comment. |
| Update image attachments | `commentId` | Yes | TypeScript, iOS, Android, Flutter | Comment ID to update. |
| Update image attachments | Uploaded image file ID | Yes | TypeScript, iOS, Android, Flutter | Uploaded image file ID to keep on the comment. |
| Update optional fields | `metadata`, mentions | No | TypeScript, iOS, Android, Flutter | Optional editable fields. See [Update Fields](#update-fields) for each platform's builder method. |
| Update optional fields | `links` | No | TypeScript, iOS, Android | Link attachments. Not exposed on the Flutter public update builder. |
...
<CardGroup cols={2}>
  ...
  <Card title="Comment Review" href="/social-plus-sdk/social/content-management/comments/moderation/comment-review" icon="gavel">
    Approve or decline comments held for moderator review.
  </Card>
</CardGroup>

...
<Warning>
  In a community that requires comment review, an edit that actually changes the comment sends it back to `pending`: it leaves public comment lists and the post's comment count until a reviewer approves it again. An edit that changes nothing leaves the comment approved. Read `approvalStatus` on the returned comment instead of assuming the edit is live — see [Comment Review](../moderation/comment-review).

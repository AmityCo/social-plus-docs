---
title: "Edit Comment"
description: "Edit a comment's text or image attachments using updateComment(). Only the comment owner can edit. The editedAt field updates automatically."
---

Use `updateComment()` to change a comment's text or image attachments. Only the comment owner can edit their own comments. Each successful edit updates the `editedAt` timestamp automatically. Replacing attachments removes all existing ones, so include existing attachment IDs to preserve them.

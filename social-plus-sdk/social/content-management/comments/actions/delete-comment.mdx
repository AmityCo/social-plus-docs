...
description: "Delete comments using soft delete (sets isDeleted flag, preserves data) or hard delete (permanently removes data, reactions, and replies)."
---

Use `deleteComment()` to remove comments. Soft delete sets the `isDeleted` flag to `true` and preserves data for moderation and audit use. Hard delete permanently removes the comment and all associated reactions and replies.
...
<CardGroup cols={2}>
  ...
  <Card title="Comment Reactions" href="/social-plus-sdk/social/content-management/comments/overview" icon="heart">

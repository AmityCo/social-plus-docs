...
Use `UserRepository` to block or unblock another user. Blocking hides the blocked user's content from all feeds and prevents them from posting on the blocker's feed or initiating a follow.

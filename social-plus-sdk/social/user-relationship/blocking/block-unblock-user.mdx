...
<Warning>
Blocking is **mutual**: once you block someone, that user's posts and comments can be hidden for both of you through the block-aware query options `excludeBlockUserPosts` and `excludeBlockUserComments` (see [Manage Blocked Content](./manage-blocking-users)). Other surfaces — such as search results and profile visibility — depend on the feature area, so keep those screens resilient by handling errors from their APIs and refreshing relationship status after block or unblock actions.
</Warning>

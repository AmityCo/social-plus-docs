...
description: "Query and search communities by membership status, category, and sort order using getCommunities(). Returns a Live Collection."
---

Call `getCommunities()` to browse or search communities. Filter by membership status (all, member, or not-member), category ID, and keyword. Sort results by display name or creation date. Results are returned as a Live Collection that updates automatically.
...
<Tip>
Combine tag filtering with category and membership filters for precise discovery experiences — for example, find `"gaming"` communities the user has not yet joined.
</Tip>
...
func queryCommunitiesExample() {
    let queryOptions = AmityCommunityQueryOptions(
        ...
        sortBy: .lastCreated, 
        categoryId: nil,
        tags: nil,          // optional: e.g. ["gaming", "tech"]
...
// Query communities filtered by tags
func queryCommunitiesByTags() {
    let queryOptions = AmityCommunityQueryOptions(
        filter: .notMember,
        sortBy: .lastCreated,
        categoryId: nil,
    ...
    let liveCollection = communityRepository.getCommunities(with: queryOptions)
    token = liveCollection.observe { collection, _, error in
        for community in collection.allObjects() {
            print("Tagged community: \(community.displayName)")
        }
    }
}
```
...
// Query communities filtered by tags
fun queryCommunitiesByTags(communityRepository: AmityCommunityRepository) {
    communityRepository.getCommunities()
        .sortBy(sortBy = AmityCommunitySortOption.LAST_CREATED)
        .filter(filter = AmityCommunityFilter.ALL)
        ...
        .getPagingData()
        .doOnNext { pagingData: PagingData<AmityCommunity> ->
            // Handle tagged community results
        }
        .doOnError { error ->
            // Handle error
        }

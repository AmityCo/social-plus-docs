...
- An **empty list applies no exclusion**, so you can pass one unconditionally and let the caller decide.
- Combined with `roles`, inclusion is resolved first and `excludingRoles` then narrows that result. Listing the same role in both yields no members, since exclusion is applied last.

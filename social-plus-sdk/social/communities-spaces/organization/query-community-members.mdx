...
- **Local-only filter** — `excludingRoles` is never sent to the backend API. All members are still fetched and cached; the exclusion only affects what the live collection emits.

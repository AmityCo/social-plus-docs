...
|-----------|-----------|-------------|
| `communityId` | All | Community to query. |
| `roles` | All | Optional role IDs to include, such as `community-moderator`. |
| `includeDeleted` | All | Include deleted users. |
| `excludingRoles` | TypeScript, iOS, Android | Exclude members holding any role in the list. Applies to both query and search. |
| `limit` / pagination token | TypeScript, Flutter | Explicit page size. iOS and Android paginate through the live collection and `PagingData` instead. |
...
Use `excludingRoles` when you need a member list that removes users with any role in the exclusion list. It is available in TypeScript, iOS, and Android, on both the member query and the member search APIs. Flutter exposes `excludingRoles` on neither builder.

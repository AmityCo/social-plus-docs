...
description: "Query community members using getMembers(). Filter by membership status, roles, keyword, and sort order. Returns a Live Collection."
---

Use `getMembers()` to retrieve a filtered list of community members. Filter by membership status (member, banned, or all), role names, or keyword. Results update automatically via Live Collections when members join, leave, or change status.
...
<Info>
  Member queries return Live Collections that automatically update when members join, leave, change status, or receive role assignments.
</Info>
...
communityRepository
    ...
    .excludingRoles(roles = listOf("community-moderator", "channel-moderator"))
    .sortBy(sortBy = AmityCommunityMembershipSortOption.FIRST_CREATED)
    .build()
    .query()
    .doOnNext { members: PagingData<AmityCommunityMember> ->

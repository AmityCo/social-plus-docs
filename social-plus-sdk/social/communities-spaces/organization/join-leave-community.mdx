...
description: "Join or leave communities using join() and leave(). Private communities create join requests that moderators approve or reject."
---

Use `join()` to add the current user to a community and `leave()` to remove them. Public communities grant membership immediately. Private communities create a join request that a moderator must approve or reject before the user gains access.
...
<CardGroup cols={2}>
  ...
  <Card title="Leave Communities" icon="user-minus">
    Remove users from communities and revoke access
  </Card>

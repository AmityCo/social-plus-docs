...
| `needApprovalOnCommentCreation` / `setNeedApprovalOnCommentCreation(_:)` / `isCommentReviewEnabled` | TypeScript / iOS / Android | Hold members' comments for moderator review. Defaults to `false`. See [Comment Review](/social-plus-sdk/social/content-management/comments/moderation/comment-review) |
...
Comment review is a separate community setting from post review. When it is on, comments on the community's posts are held until a reviewer approves them, unless their author can review the community's comments.

| Concept | TypeScript | iOS | Android | Flutter |
|---------|------------|-----|---------|---------|
| Set on create or update | `needApprovalOnCommentCreation: true` | `setNeedApprovalOnCommentCreation(true)` | `isCommentReviewEnabled(true)` on the builder | Not exposed |
| Read on the community | `community.needApprovalOnCommentCreation` | `community.isCommentReviewEnabled` | `community.isCommentReviewEnabled()` | Not exposed |

On Flutter, turn comment review on from the Console, another SDK, or the server API. For who is held, the review queue, and approve or decline, see [Comment Review](/social-plus-sdk/social/content-management/comments/moderation/comment-review).

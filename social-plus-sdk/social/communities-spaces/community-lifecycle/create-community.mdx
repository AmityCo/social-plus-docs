...
description: "Create communities with privacy, post moderation, story settings, and custom metadata using createCommunity()."
---

import CreateCommunity from '/snippets/social/communities/create-community.mdx';

Call `createCommunity()` to set up a new community with a display name, description, avatar, category IDs, privacy mode (public or private), post moderation setting, story settings, and custom metadata.
...
<Info>
**Note:** Communities can be created as public (discoverable by all) or private (invitation-only) with configurable post settings for content moderation, story interaction controls, and custom metadata.
</Info>

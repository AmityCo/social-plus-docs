...
Use the update API for your platform to change a community's display name, description, avatar, category IDs, privacy mode, post moderation setting, comment review setting, story settings, or custom metadata. TypeScript and Flutter use `updateCommunity()`, while iOS and Android use `editCommunity`.
...
| `needApprovalOnCommentCreation` / `setNeedApprovalOnCommentCreation(_:)` / `isCommentReviewEnabled` | TypeScript / iOS / Android | Turn comment review on or off. Turning it off does not clear comments already waiting for review. See [Comment Review](/social-plus-sdk/social/content-management/comments/moderation/comment-review) |

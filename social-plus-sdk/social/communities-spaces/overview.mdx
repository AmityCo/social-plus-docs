...
| `needApprovalOnCommentCreation` (TypeScript) / `isCommentReviewEnabled` (iOS) / `isCommentReviewEnabled()` (Android) | Boolean | Whether members' comments are held for moderator review. Not exposed on Flutter. See [Comment Review](/social-plus-sdk/social/content-management/comments/moderation/comment-review) |

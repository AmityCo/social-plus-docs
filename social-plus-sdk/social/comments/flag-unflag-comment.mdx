---
title: "Flag & Unflag Comments"
description: "Comprehensive content moderation tools for reporting and managing inappropriate comments"
---
...
class CommentModerationActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        moderationManager = CommentModerationManager(AmityCommentRepository.getComment())
        ...
        AlertDialog.Builder(this)
            ...
            .setItems(reasons) { _, which ->
                val reason = when (which) {
                    0 -> AmityContentFlagReason.CommunityGuidelines
                    1 -> AmityContentFlagReason.HarassmentOrBullying
                    2 -> AmityContentFlagReason.ViolenceOrThreateningContent
                    3 -> AmityContentFlagReason.SpamOrScams
                    4 -> AmityContentFlagReason.SexualContentOrNudity
                    5 -> AmityContentFlagReason.FalseInformation
...
class CommentModerationService {
  ...
  async flagComment(options: FlagCommentOptions): Promise<boolean> {
    ...
    try {
      ...
      // Handle custom reason with description
      if (reason === ContentFlagReasonEnum.Others && customDescription) {
        flagReason = ContentFlagReasonEnum.Others;
  ...
  getFlagReasons(): Array<{ code: Amity.ContentFlagReason; label: string; description: string }> {
    return [
      {
        code: ContentFlagReasonEnum.CommunityGuidelines,
        label: 'Community Guidelines',
        description: 'Violates community standards'
      },
      {
        code: ContentFlagReasonEnum.HarassmentOrBullying,
        label: 'Harassment or Bullying',
        description: 'Targeted harassment or bullying behavior'
      },
      {
        code: ContentFlagReasonEnum.SelfHarmOrSuicide,
        label: 'Self-Harm or Suicide',
        description: 'Content promoting self-harm or suicide'
      },
      {
        code: ContentFlagReasonEnum.ViolenceOrThreateningContent,
        label: 'Violence or Threats',
        description: 'Violent or threatening content'
      },
      {
        code: ContentFlagReasonEnum.SellingRestrictedItems,
        label: 'Restricted Items',
        description: 'Selling or promoting restricted items'
      },
      {
        code: ContentFlagReasonEnum.SexualContentOrNudity,
        label: 'Sexual Content',
        description: 'Inappropriate sexual content or nudity'
      },
      {
        code: ContentFlagReasonEnum.SpamOrScams,
        label: 'Spam or Scams',
        description: 'Spam or fraudulent content'
      },
      {
        code: ContentFlagReasonEnum.FalseInformation,
        label: 'False Information',
        description: 'Misinformation or fake news'
      },
      {
        code: ContentFlagReasonEnum.Others,
        label: 'Other',
  ...
  async flagCommentWithDialog(commentId: string): Promise<boolean> {
    ...
    if (selectedReason === ContentFlagReasonEnum.Others) {
  ...
  private async showReasonDialog(reasons: Array<{ code: Amity.ContentFlagReason; label: string }>): Promise<Amity.ContentFlagReason | null> {
    ...
    return new Promise((resolve) => {
      ...
      resolve(ContentFlagReasonEnum.CommunityGuidelines); // Placeholder
    });
  }
...
try {
  const flagged = await moderationService.flagComment({
    commentId: 'comment123',
    reason: ContentFlagReasonEnum.HarassmentOrBullying
  });
...
class CommentModerationService {
  ...
  CommentModerationService(this._commentRepository);
  ...
  Future<bool> flagComment({
    ...
    try {
      await _commentRepository.flagComment(
        commentId: commentId,
  ...
  Future<bool> unflagComment(String commentId) async {
    try {
      await _commentRepository.unflagComment(
        commentId: commentId,
      );
      return true;
    } catch (error) {
  ...
  Future<bool> isCommentFlaggedByMe(String commentId) async {
    try {
      final result = await _commentRepository
  ...
  AmityContentFlagReason _mapToAmityFlagReason(CommentFlagReason reason) {
    switch (reason) {
      case CommentFlagReason.communityGuidelines:
        return AmityContentFlagReason.communityGuidelines;
      case CommentFlagReason.harassmentOrBullying:
        return AmityContentFlagReason.harassmentOrBullying;
      case CommentFlagReason.selfHarmOrSuicide:
        return AmityContentFlagReason.selfHarmOrSuicide;
      case CommentFlagReason.violenceOrThreateningContent:
        return AmityContentFlagReason.violenceOrThreateningContent;
      case CommentFlagReason.sellingRestrictedItems:
        return AmityContentFlagReason.sellingRestrictedItems;
      case CommentFlagReason.sexualContentOrNudity:
        return AmityContentFlagReason.sexualContentOrNudity;
      case CommentFlagReason.spamOrScams:
        return AmityContentFlagReason.spamOrScams;
      case CommentFlagReason.falseInformation:
        return AmityContentFlagReason.falseInformation;
...
class _CommentFlagWidgetState extends State<CommentFlagWidget> {
  ...
  void initState() {
    super.initState();
    _moderationService = CommentModerationService(AmitySocialClient.newCommentRepository());
    _checkFlagStatus();
  }
  ...
  <Accordion title="Content Policy Enforcement">
    ...
    **Implementation**:
    ...
    const policyViolations = {
      [ContentFlagReasonEnum.SexualContentOrNudity]: 'ADULT_CONTENT_POLICY',
      [ContentFlagReasonEnum.ViolenceOrThreateningContent]: 'VIOLENCE_POLICY',
      [ContentFlagReasonEnum.FalseInformation]: 'MISINFORMATION_POLICY'
    };

...
| `pageSize` | Integer | Number of comments to load per page | `20` (iOS), `15` (Android), `10` (Web) |
...
class CommentQueryManager {
    ...
    func queryAllComments(
        ...
        commentCollection = commentRepository.getComments(with: options)
    ...
    func queryReplies(
        ...
        referenceId: String,
        referenceType: AmityCommentReferenceType,
        pageSize: Int = 5,          // Smaller page size recommended for reply threads
        completion: @escaping (Result<[AmityComment], Error>) -> Void
    ) {
        let options = AmityCommentQueryOptions(
            referenceId: referenceId,
...
import com.amity.socialcloud.sdk.api.social.AmityCommentRepository
import com.amity.socialcloud.sdk.api.social.comment.query.AmityCommentSortOption
import com.amity.socialcloud.sdk.model.social.comment.AmityComment
import io.reactivex.rxjava3.disposables.Disposable

class CommentQueryManager(private val commentRepository: AmityCommentRepository) {
    
    // Query all comments for a post (15 per page by default)
    fun queryAllComments(
        ...
        return commentRepository.getComments()
            ...
            .subscribe { pagingData -> callback(pagingData) }
    }
    ...
    fun queryParentComments(
        ...
        return commentRepository.getComments()
            .post(postId = postId)
            .parentId(null)             // null = root-level comments only
            .includeDeleted(false)
            .pageSize(pageSize)
            .build()
            .query()
    ...
    // Advanced query with custom filters and page size
...
class PostCommentsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        commentQueryManager = CommentQueryManager(AmitySocialClient.newCommentRepository())
...
<Warning>
  **TypeScript SDK gap**: `AmityCommentQuery` and `AmityCommentSortOption` are not exported from the TypeScript SDK. Android and Flutter expose these as typed query objects. In TS, pass query params inline and use string literals for sort options (e.g. `'lastCreated'`). Tracking: see `.docs-ops/evals/sdk-tickets-to-file.md`.
</Warning>
<Tab title="TypeScript">
```typescript
import { Client, CommentRepository } from '@amityco/ts-sdk';
// AmityCommentQuery, AmityCommentSortOption — not exported from TS SDK (see Warning below); use inline types
...
interface CommentQueryOptions {
    ...
    /** Number of comments to load per page. Default: 10. Use `5` for reply thread batches. */
    pageSize?: number;
    /** @deprecated Use `pageSize` instead. Will be removed in a future major version. */
    limit?: number;
}
...
class CommentQueryManager {
    ...
    constructor(client: Amity.Client) {
    ...
    async queryComments(
        ...
        onUpdate: (comments: Amity.Comment[]) => void,
    ...
    async queryReplies(
        ...
        referenceId: string,
        referenceType: Amity.CommentReferenceType,
        onUpdate: (replies: Amity.Comment[]) => void,
        onError?: (error: Error) => void,
        pageSize: number = 5            // Smaller page size recommended for reply threads
    ): Promise<string> {
        return this.queryComments({
            referenceId,
            referenceType,
    ...
    async createPaginatedQuery(
        options: CommentQueryOptions
    ): Promise<Amity.LiveCollection<Amity.Comment>> {
        const query: AmityCommentQuery = {
    ...
    async loadMoreComments(liveCollection: Amity.LiveCollection<Amity.Comment>): Promise<void> {
...
function useComments(
    ...
    const [comments, setComments] = React.useState<Amity.Comment[]>([]);
    const [loading, setLoading] = React.useState(true);
    const [error, setError] = React.useState<Error | null>(null);
    const [hasMore, setHasMore] = React.useState(true);
    
    const queryManagerRef = React.useRef<CommentQueryManager | null>(null);
    const liveCollectionRef = React.useRef<Amity.LiveCollection<Amity.Comment> | null>(null);
    
    React.useEffect(() => {
        // client is the instance returned by Client.createClient() during initialization
...
const CommentsSection: React.FC<{ postId: string }> = ({ postId }) => {
    const { comments, loading, error, hasMore, loadMore } = useComments(
        postId,
        'post',
        { parentId: null, pageSize: 10 } // Only parent comments; 10 per page
    );
    
    const [repliesMap, setRepliesMap] = React.useState<Map<string, Amity.Comment[]>>(new Map());
    
    const loadReplies = async (parentCommentId: string) => {
        const queryManager = new CommentQueryManager(client); // client from createClient()
...
<Tab title="Flutter">
<Warning>
**Flutter SDK gap**: `AmityCommentQuery` query-builder type is not available in the Flutter SDK. Android has `AmityCommentQuery` for typed query builders. Tracking: see `.docs-ops/evals/sdk-tickets-to-file.md`.
</Warning>
...
class CommentQueryManager {
  ...
  String queryComments({
    ...
    AmityCommentSortOption sortBy = AmityCommentSortOption.FIRST_CREATED,
    required Function(List<AmityComment>) onUpdate,
    Function(Object)? onError,
  }) {
    // (Flutter SDK gap: missing typed comment query builder)
    // final query = /* unavailable in Flutter SDK */;
    throw UnimplementedError('Typed comment query builders are not available in Flutter SDK');
  }
...
class _CommentsWidgetState extends State<CommentsWidget> {
  ...
  Future<void> _createReply(String parentCommentId, String text) async {
    try {
      await AmitySDK.instance.commentRepository
          .createComment()
          .post(widget.postId)
          .parentId(parentCommentId)
          .create()
...
_Refer to creation guidelines in the [Comment Creation](/social-plus-sdk/social/content-management/comments/creation/text-comment) page for media usage constraints._
...
|-----------|-------------|---------|---------------|
| **Page Size** (`pageSize`) | Comments loaded per page — set via the `pageSize` parameter | `20` iOS · `15` Android · `10` Web | Use `5` for reply thread batches; `10–20` for main feeds |
...
const VirtualizedComments: React.FC<{ comments: Amity.Comment[] }> = ({ comments }) => {
...
class CommentCache {
    private cache = new Map<string, Amity.Comment>();
    private maxSize = 500;
    
    set(commentId: string, comment: Amity.Comment) {
    ...
    get(commentId: string): Amity.Comment | undefined {

...
```typescript
import { Client, CommentRepository, UserRepository } from '@amityco/ts-sdk';
...
class CommentMentionManager {
    ...
    // Search users for autocomplete
    async searchUsers(query: string, limit: number = 10): Promise<Amity.User[]> {
        const userQuery = {
            displayName: query,
            limit
        };
    ...
    parseMentions(text: string, users: Amity.User[]): MentionData[] {
...
const MentionInput: React.FC<MentionInputProps> = ({
    ...
    const [suggestions, setSuggestions] = React.useState<Amity.User[]>([]);
    const [mentionQuery, setMentionQuery] = React.useState('');
    const [mentionStart, setMentionStart] = React.useState(-1);
    
    const mentionManager = new CommentMentionManager(client); // client from createClient()
    ...
    const insertMention = (user: Amity.User) => {
...
<Tab title="Flutter">
<Warning>
**Flutter SDK gap**: `AmityUserQuery` is not available in the Flutter SDK. Android has `AmityUserQuery` for typed user query builders. Tracking: see `.docs-ops/evals/sdk-tickets-to-file.md`.
</Warning>
...
class CommentMentionManager {
  ...
  AmityCommentCreationTarget _createCommentTarget({
    required String referenceId,
    required AmityCommentReferenceType referenceType,
    String? parentId,
  }) {
    late final AmityCommentCreationTarget target;

    switch (referenceType) {
      case AmityCommentReferenceType.POST:
        target = _amitySDK.commentRepository.createComment().post(referenceId);
        break;
      case AmityCommentReferenceType.STORY:
        target = _amitySDK.commentRepository.createComment().story(referenceId);
        break;
      case AmityCommentReferenceType.CONTENT:
        target = _amitySDK.commentRepository.createComment().content(referenceId);
        break;
    }

    return parentId != null ? target.parentId(parentId) : target;
  }
  ...
  Future<AmityComment> createCommentWithMentions({
    ...
    return await _createCommentTarget(
      referenceId: referenceId,
      referenceType: referenceType,
      parentId: parentId,
    ).create().text(text).mentionUsers(mentionUserIds).metadata(mentionMetadata).send();
  }
  ...
  Future<List<AmityUser>> searchUsers(String query, {int limit = 10}) async {
    // (Flutter SDK gap: missing typed user query builder)
    // final userQuery = /* unavailable in Flutter SDK */;
    throw UnimplementedError('Typed user query builders are not available in Flutter SDK');
  }
  ...
  Future<AmityComment> updateCommentWithMentions(
    ...
    return await _amitySDK.commentRepository

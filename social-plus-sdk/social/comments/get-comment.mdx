...
class CommentDetailActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        retrievalManager = CommentRetrievalManager(AmityCommentRepository.getComment())
...
class CommentRetrievalService {
  ...
  observeComment(
    commentId: string,
    onChange: (comment: Amity.Comment | null) => void,
  ...
  async getCommentsByIds(commentIds: string[]): Promise<Map<string, Amity.Comment | null>> {
    const results = new Map<string, Amity.Comment | null>();
  ...
  // Helper methods
  private canEditComment(comment: Amity.Comment, currentUserId?: string): boolean {
    return !comment.isDeleted && 
           currentUserId === comment.userId &&
           this.isWithinEditWindow(comment);
  }
  
  private isWithinEditWindow(comment: Amity.Comment, windowHours: number = 24): boolean {
  ...
  private isCacheValid(comment: Amity.Comment, maxAgeMs: number = 60000): boolean {
...
function updateCommentDisplay(comment: Amity.Comment) {
...
class CommentRetrievalService {
  ...
  CommentRetrievalService(this._commentRepository);
  ...
  Future<AmityComment?> getComment(
    ...
    try {
      ...
      final comment = await _commentRepository
...
class _CommentDetailWidgetState extends State<CommentDetailWidget> {
  ...
  void initState() {
    super.initState();
    _retrievalService = CommentRetrievalService(AmitySocialClient.newCommentRepository());
    _loadCommentDetails();
  }
...
// Usage examples
void main() async {
  final retrievalService = CommentRetrievalService(AmitySocialClient.newCommentRepository());

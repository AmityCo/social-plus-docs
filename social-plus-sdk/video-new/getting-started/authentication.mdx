...
```typescript
import { StreamRepository, Client } from '@amityco/ts-sdk-react-native';
...
class VideoAuthManager {
  ...
  static async authenticateUser(
    ...
    try {
      // Authentication is handled by Client.login — video SDK does not expose a separate authenticateUser method
      const result = await Client.login({ userId, displayName, authToken });

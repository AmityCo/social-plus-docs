...
<CodeGroup>
```swift iOS
let roomLiveObject = AmityRoomRepository().getRoom(roomId: "room-123")
...
<CodeGroup>
```swift iOS
let roomsCollection = AmityRoomRepository().getRooms(
    statuses: [.live, .idle],
    types: [.coHosts],
    isDeleted: false,
    sortBy: .firstCreated
...
<CodeGroup>
```swift iOS
let liveRooms = AmityRoomRepository().getRooms(
    statuses: [.live],
    types: nil,
    isDeleted: false,
    sortBy: .lastCreated
...
Task { @MainActor in
    do {
        let updatedRoom = try await AmityRoomRepository().updateRoom(
            roomId: "room-123",
...
Task { @MainActor in
    do {
        try await AmityRoomRepository().stop(roomId: "room-123")
...
```kotlin Android
AmityRoomRepository.stopRoom(roomId = "room-123")
    .subscribe(
        onComplete = {
...
Task { @MainActor in
    do {
        let recordedUrls = try await AmityRoomRepository().getRecordedUrls(
            roomId: "room-123"
        )
...
```swift iOS
let roomLiveObject = AmityRoomRepository().getRoom(roomId: roomId)
...
<Info>
**Automatic Thumbnails**: When a room is live, the backend automatically populates `liveThumbnailUrl` with an auto-generated thumbnail from the stream. Use this as a fallback when no creator-uploaded thumbnail (`thumbnailFileId`) is available. See [Rooms Overview - Automatic Thumbnail Generation](./rooms-overview#automatic-thumbnail-generation) for the complete fallback chain.
</Info>
...
func stopStreaming(roomId: String) async {
    do {
        endBroadcast()
        try await AmityRoomRepository().stop(roomId: roomId)
        print("Streaming stopped")
        
        // Wait for recording
        DispatchQueue.main.asyncAfter(deadline: .now() + 30) {
            Task {
                if let urls = try? await AmityRoomRepository().getRecordedUrls(roomId: roomId),
...
fun startStreaming(roomId: String) {
    AmityRoomRepository.getBroadcasterData(roomId = roomId)
        .subscribe(
...
fun stopStreaming(roomId: String) {
    endBroadcast()
    AmityRoomRepository.stopRoom(roomId = roomId)
        .subscribe(
            onComplete = {
    ...
    .subscribe(
        ...
        onError = { error ->
            when {
                ...
                error is AmityException && error.code == AmityError.ITEM_NOT_FOUND.code -> {
                    showError("Room not found")
                }
                error is AmityException.InvalidStatus -> {
                    showError("Stream is not currently active")
                }
                else -> {

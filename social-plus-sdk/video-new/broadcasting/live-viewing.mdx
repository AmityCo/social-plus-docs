...
### Thumbnail and Resolution

Alongside the playback URL, the room exposes a thumbnail and the stream's dimensions — useful for a poster image before playback starts and for sizing the player without waiting for the first frame.

| Field | TypeScript | iOS | Android | Available |
| --- | --- | --- | --- | --- |
| Live thumbnail | `room.liveThumbnailUrl` | `room.liveThumbnailUrl` | `room.getLiveThumbnailUrl()` | Live or reconnecting sessions, when generated |
| Live resolution | `room.liveResolution` | `room.liveResolution` | `room.getLiveResolution()` | Live or reconnecting sessions, when available |
| Recorded resolution | `room.recordedResolution` | `room.recordedResolution` | `room.getRecordedResolution()` | After the recording is available |

Resolution carries the stream's aspect ratio, width, and height. These fields can be absent early in a session, so treat them as optional and fall back to your own placeholder. See [Rooms Overview](./rooms-overview) for the full room field reference.

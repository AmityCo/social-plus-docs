...
class LiveRoomViewer(private val context: Context) {
    ...
    fun getRoomFromPost(post: AmityPost) {
        ...
        roomData?.let { data ->
            ...
            // getStatus() returns a String: "idle", "live", "ended", or "recorded"
            when (room.getStatus()) {
                "live" -> room.getLivePlaybackUrl()?.let { playLiveRoom(it) }
                "recorded" -> room.getRecordedPlaybackInfos().firstOrNull()?.url?.let { playRecordedRoom(it) }
                "ended" -> showMessage("Room has ended, recording will be available soon")
                "idle" -> showMessage("Room hasn't started yet")
            }
        }
    }
    ...
    private fun updateUI(room: AmityRoom) {
        when (room.getStatus()) {
            ...
            AmityRoomStatus.LIVE -> {
                showPlayer()
                room.getLivePlaybackUrl()?.let { startLivePlayback(it) }
            ...
            AmityRoomStatus.RECORDED -> {
                showPlayer()
                room.getRecordedPlaybackInfos().firstOrNull()?.url?.let { startRecordedPlayback(it) }
...
class RoomStateManager {
    ...
    observeRoomState(roomId: string) {
        this.unsubscribe = RoomRepository.getRoom(roomId, (room) => {
            this.updateUI(room);
        });
    }

...
| Unknown mode | Falls back to `alwaysShow`; a valid threshold in `[1, 1000]` is retained | `{ alwaysShow, <threshold> }` |
| Missing or out-of-range threshold, under any mode | Returns the default config and discards the selected mode | `{ alwaysShow, 50 }` |
...
<Warning>
  Fail-open applies even when the mode itself parsed cleanly: a `hideEntirely` config carrying a missing or out-of-range threshold resolves to `{ alwaysShow, 50 }`, so viewers see a count on a network whose admin chose to hide it. Threshold validation runs before the mode is read.
</Warning>

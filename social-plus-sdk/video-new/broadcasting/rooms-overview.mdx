...
Rooms provide an advanced broadcasting infrastructure that enables interactive live streaming with co-hosting capabilities. From room creation to live broadcasting and recorded playback, this guide covers the complete streaming lifecycle.
...
To enable live chat, create a channel **after** the post is created using both `postId` and `roomId`:

<CodeGroup>

```swift iOS
let channel = try await AmityChatClient.shared.channelRepository
...
func getBroadcasterData(roomId: String) async {
    do {
        ...
        if case .coHosts(let data) = broadcastData {
...
fun getBroadcasterData(
    ...
    roomRepository.getBroadcastData(roomId = roomId)
        .doOnSuccess { broadcastData: AmityRoomBroadcastData ->
            if (broadcastData is AmityRoomBroadcastData.CoHosts) {
                // Use coHostToken and coHostUrl for multi-host streaming

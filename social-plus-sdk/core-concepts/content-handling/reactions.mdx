...
const LiveReactionsType = ReactionRepository.getReactions(
  {
    referenceType: "post",
    referenceId: "YOUR_COMMENTID"
  },
  ({ data: reactions }) => {
...
```dart Flutter
  ...
  void getReactions() {
    ...
    // Initialize reaction live collection
    reactionLiveCollection = AmitySocialClient.newReactionRepository()
        .getReactions(reactionReference)
        // Optional: filter by specific reaction name, e.g., "like"
        .reactionName('like')
        .getLiveCollection();
    ...
    // Add pagination listener when scrolling to top/bottom

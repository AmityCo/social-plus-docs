---
title: "Reactions"
description: "Query, add, and remove reactions on posts, comments, stories, and messages with the social.plus SDKs."
---
...
| --- | --- | --- | --- |
| `referenceType` | Yes | TypeScript, iOS, Android, Flutter | Content type being reacted to. Supported values are post, comment, story, and message. |
| `referenceId` | Yes | TypeScript, iOS, Android, Flutter | ID of the post, comment, story, or message. |
| `reactionName` | Yes for add/remove, optional for query | TypeScript, iOS, Android, Flutter | Reaction key such as `like`. Query APIs can use it as a filter. |
| `callback` / live collection observer | Query only | TypeScript, iOS, Android, Flutter | Used by live collection APIs to receive reaction updates. |

...
Read video status and resolution URLs when your UI needs to render playback or processing state. The returned video file also exposes its frame dimensions. On TypeScript, iOS, and Android these are `getWidth()`, `getHeight()`, and `getRotation()`; on Flutter the video exposes `width` and `height` only (no rotation). Width and height are the **stored** (pre-rotation) values — apply the rotation to get the displayed orientation, swapping width and height when the rotation is ±90°.
...
if rawFile.type == .video, let videoData = rawFile.mapToVideoData() {
    ...
    let width = videoData.getWidth()
    let height = videoData.getHeight()
    let rotation = videoData.getRotation()
}
```
...
void inspectVideo(AmityVideo video) {
  ...
  // Stored frame dimensions (no rotation on Flutter).

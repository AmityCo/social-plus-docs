...
Width and height are the **stored** (pre-rotation) values. Apply the rotation to get the displayed orientation: swap width and height when `rotation % 180 != 0`.

<Warning>
Flutter exposes the stored width and height but **no rotation**. Width and height alone cannot determine the display orientation of a rotated video; use a media player or parser that handles orientation metadata.
</Warning>

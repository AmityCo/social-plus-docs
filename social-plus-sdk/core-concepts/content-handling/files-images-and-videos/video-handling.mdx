...
Read video status and resolution URLs when your UI needs to render playback or processing state. The returned video file also exposes its frame dimensions through `getWidth()`, `getHeight()`, and `getRotation()`. Width and height are the **stored** (pre-rotation) values — apply the rotation to get the displayed orientation, swapping width and height when the rotation is ±90°.

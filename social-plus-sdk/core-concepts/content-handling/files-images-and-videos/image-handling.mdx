...
description: "Upload images, read image metadata, and request sized image URLs with the social.plus SDKs."
---
...
async function getImage(imageFileId: string) {
  // getFile does not narrow its return type, so cast to the image file shape —
  // without it `metadata` widens to a union and the dimensions come back `unknown`.
  const { data } = await FileRepository.getFile(imageFileId);
  const image = data as Amity.File<'image'>;

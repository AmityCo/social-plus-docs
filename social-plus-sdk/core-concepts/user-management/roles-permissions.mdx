...
<Note>
Permission constants are not identical across every SDK. Use the enum or string constants exposed by your target SDK before passing a value to `hasPermission()`. Some Console/API permissions, such as community category or network notification settings, may not be available as typed client SDK permissions on every platform.
</Note>

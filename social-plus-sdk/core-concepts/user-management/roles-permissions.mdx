...
Task { @MainActor in
    let canDelete = await client.hasPermission(.deleteMessage, forChannel: channelId)

    if canDelete {
        // Enable delete-message controls
    } else {
        // Hide delete-message controls
    }
}
```
...
fun checkBanPermission(channelId: String) {
    AmityCoreClient.hasPermission(AmityPermission.BAN_CHANNEL_USER)
        .atChannel(channelId)
        .check()
        .doOnNext { canBan ->
            if (canBan) {
...
const canEdit = client.hasPermission('EDIT_COMMUNITY_POST').community(communityId);
...
```dart Flutter
// Check if user has permission to mute a user inside a channel
// Flutter uses MUTE_USER_INSIDE_CHANNEL (equivalent to MUTE_CHANNEL_USER on other platforms)
...
<Note>
Permission constants are not identical across every SDK. Use the enum or string constants exposed by your target SDK before passing a value to `hasPermission()`. Some Console/API permissions, such as community category, role management, or network notification settings, may not be available as typed client SDK permissions on every platform.
</Note>
...
<tr>
<td><code>PIN_COMMUNITY_POST</code></td>
<td>Can pin or unpin community feed posts where the target SDK exposes this permission</td>
</tr>
<tr>
...
<tr>
<td><code>CREATE_COMMUNITY_POST</code></td>
<td>Can create community posts; exposed as a typed permission in the current iOS SDK source</td>
</tr>
<tr>
...
<tr>
<td><code>CREATE_PRIVILEGED_POST</code></td>
<td>Can create privileged community posts where the target SDK exposes this permission</td>
</tr>
</tbody>
</table>
</div>
...
<td>Can create community events where event permissions are exposed by the target SDK</td>
</tr>
<tr>
<td><code>DELETE_EVENT</code></td>
<td>Can delete community events where event permissions are exposed by the target SDK</td>
</tr>
<tr>
<td><code>UPDATE_EVENT</code></td>
<td>Can update community events; exposed as a typed permission in the current iOS SDK source</td>
</tr>
<tr>
<td><code>CREATE_EVENT_WITH_LIVESTREAM</code></td>
<td>Can create events with livestream capability where exposed by the target SDK</td>
</tr>
<tr>
<td><code>MANAGE_COMMUNITY_EVENTS</code></td>
<td>Can manage community events where exposed by the target SDK</td>
</tr>
</tbody>
</table>
</div>
...
<td>Console/API permission for network notification settings; not exposed as a typed permission in the current SDK sources checked for this page</td>
</tr>
</tbody>
</table>
</div>

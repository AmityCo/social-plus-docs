...
- Use the permission constant that matches the action and scope. Permission names differ by platform enum casing, but they map to server permission strings such as `DELETE_MESSAGE`, `PIN_MESSAGE`, `EDIT_COMMUNITY_POST`, and `BAN_USER`.

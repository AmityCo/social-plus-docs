...
<Warning>
**Flutter SDK gap**: `AmityPermission.MUTE_CHANNEL_USER` is not available in the Flutter SDK. Flutter has `MUTE_COMMUNITY_USER`, `MUTE_CHANNEL`, and `MUTE_USER_INSIDE_CHANNEL` but not the combined `MUTE_CHANNEL_USER` permission. Tracking: see `.docs-ops/evals/sdk-tickets-to-file.md`.
</Warning>

```dart Flutter
// Check if user has permission to mute users
// (Flutter SDK gap: missing combined mute-channel-user permission)
// final canMute = await AmityCoreClient.hasPermission(
//     permission: /* unavailable in Flutter SDK */,
//     channelId: channelId
// );
final canMute = false;

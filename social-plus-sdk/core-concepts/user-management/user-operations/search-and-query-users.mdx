...
| Field | Platforms | TypeScript | iOS | Android |
| --- | --- | --- | --- | --- |
| Display name | TypeScript, iOS, Android | `SearchUsersByEnum.DISPLAY_NAME` | `.displayName` | `AmitySearchUserBy.DISPLAY_NAME` |
| User ID | TypeScript, iOS, Android | `SearchUsersByEnum.USER_ID` | `.userId` | `AmitySearchUserBy.USER_ID` |
| Profile handle | TypeScript, iOS, Android | `SearchUsersByEnum.PROFILE_HANDLE` | `.profileHandle` | `AmitySearchUserBy.PROFILE_HANDLE` |
...
| --- | --- | --- | --- | --- |
| TypeScript | `UserRepository.searchUsers(params, callback)` | `displayName` (the keyword), callback | `searchBy`, `limit`, `matchType` | Starts a live collection observer and returns an unsubscriber. |
| iOS | `userRepository.searchUsers(keyword, searchBy:, sortBy:, matchType:)` | `keyword`, `searchBy`, `sortBy` | `matchType` | Returns a live collection observed with an `AmityNotificationToken`. |
| Android | `userRepository.searchUsers(keyword).build().query()` | `keyword` | `searchBy(...)`, `sortBy(...)`, `matchType(...)` | Returns `Flowable<PagingData<AmityUser>>`. |
...
const unsubscribe = UserRepository.searchUsers(
  // `searchBy` is optional; omit it to match all fields.
  { displayName: 'Joe', limit: 20, searchBy: [SearchUsersByEnum.DISPLAY_NAME] },
...
func searchUserExample() {
    // `searchBy` selects which fields the keyword is matched against.
    // Use [.displayName] to match display name only; pass [] to keep the
    // default behavior (match all searchable fields).
...
fun searchUsers(userRepository: AmityUserRepository) {
    userRepository.searchUsers("Brian")
        .searchBy(listOf(AmitySearchUserBy.DISPLAY_NAME)) // optional; omit to match all fields

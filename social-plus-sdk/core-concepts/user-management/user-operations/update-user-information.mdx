...
| `avatarCustomUrl` | No | TypeScript, iOS, Android, Flutter | An external image URL used as the user's avatar instead of an uploaded `avatarFileId`. |
| `metadata` | No | TypeScript, iOS, Android, Flutter | Custom social metadata. Do not store sensitive personal data here. |

<Note>
Once `avatarCustomUrl` is set, the **UIKit renders it automatically** wherever a user's avatar appears — no extra wiring is needed. This is supported on Android UIKit 4.19.2+ and iOS UIKit 4.22.2+. When both `avatarFileId` and `avatarCustomUrl` are set, the custom URL takes precedence.
</Note>

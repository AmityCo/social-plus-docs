...
| 400323 | `VisitorUsageLimitExceeded` | Visitor/bot has exhausted their 100 req/day read quota — SDK emits `visitorUsageLimitReached` event |
</Accordion>

<Accordion title="Visitor & Bot Errors (403xxx)">
| Code | Constant | Description |
|------|----------|-------------|
| 403998 | `BOT_PERMISSION_DENIED` | Bot user attempted a write operation (create/update/delete) |
| 403999 | `VISITOR_PERMISSION_DENIED` | Visitor user attempted a write operation (create/update/delete) |

Write operations are blocked server-side for all visitor and bot sessions. These errors are returned synchronously on the API call; they do not affect the session.
</Accordion>

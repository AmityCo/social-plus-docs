...
description: "Observe social.plus Flutter SDK objects and collections with Dart streams, LiveCollection, and LiveResult."
---
...
| --- | --- | --- | --- |
| `postId` or another entity ID | Live object streams | Flutter | ID of the object to observe. |
| Query builder target | Live collections | Flutter | Scope for the collection, such as `targetCommunity(communityId)` or `targetUser(userId)`. |
| `pageSize` | Live collections | Flutter | Optional page size for `getLiveCollection(pageSize: ...)`. Defaults to 20 when omitted. |
| `LiveResult<T>.data` | Collection stream | Flutter | Current list snapshot. |
| `LiveResult<T>.isFetching` | Collection stream | Flutter | Whether the collection is currently fetching. |

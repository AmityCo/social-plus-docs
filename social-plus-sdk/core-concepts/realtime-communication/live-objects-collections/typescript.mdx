...
description: "Observe social.plus TypeScript SDK objects and collections with callbacks, pagination helpers, and real-time topic subscriptions."
---
...
| Parameter | Used by | Platforms | Description |
| --- | --- | --- | --- |
| `postId` or another entity ID | Live object methods | TypeScript | ID of the object to observe, such as a post, message, channel, community, user, poll, room, or stream. |
| `params.targetType` / `params.targetId` | Collection queries such as `PostRepository.getPosts()` | TypeScript | Scope for the list. For posts, `targetType` is commonly `user` or `community`. |
| `limit` | Collection queries | TypeScript | Optional page size for the query. |
| `callback` | Object and collection methods | TypeScript | Receives each live snapshot. Handle `loading` and `error` before rendering data. |
| `onNextPage` / `hasNextPage` | Live collections | TypeScript | Use when the UI asks for the next page. Do not call `onNextPage` repeatedly without checking `hasNextPage`. |
| `subscribeTopic(topic, callback?)` | Real-time updates | TypeScript | Subscribes the active client to a topic such as `getPostTopic(post)` or `getCommunityTopic(community, SubscriptionLevels.POST)`. |

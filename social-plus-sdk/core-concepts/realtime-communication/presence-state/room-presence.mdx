...
<Note>
**There's a floor to how fresh the count can be.** Viewers heartbeat roughly **every 30 seconds**, and a viewer is counted as present for **up to 60 seconds** after their last heartbeat. The count and list are always computed live (they aren't cached on a delay), but the underlying set of present users only changes as heartbeats arrive and expire — so polling much faster than the heartbeat cadence surfaces little new information while adding load.
</Note>
...
<Info>
**The list is capped; the count is not.** The who's-watching list returns the **most recently joined** active viewers, up to a maximum of **200** (the server default and hard cap). The [viewer count](#get-the-viewer-count) is **not** capped, so for rooms with large audiences it can be **higher** than the number of users in the list. Use the **count** for the total number of viewers, and the **list** for showing faces.
</Info>

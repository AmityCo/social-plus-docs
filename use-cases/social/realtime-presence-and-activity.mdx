...
// (real-time pub/sub not available via a generic .on() in TS SDK; use repository-specific observers)
```
...
sequenceDiagram
    ...
    participant Other as Other User's App

    App->>SDK: presence.enable()
    SDK->>Server: Register online
    loop Every 20-30 seconds
        SDK->>Server: Heartbeat ping
        Server-->>Server: Refresh TTL
    end
    Other->>SDK: syncUserPresence('user-123')
    SDK->>Server: Subscribe to user-123 presence
    Server-->>Other: status: online
    Note over App: User closes app
    App->>SDK: presence.dispose()
    SDK->>Server: Unregister
    Note over Server: Heartbeat TTL expires
    Server-->>Other: status: offline
```
...
<Tip>
**Dive deeper**: [Real-time Communication Reference](/social-plus-sdk/core-concepts/realtime-communication/overview) has full parameter tables, method signatures, and platform-specific details for every API used in this guide.
</Tip>

...
<Warning>
  **TypeScript SDK gap**: `PostRepository.createPollPost` is not available in the TypeScript SDK. iOS, Android, and Flutter expose this directly. In TS, use a two-step flow: `PollRepository.createPoll({{...}})` to get a pollId, then `PostRepository.createPost({{ dataType: 'poll', data: {{ pollId }} }})`. Tracking: see `.docs-ops/evals/sdk-tickets-to-file.md`.
</Warning>
```typescript
// 1. Create a poll
...
// 2. Wrap it in a post
// Poll post creation not in TS SDK — use PollRepository.createPoll + PostRepository.createPost (see Warning above)
...
<Frame caption="Polls in action — dedicated poll page with vote stats, and a product poll with live results">
  <img
    src="/images/use-cases/polls-hero-in-feed.png"
    alt="Two poll screens: dark-themed Studio Polls page with active polls count, vote percentages, and expiry timer; light-themed product poll asking 'Which color should we stock next?' with percentage bars and vote counts"
  />
</Frame>
...
<Steps>
  ...
  <Step title="Publish as a post in a feed">
    ...
    // Poll post creation not in TS SDK — use PollRepository.createPoll + PostRepository.createPost
    // pollId, text, targetType, targetId go to PostRepository.createPost({ dataType: 'poll', data: { pollId }, ... })
    });
    ```

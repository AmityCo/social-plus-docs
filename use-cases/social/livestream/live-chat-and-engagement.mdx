...
- UIKit installed if using the pre-built components → [UIKit Getting Started](/uikit/getting-started/overview)
</Info>
...
```typescript
import { MessageRepository, ChannelRepository, ReactionRepository } from '@amityco/ts-sdk';
...
```typescript
// Add a reaction
await ReactionRepository.addReaction('message', message.messageId, 'fire');

// Remove a reaction
await ReactionRepository.removeReaction('message', message.messageId, 'fire');
```
...
<AccordionGroup>
  ...
  <Accordion title="Ban users from chat" icon="ban">
    ...
    ```typescript
    await ChannelRepository.Moderation.banMembers(channel.channelId, ['user-id']);
    ```
  </Accordion>

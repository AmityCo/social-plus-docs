...
social.plus provides a full livestream stack built on top of LiveKit. A broadcast lives inside a **Room**, which owns the live chat channel, co-host invitations, and recording lifecycle. Products are tagged on the published room post (the child room post), not on the Room entity itself. This section breaks the livestream workflow into focused guides.

...
// 2. Generate a share URL for a post (null if the post pattern is not configured)
const shareUrl = config.generateLink(AmitySharableContentType.POST, postId);

// 3. Handle incoming deep link
// (deep link routing is app-layer responsibility — not part of the TS SDK)
handleDeepLink(incomingUrl, (resolved) => {
...
const getPostShareLink = async (postId: string) => {
  try {
    const config = await Client.getShareableLinkConfiguration();
    // generateLink returns null when the post pattern is not configured.
...
<Steps>
  <Step title="Get the shareable link configuration">
    ...
    const config = await Client.getShareableLinkConfiguration();
    // config.domain — your configured base domain
    // config.isEnabled(type) — whether a content type has a link pattern configured
    // config.getPattern(type) — the raw pattern for a content type
    // config.generateLink(type, referenceId) — the ready-to-share URL (null if unconfigured)
    ```
  ...
  <Step title="Generate links for different content types">
    ...
    // Post link
    const postLink = config.generateLink(AmitySharableContentType.POST, postId);

    // Community link
    const communityLink = config.generateLink(AmitySharableContentType.COMMUNITY, communityId);

    // User profile link
    const userLink = config.generateLink(AmitySharableContentType.USER, userId);
    ```
...
<Frame caption="UIKit — Shareable link configuration with customizable URL patterns">
  <img src="/images/uikit-shareable-link.png" alt="UIKit — Shareable link component" />
</Frame>

<AccordionGroup>
  <Accordion title="Share analytics" icon="chart-bar">
    Shareable links are generated on the client, so the SDK does not emit a built-in share event or a per-post share count. To measure sharing, log your own analytics event when a user taps share, and combine it with [post impressions](/social-plus-sdk/social/content-management/posts/analytics/post-impressions) for a reach picture.
  </Accordion>

...
<Steps>
  ...
  <Step title="Assign community moderator roles">
    ...
    await ChannelRepository.Moderation.addRole(communityId, 'userId', ['moderator']);

    // Remove a role
    await ChannelRepository.Moderation.removeRole(communityId, 'userId', ['moderator']);
    ```
  ...
  <Step title="Review and approve pending posts">
    ...
    const isUnderReview = post.feedType === Amity.FeedType.Reviewing;
    const isPublished  = post.feedType === Amity.FeedType.Published;
    const isDeclined   = post.feedType === Amity.FeedType.Declined;
...
<Tip>
**Dive deeper**: [User Management API Reference](/social-plus-sdk/core-concepts/user-management/overview) has full parameter tables, method signatures, and platform-specific details for every API used in this guide.
</Tip>

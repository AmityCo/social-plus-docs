...
description: "Moderate user profiles with AI-powered scanning, admin reset from the Users tab or User History, profile blocklists, and webhook automation."
---
...
| Admin reset | N/A | ✅ Tick which fields to reset — flagged profiles from the Users tab, any user from User History |
...
<Steps>
  ...
  <Step title="Reset a profile">
    Whichever route you take, fields are restored to these defaults:
    ...
    There are two places to reset from, both opening the **same reset modal** where you tick the fields to restore:
    ...
    To reset a profile that was never flagged — or to reset just one field, such as an avatar while leaving the display name alone — open the user's [User History](/analytics-and-moderation/console/user-and-content-management/user-social-history) page, click the three-dot (**⋮**) menu, and choose **Profile moderation → Reset profile**. Tick the fields to restore; unticked fields are left untouched.

    The user receives a `USER_PROFILE_RESET` notification through the notification tray listing the fields that were reset.

...
<Warning>
  **TypeScript SDK gap**: `AdRepository.markAdSeen` and `AdRepository.markAdClicked` are not yet available in the TypeScript SDK. Flutter exposes these as `AmityAdRepository` impression/click tracking. Tracking: see `.docs-ops/evals/sdk-tickets-to-file.md`.
</Warning>
```typescript
// 1. Fetch network ads
...
// Ad impression tracking not in TS SDK (see Warning)

// 3. Track a click
// Ad click tracking not in TS SDK (see Warning)
```

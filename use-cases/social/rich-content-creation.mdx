...
<Warning>
  **TypeScript SDK gap**: `PostRepository.createPollPost` is not available in the TypeScript SDK. Use a two-step flow: `PollRepository.createPoll({...})` then `PostRepository.createPost({ dataType: 'poll', data: { pollId } })`. Tracking: see `.docs-ops/evals/sdk-tickets-to-file.md`.
</Warning>
```typescript
...
// Poll post creation not in TS SDK — see Warning above
```
...
<AccordionGroup>
  <Accordion title="Post review queues" icon="hourglass">
    ...
    → [Post Review](/social-plus-sdk/social/content-management/posts/moderation/post-review) · [Admin Console: Posts](/analytics-and-moderation/console/social-management/social-management/posts)
  </Accordion>
  ...
  <Accordion title="Product-tagged posts in the console" icon="tag">
    ...
    → [Product Tagging & Social Commerce](/use-cases/social/product-tagging-and-social-commerce)
  </Accordion>
</AccordionGroup>

...
const { data: pollPost } = await PostRepository.createPost({
  targetType: 'community', targetId: 'communityId',
  data: { text: 'Vote now! 👇', pollId: poll.pollId },
});
```
...
<Steps>
  ...
  <Step title="Edit or delete a post">
    ...
    // Update a post
    const { data: updated } = await PostRepository.editPost('postId', {
      data: { text: 'Updated content' },
      tags: ['tag1', 'tag2'],
    });

    // Soft-delete — hides from feeds, recoverable
    await PostRepository.softDeletePost('postId');

    // Hard-delete — permanent removal
    await PostRepository.hardDeletePost('postId');
    ```

...
UserRepository.searchUsers(
  { displayName: 'jane', limit: 20, searchBy: [SearchUsersByEnum.DISPLAY_NAME] },
...
graph TD
    ...
    C --> F[UserRepository.searchUsers]
...
const unsub = UserRepository.searchUsers(
  { displayName: 'Joe', searchBy: [SearchUsersByEnum.DISPLAY_NAME] },
  ({ data: users }) => {
...
<Steps>
  <Step title="Search users by display name">
    Use `searchUsers()` to search as the user types. Results are returned as a live collection that updates in real-time. Pass `searchBy` to control which user fields the keyword is matched against; omit it to keep the server default, which matches all searchable fields. `searchUserByDisplayName()` still works but is deprecated.
...
<AccordionGroup>
  ...
  <Accordion title="Visitor filtering" icon="eye-slash">
    Visitor and bot users are automatically excluded from search results. Only authenticated (`SIGNED_IN`) users appear in `searchUsers()` results.
  </Accordion>
</AccordionGroup>

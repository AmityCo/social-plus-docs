...
await ChannelRepository.Moderation.muteMembers(channelId, [userId], 60); // 60 seconds
...
// Unban
await ChannelRepository.Moderation.unbanMembers(channelId, [userId]);
...
<Steps>
  <Step title="Mute a user temporarily">
    ...
    await ChannelRepository.Moderation.muteMembers(channelId, [userId], 300);
    ...
    → [Mute / Unmute](/social-plus-sdk/chat/conversation-management/overview)
  </Step>
  <Step title="Ban and unban users from a channel">
    ...
    // Query banned users
    const banned = ChannelRepository.Membership.getMembers({
      channelId,
      filter: 'bannedMember',
    });
    ...
    → [Ban / Unban](/social-plus-sdk/chat/conversation-management/overview)
  </Step>

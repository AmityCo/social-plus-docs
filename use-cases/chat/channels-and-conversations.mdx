...
<Steps>
  <Step title="Choose the right channel type">
    ...
    → [Channel Types](/social-plus-sdk/chat/conversation-management/overview)
  </Step>
  ...
  <Step title="Join and leave channels">
    ...
    // Query members
    const members = ChannelRepository.Membership.getMembers({ channelId, limit: 20 });
    ```
  ...
  <Step title="Update and archive channels">
    ...
    // archiving: set archived_reason in metadata (no dedicated archive method in v6)
    ```
...
<AccordionGroup>
  <Accordion title="Channel moderation from Admin Console" icon="shield">
    ...
    → [Channel Moderation](/social-plus-sdk/chat/conversation-management/overview)
  </Accordion>
...
<Tip>
**Dive deeper**: [Conversation Management API Reference](/social-plus-sdk/chat/conversation-management/overview) has full parameter tables, method signatures, and platform-specific details for every API used in this guide.
</Tip>

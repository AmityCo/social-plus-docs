...
- Per-channel unread badge that live-updates
- Mention indicator (`isMentioned`) distinct from the unread count
- `markAsRead` call to reset badges when the channel opens
- Per-message delivered / read ticker (like WhatsApp's ✓✓)
...
<AccordionGroup>
  <Accordion title="Push notifications on unread" icon="bell">
    ...
    → [Push Notifications Setup](/social-plus-sdk/core-concepts/foundation/push-notification)
  </Accordion>
...
<CardGroup cols={3}>
  ...
  <Card title="Build a 1:1 Chat" href="/use-cases/chat/build-a-1on1-chat" icon="comments">
    Full 1:1 chat trail where read receipts matter most.
  </Card>
</CardGroup>

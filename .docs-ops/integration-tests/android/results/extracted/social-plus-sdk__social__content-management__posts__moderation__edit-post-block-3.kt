// source: social-plus-sdk/social/content-management/posts/moderation/edit-post.mdx:362-415
...
fun docSnippet(
    ...
    fun updateImagePost(
      ...
      val productTags = listOf(
        AmityProductTag.Text(productId = "product1", text = "product1", index = 0, length = 8),
        AmityProductTag.Text(productId = "product2", text = "product2", index = 9, length = 8)
      )

      val attachmentProductTags = AmityAttachmentProductTags.Builder()
        .apply {
          set("fileId1", listOf(AmityProductTag.Media(productId = "product1")))
        }
        .build()
      ...
      val links = listOf(
        AmityLink(index = null, length = null, url = "https://example.com", renderPreview = false, domain = null, title = "Example", imageUrl = null)
      )

      postRepository.editPost(postId = "post1")
        .text("product1 product2 updated text!")
        .title("updated title")
        .attachments(*attachments.toTypedArray())
        .metadata(metadata)
        .mentionUsers(listOf("user1", "user2"))
        .hashtags(listOf("hashtag1", "hashtag2"))

---
title: "Manage access"
description: "Control who can ask about your community's data — connect codes, connector credentials, and connected apps in the Console."
---
...
<Info>
  If the page says Agentry isn't enabled, contact support to turn it on for your network.
</Info>

## The access model in one paragraph

Agentry is **read-only** and scoped to **one community's data**: the only thing any connection can do is ask questions of your engagement data. Who gets to ask is entirely in your hands — every connection exists because a super admin issued a connect code or a credential, every connection is listed, and any of them can be removed at any time without affecting the others.

## Via AI app

### Connect codes

A connect code links one application sign-in to your community's data.
...
<Steps>
  ...
  <Step title="Choose what it grants">
    Pick the scopes the code grants (for example, *Read your community's engagement data*). A code must grant at least one thing.
  </Step>
  <Step title="Hand it over">
    The code is shown **once** — copy it immediately. Codes are single-use and expire quickly (the exact expiry is shown when you generate one). If a code is lost or goes unused, generate another.
  </Step>
</Steps>
...
- Agentry connections can only **read engagement data** — there is no scope that writes, moderates, or administers anything.
- Access is community-scoped end to end: a connection to your network cannot see any other network's data.
- Under the hood this is standard OAuth 2.1 — short-lived access tokens and rotating refresh tokens — so removal takes effect at the next token refresh, which is why "within about 15 minutes" appears above.

...
<Info>
  Directory listings for ChatGPT and Claude are in review. Until they are approved, connect Claude through the [custom connector path](#via-mcp) below; the directory steps are what you'll follow once the listings are live.
</Info>
...
<Tabs>
  ...
  <Tab title="Any other MCP client">
    Any MCP-capable app connects the same way: point it at the MCP server URL and authenticate with a connector credential.
